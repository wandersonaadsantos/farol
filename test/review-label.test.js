...
// vez. O contrato aqui é o dos GUARDAS e da SEQUÊNCIA de chamadas, provados com um
// runner injetado (sem rede): a composição do nome é pura; sem conta ou sem token o
// gh NUNCA é chamado (mesma raiz A1 do resto do engine: agir sem identidade provada
// é pior que não agir); e remoção sem label aplicada não toca o engine (a adição
// que falhou não gera remoção à toa). O caminho com gh de verdade é best-effort por
// construção e não tem teste de rede, como fetchPrFiles e afins.
import { test } from 'node:test';
import assert from 'node:assert/strict';

const { inProgressLabelFor, addInProgressLabel, removeInProgressLabel, repoDoPr } = await import('../lib/engine/review.js');
