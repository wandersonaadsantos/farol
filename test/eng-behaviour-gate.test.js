...
test('o arquivo versionado fixa a versao adotada e um commit completo', () => {
  ...
  assert.equal(f.versao, '0.12.0', 'a mesma versao em que o baseline foi aberto');
...
test('o gate, rodado de verdade, falha com codigo 2 quando a CLI apontada nao e a adotada', { timeout: 60_000 }, () => {
  const { home } = pacoteDeProva({ versao: '0.13.0' });
  ...
  assert.match(r.stderr, /declara a versao 0\.13\.0, e o Farol adota a 0\.12\.0/);
