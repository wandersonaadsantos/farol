...
Executada em 02/08/2026 (fim da manhã, horário de Brasília; commits das 11:50 às 12:07). Achados cobertos: M22, B11, M19, B13, B15, B14, B16 e B17. Suite na largada da onda: 506 testes; no fechamento: 526 (523 pass, 3 skipped de plataforma, esperados no Windows), com `npm run check` ok (64 arquivos). Todos os testes novos da onda vivem em `test/ui-widgets.test.js` (20 testes), montado cumulativamente como a D1 do rascunho previu. Verificação independente (adversarial) conferiu os 8 commits diff a diff contra o rascunho e as regras do mestre (R6, R9, R10, R13, R15 e R16, todas respeitadas) e provou por anti-tautologia (checkout temporário da baseline `908bdcc`, início da onda) que os 20 testes novos falham 20/20 sem a implementação e passam 20/20 isolados no HEAD; nenhum teste passa por acaso.

**Tarefas concluídas**
- 8.1 ciclo de vida das ops vira máquina de estados pura (`opTransition`/`opDismissDelay` no `ui/pure.js`): todo estado terminal expira (done em 3s, error/cancelled em 6s), `showOp` remove a pill anterior do mesmo id e o timer velho não apaga op nova (guarda de identidade da D3), M22: commit `24bbc7d`
- 8.2 widget sys-polling ancorado na `.meta-line` real (fim do id fantasma que mandava a pill pro rodapé) e purga determinística de op terminal no começo do ciclo seguinte, B11: commit `8bf9f9b`
- 8.3 token de requisição `deliveriesReqSeq` no `loadDeliveries`: resposta de carga superada não pinta a tela nem encerra a op da carga nova, M19: commit `19c8131`
- 8.4 `stageLabel` pura + segundo loop no `tickElapsed`: o rótulo de estágio envelhece a cada segundo em vez de congelar no primeiro paint, B13: commit `644d7ca`
- 8.5 `validScope` pura + saneamento no fim do `rebuildAccounts`: escopo persistido órfão volta pra "todas" sem resetar escolha válida no boot (guarda `list.length`), B15: commit `437374b`
- 8.6 `accountBarVisible` pura (allowlist radar/destaques/time, 2+ contas): a barra de contas some da Entregas, que filtra por org própria e não respeita SCOPE, B14: commit `408573e`
- 8.7 chat-activity atualiza a pill via `updateOp` (cria a op se o evento chegar antes do snapshot), fase genérica só no primeiro paint e `closeChat` encerra a op antes de soltar a chave, B16: commit `3a86dcf`
- 8.8 `expiredSessionMarks` pura + marcadores de auto-merge/admin viram Map com a geração do refresh (`lastCheckAt`), podados no começo do `renderMyPRs`, B17: commit `9775da2`
- Fechamento da onda (esta seção + entrada "Não publicado" no CHANGELOG.md): commit próprio, `docs:`

**Pendências**
- Nenhum roteiro de verificação manual roteirizada da onda foi executado: a execução autônoma não roda o app real, não desliga a rede e não tem 2 contas configuradas. O da 8.2 é marcado como OBRIGATÓRIO no rascunho (o ciclo error do sys-polling não tem cobertura automatizada além da invariante estática). Executar com o usuário presente, antes do corte da v2.32.1, os roteiros das tarefas: 8.2 (pill de polling com rede desligada e religada, medidores `ACTIVE_OPS.size` e `.op-inline-pill` zerando entre ciclos), 8.3 (throttling Slow 3G trocando período e org na Entregas), 8.4 (stub lento de 20s observando iniciando/processando/sumiço sem snapshot novo), 8.5 (`farol-scope` fantasma no localStorage com 2 contas), 8.6 (passeio pelas 6 abas com 2 contas), 8.7 (chat com stub lento: pill viva, fechar no meio, reabrir antes dos 6s) e 8.8 (nível console com `autoUnavailableKeys` e, quando houver repo de teste, o ponta a ponta do Allow auto-merge). A preparação comum (instância isolada `FAROL_HOME` + stub lento) está na D5 do rascunho da onda.

**Desvios relevantes do plano**
- R10 do mestre na 8.7 (o mestre vence o rascunho): a regex do teste do `closeChat` usa a versão corrigida do red team (ancorada no início da função seguinte e exigindo o `closeOp` com o opId do chat), não a do rascunho, que casava errado e daria falso verde.
- R16 cumprida: os códigos dos achados foram reconciliados com `Documents/biud/analise-farol-gaps-logicos/relatorio.md` antes da execução (o rascunho foi escrito com o caminho do relatório indefinido); os 8 códigos batem.
- R6 respeitada: o `module.exports` do `ui/pure.js` foi edição aditiva, preservando os nomes das ondas anteriores. R9 respeitada nas 8.1, 8.2 e 8.7: `showOp`/`closeOp`/handler `.op-cancel` localizados pelos nomes e editados incrementalmente sobre o resultado da Onda 3, com o inventário de call sites considerando as ops que a 3 criou. R13 na 8.3: o teste automatizado do M19 é invariante estática (como proposto) e o cenário dinâmico (trocar filtro rápido e conferir que a resposta velha não vence) ficou registrado no roteiro manual, listado nas pendências acima.
- R15: contagem 506 pra 526 não é regressão, o gate é verde completo. Versão segue 2.30.1, sem tag, sem release e sem push (commits locais na `main`, conforme o mestre). Localização por âncora respeitada (regra transversal); travessão ausente do diff completo da onda e das mensagens de commit (varredura Unicode via Node, zero ocorrências).

**Dificuldades novas encontradas**
- Detalhe analisado pela verificação e descartado como não-defeito: no ramo error do `renderStatus`, o `closeOp('sys-polling', 'error', ...)` roda a cada snapshot e pode agendar mais de um timer de 6s pro mesmo op; a guarda de identidade `ACTIVE_OPS.get(opId) === op` (D3 da 8.1) e a purga determinística da 8.2 tornam isso inofensivo (o primeiro timer que dispara remove, os demais encontram a entrada trocada ou ausente e não fazem nada).
- O commit de fechamento não saiu junto da última tarefa, o mesmo escorregão das Ondas 7 e 3 (passos 3 e 4 do protocolo do mestre ficaram pra trás) e a verificação o apontou como pendência importante. Corrigido com este commit `docs`, sem mudança de código; a suite seguia verde o tempo todo. Pra Onda 9: o fechamento é parte da onda, não um extra.
- Fora isso, nenhuma além das antecipadas (D1 a D8 do rascunho, todas com a solução preparada funcionando; destaque pra D3, a guarda de identidade do timer, sem a qual o fix do M22 criaria um bug novo de corrida).

## Onda 9: persistência, consumo e cosméticos

Executada em 02/08/2026 (início da tarde, horário de Brasília; commits das 12:18 às 12:34). Achados cobertos: M23, M17, B12, B18, B19, B20 e B21. Suite na largada da onda: 526 testes; no fechamento: 538 (535 pass, 3 skipped de plataforma, esperados no Windows), com `npm run check` ok (64 arquivos). Verificação independente (adversarial) auditou os 8 commits diff a diff contra o rascunho e as regras do mestre, rodou os testes-alvo isolados (io-taxonomy, usage, ui-pure e pure: 109 pass, 0 fail; nenhum passa por acaso, com `TZ=America/Sao_Paulo` fixado antes dos requires nos dois arquivos de teste de dia local, conforme a R14) e conferiu as guardas por grep no fonte: `readJson` com logger nos 9 estados do construtor mais o `recoverInflight`, e SEM logger no artefato de workspace regenerável (de propósito, D7 do rascunho); `writeJsonAtomic` nas 10 escritas de estado, com as exclusões de propósito intactas (`SEEN_FILE`/`BASELINE_FILE` são texto linha a linha, `farol.log` é append, `~/.claude.json` tem o backup `.farol-bak` próprio e os artefatos de workspace do decision.js são regeneráveis); `DELIVERIES_LIMIT` importado no gh-queries.js; os `toISOString` restantes no fonte são só os fora do escopo do M17.

**Tarefas concluídas**
- 9.1 `readJson` distingue ENOENT de corrupção: primeiro boot segue silencioso, corrupção loga WARN e preserva a primeira evidência em `.bad` (COPYFILE_EXCL), com o logger passado pelos 9 chamadores de estado do construtor e pelo `recoverInflight`, M23 parte leitura: commit `2ecd796`
- 9.2 `writeJsonAtomic` (tmp + rename, fallback copy+unlink pro EPERM transitório do Windows) nas 10 escritas de JSON de estado, M23 parte escrita: commit `a073687`
- 9.3 bucket diário do consumo corta no dia LOCAL do processo, server (`localDay` no usage.js) e UI (`localDayKey` no pure.js) no mesmo commit, com os testes legados que afirmavam o corte UTC atualizados junto (D1), M17: commit `7d61fc6`
- 9.4 `aprovadosHoje` pura no pure.js: o contador do vazio-bom compara dia local com dia local (o ramo morto fatiava epoch como texto contra ISO), B12: commit `e9064e2`
- 9.5 `md()` protege o conteúdo de código antes dos inlines (sentinela em Private Use Area) e restaura no fim: bold/itálico/link não corrompem mais `f(*args, **kwargs)` nem link entre crases, B18: commit `d29516c`
- 9.6 `fmtCompact` promove pra M na fronteira do milhão (999.500 pra cima vira "1,0M", nunca "1000k"), B19: commit `33de184`
- 9.7 `modelLabel` ignora a data de snapshot em id de major único (regex com minor de até 2 dígitos e fronteira; "Sonnet 4", não "Sonnet 4.20250514"), B20: commit `55fdf0b`
- 9.8 aviso de teto das Entregas fala o limite real: `limit` no payload de `/api/deliveries` (fonte única: `DELIVERIES_LIMIT`) e `delivCappedMsg` no pure.js com fallback 1000 pra payload em cache antigo, B21: commit `0cf3da6`
- Fechamento da onda (esta seção + entrada "Não publicado" no CHANGELOG.md): commit próprio, `docs:`

**Pendências**
- O smoke manual do fechamento do rascunho (instância isolada com `FAROL_HOME` temporário: card Hoje e gráfico do Consumo alinhados, Radar vazio com um resolved de hoje mostrando o contador vivo, relatório de review com código e asterisco renderizado no chat) NÃO foi executado: a execução autônoma não roda o app real. A lógica de cada cenário tem cobertura automatizada; executar com o usuário presente antes do corte da v2.32.1.

**Desvios relevantes do plano**
- D10 do rascunho (bump PATCH pra v2.30.2 no fechamento) foi superada pela estratégia de release do mestre, que vence: as Ondas 8 e 9 consolidam na v2.32.1 e NADA é publicado autonomamente. Sem bump, sem tag, sem RELEASE_NOTES no ui/app.js; versão segue 2.30.1 e o corte é decisão do usuário.
- Números de linha do rascunho (base `4d39d8f`) já não batiam com o fonte após as 8 ondas anteriores; toda localização foi por âncora (regra transversal do mestre), incluindo o artefato de workspace do `readJson` sem logger (citado como linha 897, hoje em outra posição) e os pontos de uma linha do ui/app.js.
- R6 respeitada: os `module.exports` de `ui/pure.js`, `lib/io.js`, `lib/engine/usage.js` e as desestruturações de import (selfpr.js, decision.js, pushback.js) foram edição aditiva, preservando os nomes das ondas anteriores. R7 respeitada na 9.1 (o logger `warn` entrou por âncora na região compartilhada do construtor). R15: contagem 526 pra 538 não é regressão, o gate é verde completo.
- Desvio de TDD declarado na 9.1: o teste do ENOENT silencioso nasceu verde (o comportamento externo sem logger já era esse); é o pino da semântica que o invariante 3 do projeto exige (primeiro boot não é falha), e os outros dois testes da tarefa cumpriram o vermelho.
- Versão segue 2.30.1, sem tag, sem release e sem push (commits locais na `main`, conforme o mestre). Travessão ausente das linhas novas e das mensagens de commit (varredura da verificação, zero ocorrências).

**Dificuldades novas encontradas**
- O commit de fechamento não saiu junto da última tarefa, o mesmo escorregão das Ondas 7, 3 e 8 (passos 3 e 4 do protocolo do mestre ficaram pra trás), mesmo com o aviso explícito registrado no fechamento da Onda 8; a verificação o apontou como única pendência importante. Corrigido com este commit `docs`, sem mudança de código; a suite seguia verde o tempo todo. Fica a lição estrutural pra planos futuros: o fechamento precisa ser uma TAREFA numerada da onda, não um passo de protocolo, senão ele escorrega em toda onda.
- Fora isso, nenhuma além das antecipadas (D1 a D10 do rascunho, todas com a solução preparada funcionando; destaque pra D1, os testes legados que travavam o corte UTC foram atualizados no mesmo commit da 9.3, e pra D8, o rename por cima de destino existente validado no NTFS deste computador).
