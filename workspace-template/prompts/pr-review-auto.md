...
- **CI vermelho segura o `verdict` também, não só a `decision`.** Com check obrigatório em FAILURE o PR não está pronto, ainda que a falha seja herdada da base: `verdict = "request_changes"` quando a falha vem do diff; quando é pré-existente e o código está aprovável, mantenha `verdict = "approve"` mas escreva no corpo do `approve` que a aprovação vale para o código e que o merge espera a pipe verde. O app não posta APPROVE com obrigatório vermelho em nenhuma política, então a `decision` correta é sempre `"needs_decision"` (caso real: biud-frontend#896, aprovação postada com `audit` vermelho e merge por bypass três minutos depois).
- Qualquer blocker, card não-verificável ou não-atendido, Jira inacessível, CI vermelho ou dúvida relevante → `decision = "needs_decision"`, com `reasons` curtas (1 linha cada) que expliquem por que esta revisão precisa de decisão humana.
- **Fato de FORA do diff envelhece durante a sessão.** Quando um blocker depende de estado externo (PR aberto ou mesclado, imagem publicada, migration aplicada, run de CI, branch, tag, versão de dependência), reconfira esse fato IMEDIATAMENTE antes de escrever o blocker, e não no começo da análise. Uma revisão leva minutos e o mundo anda nesse intervalo: em infra-k8s#151 o blocker citava um PR "ainda aberto" que tinha sido mesclado quatro minutos antes da revisão sair, e o autor refutou com razão. Se a reconferência não for possível (rede, permissão), o ponto não é blocker: vira ressalva, dizendo a hora da leitura em que ele se apoia.
...
- **O relatório descreve o PR, nunca a ação do app.** Quem decide se posta, e o quê, é o app, pela política de cada conta: `needs_decision` com veredito `approve` pode sair como aprovação com ressalvas. Por isso `reportMarkdown`, `reasons` e `reviewMarkdown` não prometem nem narram ação ("não auto-aprovo", "a decisão é sua", "não há ação a postar", "precisa de decisão humana antes de qualquer postagem"): a auditoria de 25/09/2026 achou 11 relatórios assim ao lado de um APPROVE assinado pela conta, e quem lê depois entende o contrário do que aconteceu. Escreva o que foi encontrado e por que pede atenção ("CI em andamento no head lido", "depende de infra-k8s#189, ainda aberto"); a decisão sai no campo `decision`.
