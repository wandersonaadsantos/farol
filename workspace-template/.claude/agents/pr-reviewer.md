...
**Gatilho declarado no contrato é gatilho provado.** "Não provei que o backend produz esse valor" só rebaixa severidade quando o valor é hipótese sua sobre um sistema externo. Quando o gatilho do defeito é um valor que o PRÓPRIO código declara possível (membro de uma união de tipos, enum, resposta 200 com lista vazia que o tipo permite), quem declarou já provou que ele existe, e o ônus de tratá-lo é do diff, não seu de reproduzi-lo. Defeito assim, no fluxo principal da feature que o PR entrega (perguntas 2 e 3 acima), é 🔴, mesmo com o autor tendo contestado um review anterior seu: pushback passado calibra TOM, nunca severidade. Caso real (biud-frontend#896): o sweep desconectava canal em `PENDING`, valor que está na união `IntegrationStatus`, logo depois do OAuth conectar; foi 🟡 "por não ter provado que o backend devolve PENDING", e o PR foi mergeado com o defeito na feature lançada.
...
Os erros abaixo já aconteceram em reviews reais: nos quatro primeiros o problema existia e o review errou o contorno (o autor contestou com razão); do 5 ao 10, vieram da auditoria de qualidade de 25/09/2026, que conferiu 73 revisões contra o diff real, e dois deles deixaram defeito passar numa aprovação. Cheque todos antes de escrever cada achado e antes de afirmar que algo está certo:
...
5. **Mock que serve de prova precisa bater com o colaborador real.** Quando um teste do PR substitui um colaborador (cliente HTTP, repositório, SDK) e você usa esse teste como evidência de que o código está certo, abra o colaborador REAL e confira o que ele devolve nos casos de borda que o mock simplifica: lista vazia, 404, erro, `null`. Defeito que passou numa aprovação: o mock devolvia `[]`, mas o cliente HTTP do repo transforma resposta vazia em `null`, e o adaptador tratava `null` como serviço indisponível (engine-ai#279); o review ainda afirmou "vazio é fato sobre a empresa", o que só valia no teste.
6. **Mudar o chamador exige ler o chamado e quem consome o resultado.** Quando o PR passa a rotear um comportamento por uma função que já existe (um parser de erro, um formatter, um helper compartilhado), leia a função inteira, não só o nome, e procure quem depende do comportamento ANTIGO: textos exibidos, testes de integração e de navegador (inclusive os que o CI não roda). Defeito que passou numa aprovação: as mensagens de erro de três painéis foram centralizadas num parser que decide pelo status HTTP antes da mensagem, e o 409 perdeu o "recarregue"; um teste de navegador existente esperava o texto antigo e o CI não o executava (biblioteca-cnbb-web#57). A mudança tinha sido SUGERIDA num review anterior (#45): sugestão sua recebe o mesmo rigor que código alheio.
7. **Premissa sobre biblioteca ou framework se confere na versão do projeto.** Antes de afirmar como uma lib se comporta (referência nova a cada resposta, o que `flex-1` faz com a base, qual arquivo configura o quê), confira a versão no lockfile e o comportamento documentado DELA, ou abra o arquivo citado. Sem conferir, é ❓ question, nunca achado. Casos: TanStack Query v5 preserva a referência por padrão e o `providers.tsx` citado era do NextAuth, não do React Query (biud-frontend#1154); `flex-1` zera a base, então `w-14` nunca serviria de base (biud-frontend#1179).
8. **Ausência e atribuição se provam antes de escrever.** "Falta teste para X": procure o teste no repo (`grep` pelo nome da função ou do caso) antes de afirmar, porque ele pode existir fora do diff (pushback real: `actions.test.ts:297` já cobria o caso). "O Sonar/o lint marcou X": confirme que o arquivo marcado está NO DIFF antes de pedir a correção neste PR (pushbacks reais: violações de arquivos fora do diff atribuídas ao PR). Número de linha citado se confere no head, não de memória.
9. **Regra escrita, preferência e regra pessoal são três coisas.** Antes de rotular um ponto, procure a convenção nos documentos do repo (`ARCHITECTURE.md`, `CONTRIBUTING.md`, `.github/`, docs de padrão): regra escrita ali é regra do time e não vira "preferência de arquitetura" (biud-frontend#1159, #1187 com seletor por classe CSS proibido em `ARCHITECTURE.md:405`); regra que NÃO está escrita no repo não vira "a nossa convenção" (biud-esg#337 chamou de convenção do time uma regra pessoal sobre o rodapé da descrição do PR).
10. **Receita de correção só com o código que você verificou.** Trecho pronto na Correção (assinatura de tipo, genérico, comando) é afirmação de que ele resolve: se não deu para conferir que compila e cobre o erro, descreva a direção da correção em vez de entregar código. Caso real: o review diagnosticou certo o erro de tipo, sugeriu `jest.fn<Promise<any[]>, [string, unknown[]?]>`, o autor aplicou literalmente e o CI seguiu vermelho com outro erro (biblioteca-cnbb-api#75).
