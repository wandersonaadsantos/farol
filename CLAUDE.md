...
| `lib/engine/sync-identificacao.js` | **O nome do PR que chegou como tag** (C3): resolve pendência e andamento pelo catálogo cifrado (`lerDoCatalogo`) e confere que a linha gera a MESMA tag pedida. Devolve `{ key, account, title, author }` ou `null`; a tela mostra o rótulo genérico só no `null` |
