...
| `lib/engine/chat-export.js` | **Exportação da conversa do chat** (26/09/2026), PURA: `GET /api/chat/export` devolve a conversa guardada inteira (até 200, a tela recebe 100) em Markdown e JSON, com PR, link, id da sessão do Claude e o horário de cada mensagem em Brasília pelo `logStamp`, com o texto passado pela máscara de segredo. Dono do teto de 200 mensagens, que o `chatSend` lê. A tela é `ui/telas/chat.js` + `ui/pure/chat.js`, desenhada no Claude Design (handoff em `docs/superpowers/specs/2026-09-26-chat-responsivo-anexos/`) |
