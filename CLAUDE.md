...
| `lib/engine/skip-review.js` | Pular a revisão quando OUTRA PESSOA já está revisando o PR (v2.49.0). `revisandoPorOutros` é PURA (lê os nomes de label que a busca já trouxe), `outrosRevisando` é o gate SÍNCRONO e sem IO (contrato do reReviewTargets), e `comentarPulos` posta o aviso UMA vez por PR (âncora em `state/skip-comentado.json`, podada quando o PR sai do panorama). **`acrity` nunca conta como pessoa** (regra do Wanderson: review de ferramenta não dispensa olho humano). Vale só no caminho AUTOMÁTICO: clique manual sempre revisa |
