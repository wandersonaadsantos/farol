...
// Perder para uma TOMADA não é o mesmo que perder por vencimento: o aparelho antigo
// precisa saber que alguém assumiu deliberadamente, porque é isso que ele registra como
// risco de consumo duplicado ao voltar.
function motivoDaPerda(atual, leaseId) {
  if (ehObjeto(atual) && atual.leaseId !== leaseId && atual.tomadoDe) return 'tomado';
  return 'perdido';
}
...
async function renewLease(client, ids, { leaseId, nowMs }) {
  ...
  if (!vivoEMeu(lido.data, leaseId, nowMs)) return { ok: false, reason: motivoDaPerda(lido.data, leaseId), lease: lido.data };
...
// TOMADA FORÇADA (7.C8): o sucessor sobe por cima de um lease VIVO, com a geração
// anterior mais um e com o nome de quem foi tomado. É uma escrita diferente da disputa
// normal de propósito: a regra do banco distingue as duas, e um bug de escrita por cima
// de lease vivo continua sendo recusado pelo servidor.
async function takeoverLease(client, ids, dados) {
  const path = caminhoDe(ids);
  const lido = await client.get(path, COM_ETAG);
  if (!lido.ok) return indisponivel(lido);
  const pode = tomada.podeTomar(lido.data, { nowMs: dados.nowMs, deviceId: dados.deviceId });
  if (!pode.pode) return { ok: false, reason: pode.motivo, lease: lido.data };
  const sucessor = tomada.sucessorDe(lido.data, dados);
  const w = await client.put(path, sucessor, { ifMatch: lido.etag || 'null_etag', etag: true });
  if (w.ok) return { ok: true, lease: sucessor, etag: w.etag, anterior: lido.data };
  if (w.code === SYNC_CODES.CONFLITO) return { ok: false, reason: 'conflito', lease: w.data };
  return indisponivel(w);
}

export default { leasePath, leaseAcquirable, buildLease, acquireLease, renewLease, releaseLease, takeoverLease };
export { leasePath, leaseAcquirable, buildLease, acquireLease, renewLease, releaseLease, takeoverLease };
