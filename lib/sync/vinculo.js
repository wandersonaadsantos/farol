...
// Mapa SEM protótipo: o id do perfil chega pela rota local, e um id como `__proto__` num
// objeto comum devolveria Object.prototype no lugar da lista (e a rota caía com TypeError).
function mapaVazio() {
...
function lerTudo() {
  ...
  if (!objeto(d) || d.v !== VERSAO || !objeto(d.perfis)) return mapaVazio();
  const saida = mapaVazio();
...
// Para quem lê de fora, um objeto comum: `Object.fromEntries` cria propriedade própria até
// para `__proto__`, sem trocar protótipo.
function lerVinculos() {
  const pares = [];
  for (const [perfil, lista] of Object.entries(lerTudo())) {
    const aberto = abertoDe(lista);
    if (aberto) pares.push([perfil, aberto]);
  }
