...
async function atualizarPublicacaoDoRecibo(client, ids, { operationKind, materialVersion, publicationState, nowMs }) {
  ...
  // o nó que só tem o registro de postagem (a intenção nasce antes do recibo) não é recibo:
  // gravar publicationState nele seria recibo pela metade, e a regra do banco recusa
  if (!lido.receipt || !ehNumero(lido.receipt.completedAt)) return { ok: false, motivo: 'sem-recibo' };
