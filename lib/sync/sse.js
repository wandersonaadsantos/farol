...
function createSseParser(onEvent) {
  ...
  // Não existe end(): evento sem a linha em branco final é evento CORTADO, e a
  // especificação SSE (WHATWG) manda descartar. O único consumidor (lib/sync/rtdb.js)
  // já decidiu não despachar o resto do buffer quando a conexão cai; uma porta que
  // fizesse o contrário só serviria para alguém entregar um put truncado sem perceber.
  return { feed };
}
