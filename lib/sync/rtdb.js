...
// Consulta ordenada do RTDB: os valores vão em JSON na URL (`orderBy="t"`). A chave de
// ordenação é conferida contra a forma de nome de campo, porque ela entra na URL e um
// valor torto viraria parâmetro a mais. O resultado vem sem ordem: quem chama ordena.
const CHAVE_DE_ORDEM = /^[A-Za-z0-9_$]{1,40}$/;

function queryDaConsulta(consulta) {
  if (!consulta) return {};
  if (!CHAVE_DE_ORDEM.test(String(consulta.orderBy || ''))) return null;
  const q = { orderBy: io.safeStringify(String(consulta.orderBy), '') };
  for (const campo of ['startAt', 'endAt', 'equalTo']) {
    if (consulta[campo] !== undefined) q[campo] = io.safeStringify(consulta[campo], '');
  }
  for (const campo of ['limitToFirst', 'limitToLast']) {
    if (consulta[campo] !== undefined) q[campo] = String(Math.max(1, Math.floor(Number(consulta[campo]) || 1)));
  }
  return q;
}

async function get(ctx, path, { etag = false, shallow = false, consulta = null } = {}) {
  const daConsulta = queryDaConsulta(consulta);
  if (daConsulta === null) return falha(SYNC_CODES.FALHA_INTERNA, 0, 'chave de ordenação inválida');
  const query = shallow ? { shallow: 'true' } : daConsulta;
