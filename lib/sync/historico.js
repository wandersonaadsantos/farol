...
//
// O CORPO é a projeção que a tela já usa (decisionForUi), sem o login da conta e sem o
// estado de retry, que é deste aparelho. Cada mudança de status é uma VERSÃO nova, e a
...
const FORA_DO_CORPO = ['account', 'postRetry', 'reportMarkdown', 'payloads', 'memory'];
...
function indiceDe(d, { kId }) {
  ...
  return {
    ...
    prTag: x.key ? prTag(kId, String(x.key)) : '',
  };
}
