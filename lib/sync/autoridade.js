...
function sinalValido(sinal, ctx) {
  ...
  // o caminho entra na assinatura: o mesmo mecanismo serve ao batimento de autoridade e
  // à prontidão do distribuidor (CT-PRONT), e um sinal não vale no lugar do outro
  const caminho = ctx.caminho || CAMINHO_BEAT;
  return verificar(ctx.publicKey, sinal.sig, { uid: ctx.uid, caminho, generation: Number(sinal.generation), valor });
}
