...
function createTokenSource({ apiKey, refreshToken, fetchImpl, tokenUrl, agora = Date.now, onRefresh }) {
  ...
  let cache = null;
  let emVoo = null;
  // quando o token em uso foi obtido: quem toma 401 do banco precisa saber se o token era
  // velho (então o caminho é renovar) ou recém-obtido (então quem recusa é a regra)
  let obtidoEm = 0;
  ...
  // A senha acabou de ser conferida (admin, limpeza, revogação, chave nova): a entrada
  // nova passa a valer AQUI, no cliente vivo. Sem isto o token em cache continua sendo o
  // do login anterior, e as regras que exigem senha RECENTE (auth_time + 5 min no nó do
  // admin e na limpeza) recusam a escrita que a pessoa acabou de autorizar.
  function adotar(entrada) {
    if (!entrada || !naoVazio(entrada.idToken) || !naoVazio(entrada.refreshToken)) return false;
    atual = entrada.refreshToken;
    cache = { idToken: entrada.idToken, expiresAtMs: Number(entrada.expiresAtMs) || 0 };
    obtidoEm = agora();
    return true;
  }

  function obtidoHa() {
    return obtidoEm ? agora() - obtidoEm : Infinity;
  }

  return { getIdToken, invalidate, adotar, obtidoHa };
}
