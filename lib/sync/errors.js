...
const MOTIVOS = {
  ...
  segundo_fator: 'esta conta do Firebase pede confirmação num segundo aparelho, e o Farol não sabe fazer essa etapa; use um usuário sem segundo fator para a sincronização',
...
function codeFromIdentityMessage(msg) {
  ...
  // a etapa do segundo fator chega quase sempre como 200 sem idToken (ver
  // signInWithPassword), mas o Identity Platform também a anuncia por erro em algumas
  // configurações; nos dois caminhos o desfecho de quem usa é o mesmo
  if (prefixo === 'SECOND_FACTOR_REQUIRED' || prefixo === 'MFA_ENROLLMENT_NOT_FOUND') return SYNC_CODES.SEGUNDO_FATOR;
