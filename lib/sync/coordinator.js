...
// O nome NÃO basta: o aviso precisa saber se aquele aparelho é ESTE. Sem o `local`, o
// texto descrevia em terceira pessoa a máquina em que a tela está aberta ("este commit
// já foi analisado por o aparelho <nome desta máquina>"), e quem lia procurava um
// aparelho fantasma. É a mesma distinção que `alheioDeVerdade` faz para o lease, agora
// também para o recibo, que nunca teve guarda nenhuma.
//
// Compara por ID, nunca por nome: dois aparelhos podem se chamar igual.
function sujeitoDoDispositivo(rt, deviceId) {
  return { deviceName: nomeDoDispositivo(rt, deviceId), local: !!deviceId && deviceId === rt.deviceId };
}
...
async function consultarRecibo(engine, ctx, pre) {
  ...
  return recusa('recibo', { receipt: r.receipt, ...sujeitoDoDispositivo(rt, r.receipt.deviceId) });
}
...
function recusaDoLease(rt, a) {
  ...
  return recusa('alheio', { deviceId: l.deviceId || '', ...sujeitoDoDispositivo(rt, l.deviceId), since: Number(l.acquiredAt) || 0, operationKind: String(l.operationKind || '') });
}
...
// A TOMADA também é perda, e era a única que não entrava aqui: o aparelho tomado seguia a
// sessão inteira, sem registrar nada e gastando o provedor duas vezes (medido na bancada
// com engines reais, 17/09/2026).
function leaseVencido(r, estado, agora) {
  if (r.reason === 'perdido' || r.reason === 'tomado') return true;
...
async function verificarManual(engine, pr) {
  ...
  if (receiptBlocks(r.receipt, pre.nowMs)) return bloqueio(recusa('recibo', { receipt: r.receipt, ...sujeitoDoDispositivo(rt, r.receipt.deviceId) }));
