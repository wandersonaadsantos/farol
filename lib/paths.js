...
function sinaisDoModoCelular() {
  ...
  return {
    ...
    osrelease,
    // o /system do Android, que o proot-distro monta dentro do proot (a decisão é do modo.js)
    sistemaAndroid: fs.existsSync('/system/build.prop'),
  };
}
...
// Sinais de que o Farol roda dentro de um proot no Android (26/09/2026). A detecção do modo
// celular não basta: o proot-distro não repassa TERMUX_VERSION nem PREFIX e troca a versão do
// kernel por uma falsa ("...-PRoot-Distro"), sem "android", e no aparelho do dono ela
// respondeu "não". Aqui entram o kernel falso do proot e o /system do Android, que o
// proot-distro monta para os binários do sistema funcionarem. Quem decide é
// lib/engine/login-celular.js (pura), e só para o login: a A4 segue com a regra dela.
function sinaisDoProot() {
  let osrelease = '';
  try { osrelease = fs.readFileSync('/proc/sys/kernel/osrelease', 'utf8'); } catch { /* fora do Linux o arquivo não existe */ }
  return { osrelease, sistemaAndroid: fs.existsSync('/system/build.prop') };
}

export default {
  executadoDireto, rodandoComoRoot, sinaisDoModoCelular, distroDoLinux, sinaisDoProot,
