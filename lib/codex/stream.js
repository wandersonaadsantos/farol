...
function codexArgs(config = {}, opts = {}) {
  if (opts.stub) return [];
  const args = ['-a', 'never', 'exec', '--json', '--color', 'never', '--skip-git-repo-check'];
  const model = sanitizeCodexModel(config.reviewModel) || '';
  const effort = sanitizeCodexEffort(config.reviewEffort) || '';
...
function registrarUsoCodex(engine, opts, run, resultEvent) {
  ...
  const modelo = modelLabel(engine.config.reviewModel) || 'codex';
...
function runCodexStream(engine, prompt, opts = {}) {
  ...
  const args = codexArgs(engine.config, { stub: !!stub });
  const onEvent = opts.onEvent || (() => {});
  logSpawn('codex-session', stub ? [stub] : ['codex', ...args]);
  return new Promise((resolve, reject) => {
    const finish = encerrarUmaVez(resolve, reject);
    const run = { child: null, cancelled: false, stub: !!stub };
...
export default { codexEnv, codexArgs, cotaCodex, eventoCodex, verificarPlanoCodex, runCodexStream };
export { codexEnv, codexArgs, cotaCodex, eventoCodex, verificarPlanoCodex, runCodexStream };
