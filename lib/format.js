...
import { SELECOES_CLAUDE } from './modelos.js';
...
// tabela na ordem em que é consultada; a primeira que casa vence, como era na cadeia
// de ternários que isto substituiu. As famílias saem do catálogo (lib/modelos.js):
// família nova entra lá, e o rótulo passa a reconhecê-la sem uma lista paralela aqui.
const FAMILIAS = SELECOES_CLAUDE.filter((s) => s.familia).map((s) => [s.familia, s.nome]);
function familiaDoModelo(raw) {
  for (const [re, nome] of FAMILIAS) if (re.test(raw)) return nome;
  return '';
}
...
// QUEM segurou a análise, e de que lado da tela ele está. São TRÊS casos, e o do meio
// é o que faltava até 20/09/2026: o aparelho nomeado pode ser ESTE, aquele em que a
// mensagem está sendo lida, e aí descrevê-lo em terceira pessoa ("o aparelho X") faz
// quem lê procurar um aparelho fantasma que não existe. Quem informa a localidade é o
// coordinator, que sabe o deviceId; aqui só se escolhe a frase.
//
// `por` já sai com a preposição contraída: montar "por " + "o aparelho X" produzia
// "por o aparelho X", que estava na tela.
function sujeitoDoBloqueio(d) {
  if (d.local === true) return { local: true, por: 'neste aparelho', sujeito: 'este aparelho' };
  if (d.deviceName) return { local: false, por: `pelo aparelho ${d.deviceName}`, sujeito: `o aparelho ${d.deviceName}` };
  return { local: false, por: 'por outro aparelho', sujeito: 'outro aparelho' };
}
...
function textoBloqueio(key, admissao) {
  ...
  const s = sujeitoDoBloqueio(d);
  // `alheioDeVerdade` (lib/sync/coordinator.js) já impede que `alheio` fale do próprio
  // aparelho; o caso local fica aqui porque mandar esperar por si mesmo é exatamente o bug
  // que aquele comentário registra, e ele não pode voltar em silêncio por outro caminho
  if (a.reason === 'alheio' && s.local) return `${key}: este PR já está sendo analisado neste aparelho; nada foi iniciado de novo.`;
  if (a.reason === 'alheio') return `${key}: ${s.sujeito} está analisando este PR agora; este aparelho espera ele terminar.`;
  if (a.reason === 'recibo' && d.externo) return `${key}: você já se manifestou neste commit no GitHub; nada foi refeito.`;
  if (a.reason === 'recibo') return `${key}: este commit já foi analisado ${s.por}; nada foi refeito.`;
