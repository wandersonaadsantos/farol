  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      if (p.startsWith('/api/')) {
        ...
        if (p === '/api/sync/group') return send(200, syncResult(await engine.syncPublicarGrupo(syncGrupoDe(body))));
        ...
        if (p === '/api/sync/policy') return send(200, syncResult(await engine.syncPublicarPolitica(syncPoliticaDe(body))));
        ...
        if (p === '/api/sync/link') return send(200, syncResult(engine.syncVincularPerfil(body)));
        ...
        if (p === '/api/sync/cleanup') return send(200, syncResult(await engine.syncLimpar(syncLimpezaDe(body))));
