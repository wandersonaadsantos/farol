...
// A rota chama o colaborador direto, com o engine como contexto (o mesmo contrato das
// fachadas da Onda 2), porque o server.js esta no teto de linhas do ratchet e uma fachada
// de uma linha so repassaria a chamada.
import { chatExport } from './engine/chat.js';
...
// argumentos das rotas da C2b montados aqui fora: objeto literal dentro do roteador conta
// como nível de aninhamento no gate de qualidade
// a tela precisa do NÚMERO medido para mostrar antes de confirmar, e da impressão para
// confirmar exatamente aquele conteúdo; nada além disso sai daqui
function syncMedidaDe(r) {
  if (!r || !r.ok) return syncFalha(r);
  return { ok: true, categorias: r.categorias, pendentes: r.pendentes, bytes: r.bytes, impressao: r.impressao, lote: r.lote, lotes: r.lotes };
}
function syncEnvioDe(r) {
  if (!r || !r.ok) return syncFalha(r);
  return { ok: true, enviados: r.enviados, restantes: r.restantes, concluido: r.concluido, lote: r.lote, lotes: r.lotes };
}
// null é a leitura do banco que falhou, e ela não pode chegar à tela como lista vazia
function syncRevisoesDe(r) { return r && Array.isArray(r.revisoes) ? { ok: true, revisoes: r.revisoes, naoAbriram: Number(r.naoAbriram) || 0 } : syncFalha({ code: 'indisponivel', motivo: 'não deu para ler as revisões dos aparelhos agora' }); }
function syncRevisaoAberta(r) { return r ? { found: true, revisao: r } : { found: false }; }
function syncGrupoDe(body) { return { grupo: body.grupo }; }
function syncComandoDe(body) { return { alvo: String(body.alvo || ''), tipo: String(body.tipo || ''), args: body.args }; }
// plano e chaves (A2): a confirmação da adoção é literal, nunca um valor qualquer que seja
// verdadeiro, porque quem grava a configuração da assinatura é a pessoa, não a requisição
function perfilTesteDe(body) { return { profileId: String(body.profileId || '') }; }
function perfilAdocaoDe(body) { return { confirmar: body.confirmar === true, label: String(body.label || '') }; }
// diagnóstico unificado (A3): envelope, para a tela separar "veio vazio" de "a busca falhou"
function diagnosticoResult(engine) { return { ok: true, markdown: engine.diagnosticoMarkdown(), falhas: engine.diagnosticoFalhas() }; }
function syncPoliticaDe(body) { return { deviceId: String(body.deviceId || ''), politica: body.politica }; }
function syncCmdIdDe(body) { return { cmdId: String(body.cmdId || '') }; }
function syncAvisoDe(body) { return { prKey: String(body.prKey || ''), account: String(body.account || '') }; }
// a mesma lista responde a duas perguntas: para quem TRANSFERIR (`dono` roda a análise) e
// quem pode INICIAR um candidato que espera colocação (`itemId`)
function syncDestinosDe(body) { return { dono: String(body.dono || ''), acctTag: String(body.acctTag || ''), itemId: String(body.itemId || '') }; }
function syncLeituraResult(r) { return r && r.ok ? r : syncFalha(r); }
function syncComandoResult(r) { return r && r.ok ? { ok: true, cmdId: String(r.cmdId || ''), estado: String(r.estado || '') } : syncFalha(r); }
function syncChaveDeLimpezaDe(body) { return { ligada: body.ligada === true }; }
function syncLimpezaDe(body) { return { ...syncCredenciais(body), categorias: body.categorias }; }
...
// publicar política ou grupo: a versão que foi para o banco, para a tela dizer "publicada na
// versão N" sem prometer o aceite, que acontece no aparelho de destino
function syncPublicacaoResult(r) { return r && r.ok ? { ok: true, versao: Number(r.versao) || 0 } : syncFalha(r); }
// vínculo: o que ficou valendo, para a tela não esperar o próximo snapshot para saber
function syncVinculoResult(r) { return r && r.ok ? { ok: true, vinculo: r.vinculo || null } : syncFalha(r); }
// limpeza: o desfecho POR CATEGORIA (a remoção é uma por nó, e pode falhar pela metade)
function syncLimpezaResult(r) {
  if (!r || !r.ok) return syncFalha(r);
  return { ok: true, apagadas: r.apagadas || [], falharam: r.falharam || [], corteGravado: r.corteGravado === true };
}
function syncDeviceIdDe(body) { return { deviceId: String(body.deviceId || '') }; }
...
function startServer(engine, onReady) {
  // cada stream aberto guarda a própria vigília de credencial (A4): o que perdeu a sessão
  // é encerrado antes do próximo envio, e não recebe mais nada
  const sseClients = new Map();

  function encerrarStream(res) {
    sseClients.delete(res);
    try { res.end(); } catch { /* cliente já tinha caído */ }
  }

  function broadcast(event, data) {
    const payload = `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
    for (const [res, valida] of sseClients) {
      if (!valida()) { encerrarStream(res); continue; }
      try { res.write(payload); } catch { /* cliente caiu */ }
    }
  }
  ...
  // andamento de outros aparelhos: delta próprio, nunca o estado inteiro
  engine.on('sync-live', p => broadcast('sync-live', p));
  engine.on('sync-pending', p => broadcast('sync-pending', p));
  engine.on('sync-lists', p => broadcast('sync-lists', p));
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      if (p.startsWith('/api/')) {
        ...
        if (p === '/api/chat/export' && req.method === 'GET') return send(200, chatExport(engine, String(url.searchParams.get('key') || '')));
        ...
        if (p === '/api/diagnostics') return send(200, diagnosticoResult(engine));
        ...
        if (p === '/api/events') {
          ...
          const vigia = acesso.vigiaDoStream(req, engine.config);
          sseClients.set(res, vigia);
          // o batimento também confere: sem envio nenhum, a aba revogada não fica aberta
          const beat = setInterval(() => {
            if (!vigia()) { clearInterval(beat); encerrarStream(res); return; }
            try { res.write(': ping\n\n'); } catch { }
          }, TEMPOS.SSE_PING_MS);
        ...
        if (p === '/api/auth/sessions') return send(200, acesso.sessoes(req));
        if (p === '/api/auth/revoke') return send(200, acesso.revogar(req, body));
        ...
        // tornar ESTE aparelho admin: a senha real do Firebase, conferida antes de gravar
        if (p === '/api/sync/admin') return send(200, syncResult(await engine.syncTornarAdmin(syncCredenciais(body))));
        // C2b: configurar o grupo de consumo, vincular perfil, gerir aparelho, e a
        // limpeza e a revogação, que recebem a senha real e por isso são classificadas
        // como `recebe-segredo` no inventário da A4
        if (p === '/api/sync/group') return send(200, syncPublicacaoResult(await engine.syncPublicarGrupo(syncGrupoDe(body))));
        if (p === '/api/sync/command') return send(200, syncComandoResult(await engine.syncEmitirComando(syncComandoDe(body))));
        if (p === '/api/sync/command-status') return send(200, syncLeituraResult(await engine.syncDesfechoDoComando(syncCmdIdDe(body))));
        if (p === '/api/sync/policy') return send(200, syncPublicacaoResult(await engine.syncPublicarPolitica(syncPoliticaDe(body))));
        if (p === '/api/sync/policy-read') return send(200, syncLeituraResult(await engine.syncLerPolitica(syncDeviceIdDe(body))));
        if (p === '/api/sync/designation-decline') return send(200, syncResult(await engine.syncRecusarDesignacao()));
        if (p === '/api/sync/takeover-notice') return send(200, syncLeituraResult(await engine.syncAvisoDaTomada(syncAvisoDe(body))));
        // destinos de uma transferência (7.C7b): leitura avulsa, com o motivo de cada inapto
        if (p === '/api/sync/transfer-targets') return send(200, syncLeituraResult(await engine.syncDestinosDaTransferencia(syncDestinosDe(body))));
        if (p === '/api/sync/cleanup-state') return send(200, syncLeituraResult(await engine.syncEstadoDaLimpeza()));
        if (p === '/api/sync/link') return send(200, syncVinculoResult(engine.syncVincularPerfil(body)));
        // história de revisões de todos os aparelhos: lista e abertura sob demanda
        if (p === '/api/sync/reviews') return send(200, syncRevisoesDe(await engine.syncRecentes(body)));
        // Panorama e Meus PRs dos outros aparelhos, como o relógio leu por último
        if (p === '/api/sync/lists') return send(200, engine.syncListasRemotas());
        if (p === '/api/sync/review-body') return send(200, syncRevisaoAberta(await engine.syncAbrirRevisao(body)));
        // envio explícito do histórico local: medir primeiro, confirmar com a medição
        if (p === '/api/sync/history-measure') return send(200, syncMedidaDe(engine.syncMedirEnvio()));
        if (p === '/api/sync/history-send') return send(200, syncEnvioDe(await engine.syncEnviarHistorico(body)));
        if (p === '/api/sync/seen') return send(200, syncResult(await engine.syncMarcarVisto(body)));
        if (p === '/api/sync/device') return send(200, syncResult(await engine.syncAparelho(body)));
        if (p === '/api/sync/cleanup-key') return send(200, syncResult(await engine.syncChaveDeLimpeza(syncChaveDeLimpezaDe(body))));
        if (p === '/api/sync/cleanup') return send(200, syncLimpezaResult(await engine.syncLimpar(syncLimpezaDe(body))));
        if (p === '/api/sync/revoke') return send(200, syncResult(await engine.syncRevogar(syncCredenciais(body))));
        ...
        if (p === '/api/claude/profile-test') return send(200, await engine.claudeTestarPerfil(perfilTesteDe(body)));
        if (p === '/api/claude/profile-adopt') return send(200, engine.claudeAdotarLegado(perfilAdocaoDe(body)));
        if (p === '/api/claude-login') { const r = engine.openClaudeLoginSession(body.profileId || ''); return send(200, r && r.comando ? { ok: true, comando: r.comando, onde: r.onde || '' } : { ok: true }); }
        ...
        if (p === '/api/settings') { const r = engine.updateSettings(body || {}, engine.origemDe(req.headers['user-agent'])); return send(200, { ...r, config: engine.config }); }
        if (p === '/api/accounts/edit') return send(200, engine.editarConta(body, engine.origemDe(req.headers['user-agent'])));
