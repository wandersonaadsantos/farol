...
function syncMedidaDe(r) {
  ...
  return { ok: true, categorias: r.categorias, pendentes: r.pendentes, bytes: r.bytes, impressao: r.impressao };
}
function syncEnvioDe(r) {
  if (!r || !r.ok) return syncFalha(r);
  return { ok: true, enviados: r.enviados, restantes: r.restantes, concluido: r.concluido };
}
function syncRevisoesDe(lista) { return { ok: true, revisoes: lista }; }
