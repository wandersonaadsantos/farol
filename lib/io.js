...
// Leitura de JSON que OUTRO processo reescreve (o `~/.claude.json` do Claude Code, que ele
// regrava o tempo todo): a leitura pode cair no meio da escrita alheia e devolver JSON pela
// metade. Tenta de novo na hora, porque a janela da escrita é de milissegundos, e NUNCA cria
// `.bad` nem grava nada: o arquivo é de outro dono. Medido nos boots de 18 e 19/09/2026, em
// que o Farol desistiu de pré-confiar o workspace por causa dessa corrida.
function lerJsonTeimoso(file, { tentativas = 3, ler = (f) => fs.readFileSync(f, 'utf8') } = {}) {
  let ultimo = '';
  for (let i = 0; i < tentativas; i++) {
    let cru;
    try { cru = ler(file); } catch (err) { return { ok: false, motivo: err.code === 'ENOENT' ? 'ausente' : err.message, tentativas: i + 1 }; }
    try { return { ok: true, data: JSON.parse(cru), tentativas: i + 1 }; } catch (err) { ultimo = err.message; }
  }
  return { ok: false, motivo: ultimo, tentativas };
}
...
// JSON cru tem UM endereço (core.duplication.business-rule).
...
// Põe `dirs` na FRENTE do PATH, na ordem dada, tirando qualquer ocorrência anterior deles
// (26/09/2026). O prependPathDirs só acrescenta o que falta, então uma pasta que já estava
// no PATH, mas atrás de outra, seguia atrás: no celular o ~/.local/bin do instalador do
// Claude Code (2.1.283) ficava atrás de um `claude` antigo (2.1.246) e as revisões rodavam
// no antigo. PURA. Sem `dirs`, devolve o PATH como veio.
function priorizarNoPath(currentPath, dirs) {
  const atual = String(currentPath || '');
  const lista = (dirs || []).filter(Boolean);
  if (!lista.length) return atual;
  const sep = atual.includes(';') ? ';' : ':';
  const resto = atual.split(sep).filter((d) => d && !lista.includes(d));
  return lista.concat(resto).join(sep);
}

// PONTO CRÍTICO (regra 8 da receita ESM, brief da migração): os testes
...
const io = { ensureDir, readJson, lerJsonTeimoso, parseJson, safeStringify, writeJsonAtomic, writeTextAtomic, copyRecursive, detectGitBash, run, runShell, prependPathDirs, priorizarNoPath };
export default io;
export { ensureDir, readJson, lerJsonTeimoso, parseJson, safeStringify, writeJsonAtomic, writeTextAtomic, copyRecursive, detectGitBash, prependPathDirs, priorizarNoPath };
