...
function prependPathDirs(currentPath, candidates, exists) {
  const current = String(currentPath || '').split(':');
  const missing = (candidates || []).filter(d => d && !current.includes(d) && exists(d));
  if (!missing.length) return null;
  return missing.concat(currentPath || '').join(':');
}
...
const io = { ensureDir, readJson, parseJson, writeJsonAtomic, writeTextAtomic, copyRecursive, detectGitBash, run, runShell, prependPathDirs };
export default io;
export { ensureDir, readJson, parseJson, writeJsonAtomic, writeTextAtomic, copyRecursive, detectGitBash, prependPathDirs };
