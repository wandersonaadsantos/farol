...
function normalizeClaudeProfiles(val) {
  ...
  return val.map(p => {
    ...
    if (p && p.kind === 'apikey') {
      ...
      return aplicaOrcamento({ id, label, kind: 'apikey', apiKey, baseUrl }, p);
    }
    if (p && p.kind === 'codex') {
  ...
  }).filter(p => p && p.id && (p.kind === 'codex' || (p.kind === 'apikey' ? p.apiKey : p.dir)));
}
...
const parse = {
  ...
  sanitizeParallelReviews, extractCardKeys,
};
