...
// As duas vars de auth que a MÁQUINA pode ter no ambiente e que o claude CLI honra ACIMA
// do login OAuth. applyClaudeAuthEnv já as apaga do env do filho, mas no posix isso não
// basta: o profile do usuário é sourceado DEPOIS do env montado (o -l do `/bin/sh -lc` no
// headless, o login shell do Terminal.app antes de executar o .command), então um `export
// ANTHROPIC_API_KEY` perdido no ~/.profile re-injeta a chave POR CIMA do perfil resolvido e
// anula em silêncio o perfil de assinatura do Farol. Por isso o unset é emitido DENTRO do
// shell, depois de qualquer sourcing e antes do exec do claude (G21). No Windows não existe
// esse sourcing (cmd.exe não lê profile nenhum), então lá o env limpo é suficiente.
const POSIX_AUTH_UNSET = 'unset ANTHROPIC_API_KEY ANTHROPIC_AUTH_TOKEN';

// Prefixo da linha headless posix (`/bin/sh -lc "<isto><comando>"`). Perfil de chave de API
// fica de FORA de propósito: a chave dele viaja pelo env, e re-setá-la aqui a colocaria na
// linha de comando, visível no `ps` de qualquer processo da máquina. Nos scripts (.command)
// isso não se aplica, porque lá a chave já está no arquivo, então claudeAuthShellLines
// emite o unset pros dois kinds e o perfil de chave re-seta a própria chave logo depois.
function claudeAuthPosixPrefix(auth) {
  if (auth && auth.kind === 'apikey' && auth.apiKey) return '';
  return `${POSIX_AUTH_UNSET}; `;
}
...
module.exports = {
  ...
  applyClaudeAuthEnv, claudeAuthShellLines, claudeAuthPosixPrefix,
