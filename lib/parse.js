...
import { APELIDOS_CLAUDE, aceitaEsforco } from './modelos.js';
...
function parseAccounts(val) {
  const norm = (user, owners, meta) => {
    ...
    if (meta) {
      ...
      /* Peso desta conta no rateio da cota do perfil (Politica 2 da spec
         2026-09-10-justica-de-fila-entre-orgs). Ausente = 1, e com todo mundo em 1 a
         divisao e igual, que e o default desejado. So guarda numero POSITIVO: 0 e
         negativo zerariam a cota da conta e a barrariam pra sempre num rodizio que
         ninguem pediu, entao eles nao chegam nem a ser persistidos. */
      const peso = Number(meta.budgetWeight);
      if (Number.isFinite(peso) && peso > 0) o.budgetWeight = peso;
    }
    return o;
  };
...
const OPENROUTER_SECRET_ENV = 'FAROL_OPENROUTER_AUTH_TOKEN';
...
//
// Limpa a autenticação herdada ANTES de aplicar só a do kind resolvido.
...
// CLAUDE_CODE_OAUTH_TOKEN também vence o login salvo e precisa sumir: um token expirado
// herdado continuaria sendo usado mesmo depois de renovar o /login do perfil.
function applyClaudeAuthEnv(env, auth) {
  ...
  delete env[OPENROUTER_SECRET_ENV];
  ...
  if (auth.kind === 'openrouter' && auth.apiKey) {
    ...
    // /bin/sh -lc pode sobrescrever ANTHROPIC_* ao carregar o profile. A cópia
    // reservada permite restaurar a credencial depois do sourcing sem colocar o
    // segredo na linha de comando; claudeAuthPosixPrefix a apaga em seguida.
    env[OPENROUTER_SECRET_ENV] = auth.apiKey;
...
// As mesmas cinco vars de auth Claude que applyClaudeAuthEnv apaga do env: as três da API
// (`ANTHROPIC_*`), o CLAUDE_CODE_OAUTH_TOKEN e o CLAUDE_CONFIG_DIR. applyClaudeAuthEnv já
...
//
// A lista cobre as cinco, e não só as três de credencial, porque as outras duas são a mesma
...
const POSIX_AUTH_UNSET = 'unset ANTHROPIC_API_KEY ANTHROPIC_AUTH_TOKEN ANTHROPIC_BASE_URL CLAUDE_CONFIG_DIR CLAUDE_CODE_OAUTH_TOKEN';
...
//
// Perfil de chave de API limpa só o token OAuth: a chave dele viaja pelo env, e re-setar
// seu valor aqui a colocaria na linha de comando, visível no `ps` de qualquer processo.
// Nos scripts isso não se aplica, porque lá a chave já está no arquivo, então
// claudeAuthShellLines emite o unset completo e re-seta a chave do perfil logo depois.
function claudeAuthPosixPrefix(auth) {
  if (auth && auth.kind === 'openrouter' && auth.apiKey) {
    const base = auth.baseUrl || OPENROUTER_DEFAULT_BASE;
    // O valor vem do env do filho. A linha contém só o NOME da variável, então
    // nem `ps` nem spawns.log recebem a chave. Depois da cópia, a variável
    // temporária some antes do exec do Claude.
    return `${POSIX_AUTH_UNSET}; export ANTHROPIC_AUTH_TOKEN="$${OPENROUTER_SECRET_ENV}"; unset ${OPENROUTER_SECRET_ENV}; export ANTHROPIC_API_KEY=; export ANTHROPIC_BASE_URL='${shEsc(base)}'; `;
  }
  // Chave Anthropic viaja só pelo env: re-exportar o valor na cmdline colocaria
  // o segredo no `ps` e no spawns.log.
  if (auth && auth.kind === 'apikey' && auth.apiKey) return 'unset CLAUDE_CODE_OAUTH_TOKEN; ';
...
function claudeAuthShellLines(auth, isWin) {
  ...
  if (auth.kind === 'apikey' && auth.apiKey) {
    ...
    lines.push(auth.baseUrl
      ? linhaDoShell(isWin, `set "ANTHROPIC_BASE_URL=${auth.baseUrl}"`, `export ANTHROPIC_BASE_URL='${esc(auth.baseUrl)}'`)
      : linhaDoShell(isWin, 'rem sem base url propria', '# sem base url propria'));
    return lines;
  }
  const dir = auth.dir || '';
  return [...head, dir
    ? linhaDoShell(isWin, `set "CLAUDE_CONFIG_DIR=${dir}"`, `export CLAUDE_CONFIG_DIR='${esc(dir)}'`)
    : linhaDoShell(isWin, 'rem sem config dir proprio', '# sem config dir proprio')];
}

// A mesma linha nos dois shells: o `set`/`rem` do cmd.exe ou o `export`/`#` do sh. Nomeada
// para tirar o ternario de dentro do ternario das duas escolhas acima (base url e config
// dir), que a constituicao reprova como ternario aninhado.
function linhaDoShell(isWin, win, posix) { return isWin ? win : posix; }
...
// A lista sai do catalogo (lib/modelos.js): selecao nova entra la, e a config passa a
// aceita-la sem uma segunda lista aqui que alguem precisaria lembrar de atualizar.
const MODEL_ALIASES = APELIDOS_CLAUDE;
...
// O caminho Claude filtra esta lista depois. O Codex CLI usa `minimal` como o
// menor nivel; a allowlist acompanha o config-reference oficial do CLI.
...
function sanitizeClaudeModel(v) {
  ...
  if (s === '') return '';
  if (CODEX_MODEL_RE.test(s)) return null;
  // 'auto' permanece válido na config (roteador do Farol); buildModelFlags
  // é quem impede --model auto na cmdline.
  return s;
}
...
function sanitizeCodexModel(v) {
  ...
  if (s === null) return null;
  if (s === '') return '';
  if (MODEL_ALIASES.includes(s) || MODEL_FULL_RE.test(s)) return null;
  return s;
}
...
/* Teto GLOBAL de revisoes simultaneas somando todas as contas (Politica 3 da spec
   2026-09-10-justica-de-fila-entre-orgs). Difere do sanitizeParallelReviews em duas
   coisas, e as duas sao de proposito:

   - 0 e um valor VALIDO e significa DESLIGADO (sem teto global), que e o default e o
     comportamento de sempre. O teto por conta nao tem esse estado: ele sempre vale.
   - o clamp e 1..8 em vez de 1..4, porque este teto soma contas e nao PRs da mesma.

   Lixo (NaN, string, null) devolve null, e quem chama resolve pro default. Negativo
   vira 0 (desligado) em vez de 1: config torta nao pode ligar uma trava que ninguem
   pediu. */
function sanitizeGlobalParallelReviews(v) {
  const n = parseInt(v, 10);
  if (!Number.isFinite(n)) return null;
  if (n <= 0) return 0;
  return Math.min(8, n);
...
// nao adivinha: filtra so o que o catalogo afirma por familia (hoje, o Haiku). Quem decide
// e o catalogo, e por familia: o nome completo fixado a mao (`claude-haiku-4-5`) segue a
// mesma regra do apelido, o que antes nao acontecia. Nos demais, deixa o CLI decidir.
function effortForModel(model, effort) {
  if (!effort) return '';
  return aceitaEsforco(model) ? effort : '';
}
...
// Pre-confianca do workspace no ~/.claude.json: decide O QUE muda, sem tocar em disco.
// O chamador (ensureWorkspaceTrusted, server.js) le, chama isto e so grava se changed.
//
// A condicao de reescrita olha SO hasTrustDialogAccepted de proposito. O onboarding e
// cortesia (pula a tela de boas-vindas); a confianca e o que evita o dialogo bloqueante
// que mata a autonomia. O Claude Code regrava o arquivo inteiro a partir da copia que
// tem em memoria e, nisso, come o campo de onboarding que o Farol acabou de escrever:
// exigir os dois true fazia o Farol reescrever o arquivo a cada boot pra sempre, e cada
// reescrita e uma chance de o outro lado ler o arquivo pela metade (farol.log de 10 e
// 11/09/2026, "~/.claude.json ilegivel" em todo boot).
//
// O claude ora registra a chave com \ ora com /: semeia as duas variantes.
function workspaceTrust(data, workspace) {
  const saida = data && typeof data === 'object' ? data : {};
  saida.projects = saida.projects && typeof saida.projects === 'object' ? saida.projects : {};
  let changed = false;
  for (const key of [workspace, String(workspace).replace(/\\/g, '/')]) {
    const entry = saida.projects[key] || {};
    if (entry.hasTrustDialogAccepted === true) continue;
    entry.hasTrustDialogAccepted = true;
    entry.hasCompletedProjectOnboarding = true;
    saida.projects[key] = entry;
    changed = true;
  }
  return { data: saida, changed };
}
...
const parse = {
  ...
  MODEL_ALIASES, EFFORT_LEVELS, CODEX_EFFORT_LEVELS, sanitizeModel, sanitizeEffort, effortForModel,
  sanitizeClaudeModel, sanitizeClaudeEffort, sanitizeCodexModel, sanitizeCodexEffort,
  sanitizeParallelReviews, sanitizeGlobalParallelReviews, extractCardKeys, OPENROUTER_DEFAULT_BASE, OPENROUTER_SECRET_ENV,
};
