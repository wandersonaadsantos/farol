...
function aplicaOrcamento(out, p) {
  ...
  const daily = isNumericInput(p && p.budgetDaily) ? Number(p.budgetDaily) : NaN;
  if (Number.isFinite(daily) && daily >= 0) out.budgetDaily = daily;
  const total = isNumericInput(p && p.budgetTotal) ? Number(p.budgetTotal) : NaN;
  if (Number.isFinite(total) && total >= 0) out.budgetTotal = total;
  if (typeof (p && p.budgetSince) === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(p.budgetSince)) {
