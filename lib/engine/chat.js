...
import { CHATS_FILE } from '../paths.js';
import { writeJsonAtomic } from '../io.js';
import { novoIdDeSessao } from './sessao-id.js';
import { exportarConversa, TETO_DE_MENSAGENS } from './chat-export.js';
...
// Quantas mensagens a tela recebe: a conversa vai pelo SSE a cada bloco da resposta. A tela
// lê este valor e o teto do que vem na resposta, nunca de número próprio.
const JANELA_DA_TELA = 100;

// A tela recebe as últimas 100 (a conversa vai pelo SSE a cada bloco da resposta); a
// conversa inteira guardada sai pela exportação. O id da sessão do Claude e o início da
// conversa vão junto para a tela poder copiar o id, e o total para ela dizer "últimas 100
// de T" (26/09/2026).
function chatPublic(engine, key) {
  const c = engine.chats[key];
  const limites = { janela: JANELA_DA_TELA, teto: TETO_DE_MENSAGENS };
  if (!c) return { key, url: null, status: 'idle', sessionId: null, createdAt: null, total: 0, ...limites, messages: [] };
  return { key, url: c.url, status: c.status, sessionId: c.sessionId || null, createdAt: c.createdAt || null, total: c.messages.length, ...limites, messages: c.messages.slice(-JANELA_DA_TELA) };
}

function chatExport(engine, key) {
  return exportarConversa(engine.chats[String(key || '').trim()], { agora: Date.now(), versao: APP_VERSION });
}
...
function chatPreamble(engine, key, url, inherited) {
  const intro = inherited
    ? 'MUDANÇA DE MODO: a revisão headless terminou; a partir de agora você está em CONVERSA ao vivo com quem opera este Farol, pela interface dele. As regras "sem interlocutor" e "responda só JSON" não valem mais.'
    : `Você é o Claude dentro do app Farol, em conversa ao vivo com quem opera este Farol sobre o PR ${key}${url ? ` (${url})` : ''}. Use gh e as ferramentas do workspace pra examinar o que precisar.`;
  return intro + '\n' +
    ...
    // QUEM a sessao e. O comentario de biud-frontend#1150 ("@Gabrielk5 ja deixou isso
    // registrado") saiu de uma conversa como esta: a sessao nao sabia que a conta citada
    // era a dela propria. Mesmo bloco da revisao, uma fonte so.
    + promptMod.identidadeBlock(engine.accountForPr(prConhecido(engine, key) || { key })) + '\n';
}

// O PR como o app já o conhece (com a `account` que a busca resolveu), procurado onde
// ele fica gravado. O chat só tem `key` e `url` na mão, e derivar a conta pela ORG é o
// último recurso: org fora do `owners` do config cai no fallback da primária, ou seja,
// agir no GitHub com identidade que não é a dona do PR (raiz A1). Ordem por frescor da
// informação; nenhum lugar tendo o PR, quem chama segue com o objeto mínimo de sempre.
function prConhecido(engine, key) {
  const listas = [engine.decisions.pending, engine.decisions.resolved, engine.queue, engine.panorama];
  for (const lista of listas) {
    if (!Array.isArray(lista)) continue;
    const achado = lista.find(x => x && x.key === key);
    const pr = (achado && achado.pr) || achado;
    if (pr && pr.account) return pr;
  }
  return null;
}
...
async function chatSend(engine, key, url, text) {
  ...
  const acc = engine.accountForPr(prConhecido(engine, key) || { key, url: url || chat.url || null });
  ...
  chat.status = 'running';
  const { id } = novoIdDeSessao(engine, 'c');
  ...
  const runOnce = (sid, prompt) => engine.runClaudeStream(prompt, {
    ...
    // nunca pra string solta no sh/cmd
    extraArgs: sid && /^[0-9a-zA-Z-]{8,64}$/.test(sid) ? ['--resume', sid] : [],
    onEvent: (e) => {
  ...
  (async () => {
    try {
      ...
      chat.status = 'idle';
      chat.runId = null;
      if (chat.messages.length > TETO_DE_MENSAGENS) chat.messages = chat.messages.slice(-TETO_DE_MENSAGENS);
      engine.saveChats();
      engine.emit('chat', engine.chatPublic(key));
      engine.pushState();
...
const chatMod = { saveChats, chatPublic, chatExport, chatSummaries, chatPreamble, chatSend, chatStop };
export default chatMod;
export { saveChats, chatPublic, chatExport, chatSummaries, chatPreamble, chatSend, chatStop };
