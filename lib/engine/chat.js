...
// conversa vão junto para a tela poder copiar o id (26/09/2026).
function chatPublic(engine, key) {
  const c = engine.chats[key];
  if (!c) return { key, url: null, status: 'idle', sessionId: null, createdAt: null, messages: [] };
  return { key, url: c.url, status: c.status, sessionId: c.sessionId || null, createdAt: c.createdAt || null, messages: c.messages.slice(-100) };
}
