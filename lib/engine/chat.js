...
function chatPublic(engine, key) {
  ...
  if (!c) return { key, url: null, status: 'idle', sessionId: null, createdAt: null, total: 0, messages: [] };
  return { key, url: c.url, status: c.status, sessionId: c.sessionId || null, createdAt: c.createdAt || null, total: c.messages.length, messages: c.messages.slice(-100) };
}
