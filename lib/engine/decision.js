...
async function reconcilePending(engine, keys) {
  ...
  for (const item of candidatas) {
    ...
    if (estado === 'MERGED') {
      ...
      engine.resolveIntoHistory({ ...item, status: 'already_merged', action: 'skip' });
      resolvidas++;
      engine.emit('toast', { kind: 'info', text: `${item.key}: já foi mergeado; cancelei a revisão pendente.` });
      continue;
    }
    let reviews;
  ...
  try {
    ...
    if (!r.ok && /line could not be resolved|422/i.test(r.stderr) && (payload.comments || []).length) {
      ...
      const fallback = {
        ...
        comments: [],
        // G1: o fallback é montado do zero, então a âncora do head precisa ser
        // recopiada aqui, senão o recuo pro corpo perderia o commit_id
...
module.exports = {
  ...
  postReview, postReviewFromSession, decisionForUi,
