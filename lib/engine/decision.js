...
// Teto ABSOLUTO da isenção de dona viva: 12h cobre com folga o uso humano da sessão de
// terminal (almoço, reunião, voltar de manhã cedo no dia seguinte). Acima disso, quem
// segura a cap quase sempre é um terminal FANTASMA (janela morta cujo aviso de saída
// nunca chegou, pendência conhecida do macOS), e sessão fantasma não pode segurar
// credencial de escrita pra sempre, enquanto o engine viver.
const REVIEW_CAP_OWNER_MAX_MS = 12 * 60 * 60 * 1000;
...
function terminalOwnerAlive(engine, cap, now) {
  ...
  if (!Number.isFinite(cap.createdAt) || now - cap.createdAt > REVIEW_CAP_OWNER_MAX_MS) return false;
...
function reviewCaps(engine) {
  ...
  for (const [token, cap] of engine.reviewPostCaps) {
    ...
    // E a isenção tem teto absoluto (REVIEW_CAP_OWNER_MAX_MS): terminal fantasma não vira
    // credencial eterna.
