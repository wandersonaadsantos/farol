...
// A sessão dona da cap ainda está aberta E é uma sessão de terminal? Só esse par
// isenta do TTL (ver o comentário do laço de poda em reviewCaps).
function terminalOwnerAlive(engine, cap) {
...
function reviewCaps(engine) {
  ...
  for (const [token, cap] of engine.reviewPostCaps) {
    ...
    if (!cap || (cap.expiresAt <= now && !terminalOwnerAlive(engine, cap))) engine.reviewPostCaps.delete(token);
  }
