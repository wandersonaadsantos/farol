...
function recordDecision(engine, pr, result, extra) {
  ...
  if (item.status === 'pending') {
    // Round novo do MESMO PR SUBSTITUI a pendência que travou em head velho. Aquele
    // card fala de código que não existe mais e nunca vai poder ser postado (o payload
    // está ancorado num sha que o GitHub recusa), então empilhar dois cards do mesmo PR
    // só confunde. É a mesma razão pela qual reReviewTargets se recusa a relançar com
    // pendência na mesa: aqui o round já veio, e quem sai é o card morto, não o novo.
    // Só o bloqueio por head velho é substituído: pendência viva (gate barrou por
    // regra ou conteúdo) continua sendo sua pra decidir.
