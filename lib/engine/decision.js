...
async function retryFailedPosts(engine) {
  ...
  for (const item of candidatas) {
    ...
    const jaPostado = states && states.includes(event === 'approve' ? 'APPROVED' : 'CHANGES_REQUESTED');
    if (!jaPostado) {
      ...
      if (!post.ok) {
        ...
        const attempts = (item.postRetry.attempts || 0) + 1;
        engine.decisions.pending[idx].postRetry = {
          ...item.postRetry, attempts, lastError: post.error,
          exhausted: attempts >= MAX_POST_RETRY_ATTEMPTS,
        };
        engine.saveDecisions();
        continue;
      }
