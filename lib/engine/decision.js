...
async function decide(engine, id, action) {
  ...
  if (head && item.headSha && head !== item.headSha) {
    item.blockedReason = `${staleHeadText(item.headSha, head)}; este texto fala do código anterior. Peça uma revisão nova do head atual antes de postar.`;
    ...
    engine.saveDecisions();
    engine.pushState();
    engine.emit('toast', { kind: 'error', text: `Não postei em ${item.key}: ${staleHeadText(item.headSha, head)}. Revise de novo antes de postar.` });
