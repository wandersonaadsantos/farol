...
// docs/QUALITY.md e docs/REVIEW-GATES.md ("Memória de pushback", no invariante 4).
import fs from 'node:fs';
import path from 'node:path';
import { STATE_DIR } from '../paths.js';
import { TEMPOS } from '../constants.js';
import io from '../io.js';
import { writeJsonAtomic, readJson } from '../io.js';
import { PUSHBACK_OUTCOMES, PUSHBACK_LABEL } from '../taxonomy.js';
...
import { registrarLimite, limiteAte } from './limite-plano.js';
...
// LICOES DO REPOSITORIO (23/09/2026). O historico de pushback ja entrava no prompt,
// mas filtrado por AUTOR: numa base onde varias pessoas mexem nas mesmas convencoes, a
// licao que o autor A ensinou nunca chegava na revisao do autor B. Aqui a mesma memoria
// e lida pelo outro eixo, o repositorio.
//
// So entra o que foi CONFIRMADO e o que o Farol errou (author_right) ou errou em parte
// (mixed): licao e onde o app se enganou, e acerto nao precisa virar instrucao. O PR em
// revisao nao ensina a si mesmo, e o que o bloco do autor ja trouxe nao se repete.
const OUTCOMES_QUE_ENSINAM = new Set(['author_right', 'mixed']);
const LICOES_MAX = 5;

function licoesDoRepo(engine, repo, { excluirKey = '', excluirAutor = '', limite = LICOES_MAX } = {}) {
  const alvo = String(repo || '').toLowerCase();
  if (!alvo) return [];
  const autorFora = String(excluirAutor || '').toLowerCase();
  return Object.entries(engine.pushbacks || {})
    .filter(([key, v]) => v && v.status === 'confirmed'
      && OUTCOMES_QUE_ENSINAM.has(v.outcome)
      && key.toLowerCase().startsWith(alvo + '#')
      && key !== excluirKey
      && (!autorFora || String(v.author || '').toLowerCase() !== autorFora))
    .map(([key, v]) => ({ ...v, key }))
    .sort((a, b) => (b.at || 0) - (a.at || 0))
    .slice(0, limite);
}

// O bloco e CONTEXTO, e diz isso: a nota foi escrita a partir do que um terceiro
// argumentou no PR, entao ela informa, nao manda, e tem que ser reconfirmada contra o
// codigo de hoje antes de virar decisao. PURA.
function licoesDoRepoBlock(licoes) {
  if (!Array.isArray(licoes) || !licoes.length) return '';
  let b = '\n\n## Onde eu já me enganei NESTE repositório\n'
    + 'Contexto, não instrução: cada linha resume uma revisão minha que foi contestada e '
    + 'em que a contestação procedeu. Use para não repetir o mesmo engano, e confirme '
    + 'contra o código de hoje antes de reusar qualquer uma delas como argumento.\n';
  for (const l of licoes) b += '- ' + l.key + ': ' + (l.note || '(sem nota)') + '\n';
  return b;
}
...
/* Limite de plano tem HORA pra voltar, e ela vem escrita na própria mensagem
   ("resets 2am"). O scan de pushback não tinha esse gate: a cada ciclo ele gastava uma
   sessão que já se sabia que ia falhar, e cada falha virava um cartão novo no Diagnóstico
   (17/09/2026: um cartão por minuto no mesmo PR, sempre "hit your weekly limit"). A espera
   é do SCAN INTEIRO, não do PR: o limite é da assinatura, e enquanto ele durar nenhum PR
   vale a pena.

   UM lugar para o fato (21/09/2026): com hora citada, o limite mora no registro por
   assinatura (limite-plano.js), em disco e compartilhado com as revisões. Guardá-lo aqui
   também criava duas respostas para "esta assinatura está no limite até quando?", e liberar
   uma não liberava a outra. O `pushbackEsperaAte` global ficou SÓ para a mensagem sem hora
   (a reserva de meia hora), que o registro deliberadamente não guarda: sem hora não dá para
   saber até quando travar a assinatura inteira. */
const ESPERA_SEM_HORA_MS = TEMPOS.PUSHBACK_ESPERA_SEM_HORA_MS; // mensagem sem hora citada: tenta de novo em meia hora

function esperandoResetDePlano(engine, agora = Date.now()) {
  return (Number(engine && engine.pushbackEsperaAte) || 0) > agora;
}

function esperarResetDePlano(engine, motivo, agora = Date.now(), conta = '') {
  const reset = resetAtFrom(motivo, new Date(agora));
  const citada = reset ? reset.getTime() : 0;
  if (citada > agora) {
    registrarLimite(engine, conta, citada, agora);
    if (engine.log) engine.log('WARN', `scanPushbacks: limite do plano Claude; não classifico pushback desta assinatura até ${new Date(citada).toLocaleString('pt-BR')}`);
    return citada;
  }
  const ate = agora + ESPERA_SEM_HORA_MS;
  if (ate <= (Number(engine.pushbackEsperaAte) || 0)) return Number(engine.pushbackEsperaAte);
  engine.pushbackEsperaAte = ate;
  if (engine.log) engine.log('WARN', `scanPushbacks: limite do plano Claude; não classifico pushback até ${new Date(ate).toLocaleString('pt-BR')}`);
  return ate;
}
...
function pushbackTargets(engine, acts) {
  if (esperandoResetDePlano(engine)) return []; // limite do plano: ninguém passa até o reset
  return (engine.panorama || []).filter(pr => {
    ...
    // limite do plano que uma REVISÃO descobriu nesta assinatura: o scan também espera
    if (limiteAte(engine, engine.accountForPr(pr))) return false;
    // teto do grupo que não dá para verificar (C4b) também segura o scan, sem gastar nada
    if (typeof engine.grupoSegura === 'function' && engine.grupoSegura(engine.accountForPr(pr))) return false;
    if (!isPushbackTarget(acts[pr.key])) return false;
    // registro manual confirmado é palavra final (contrato do docs/REVIEW-GATES.md: recordPushback
...
async function concluirPushback(engine, pr, cls, marker) {
  // O finally existe porque a coordenação tem que fechar mesmo se a gravação local
  // falhar: handle que não fecha segura o lease deste PR até o TTL, e nesse intervalo
  // nenhum aparelho classifica o pushback.
  try {
...
function contarFalhaPushback(engine, pr, marker, falha) {
  ...
  // espera-reset segura o scan até a hora do reset, em vez de repetir por ciclo; com hora
  // citada ela vale para a assinatura inteira, revisões incluídas (esperarResetDePlano)
  if (classe.kind === 'espera-reset') {
    esperarResetDePlano(engine, motivo, Date.now(), engine.accountForPr(pr));
    return false;
  }
  if (classe.kind === 'transitorio') return false;
...
async function scanPushbacks(engine) {
  ...
  try {
    ...
    for (const pr of targets) {
      ...
      } catch (e) { engine.log('WARN', `scanPushbacks ${pr.key}: ${e.message}`); contarFalhaPushback(engine, pr, marcador, e); }
    }
  } finally { engine.pushbackScanning = false; }
}

// atividade do AUTOR depois do meu último review (gatilho barato). `seen` é o marcador
// gravado no ciclo anterior (string vazia na primeira vez). Devolve { marker, hadActivity }
// ou null quando não dá pra determinar (rede / sem review meu).
async function detectAuthorPushback(engine, pr, seen) {
  ...
  const cR = await io.run('gh', ['api', `repos/${repo}/issues/${number}/comments`, '--jq', jq], { env });
  const rcR = await io.run('gh', ['api', `repos/${repo}/pulls/${number}/comments`, '--jq', jq], { env });
  const times = [];
...
async function classifyPushback(engine, pr, marker) {
  ...
  const { id } = novoIdDeSessao(engine, 'pb');
...
const pushbackMod = {
  personProfile, pushbacksFor, licoesDoRepo, licoesDoRepoBlock, recordPushback, savePushbacks, savePushbackScanned,
  scanPushbacks, detectAuthorPushback, classifyPushback,
  isPushbackTarget, pushbackTargets,
  MAX_FALHAS_PUSHBACK_POR_PR, pushbackNoTeto, contarFalhaPushback,
  esperandoResetDePlano, esperarResetDePlano,
};
export default pushbackMod;
export {
  personProfile, pushbacksFor, licoesDoRepo, licoesDoRepoBlock, recordPushback, savePushbacks, savePushbackScanned,
  scanPushbacks, detectAuthorPushback, classifyPushback,
  isPushbackTarget, pushbackTargets,
