...
/* Limite de plano tem HORA pra voltar, e ela vem escrita na própria mensagem
   ...
   vale a pena. Mora na memória do processo de propósito; perder a espera num reinício custa
   no máximo uma sessão, e gravar mais um arquivo de estado para isso não se paga. */
...
function esperarResetDePlano(engine, motivo, agora = Date.now()) {
  const reset = resetAtFrom(motivo, new Date(agora));
  const citada = reset ? reset.getTime() : 0;
  const ate = citada > agora ? citada : agora + ESPERA_SEM_HORA_MS;
...
function contarFalhaPushback(engine, pr, marker, falha) {
  ...
  // espera-reset segura o scan inteiro até a hora do reset, em vez de repetir por ciclo
  if (classe.kind === 'espera-reset') { esperarResetDePlano(engine, motivo); return false; }
