...
import { STATUS, permanente, falhaSem, avisarTela, registrarFalha, superado } from './sync-falha.js';
...
import syncTelas from './sync-telas.js';
import syncListas from './sync-listas.js';
...
async function iniciar(engine) {
  ...
  // o relógio de 10 s (sinais do admin, andamento) começa AQUI, ao conectar: até a v2.62.1 ele
  // só era ligado no tique de polling seguinte, e a tela passava minutos sem dizer quem é o
  // admin. `engine.timer` é a prova de que o app está monitorando: sem polling não há Farol
  // rodando, e criar intervalo em processo de teste ou de ferramenta vazaria trabalho de fundo.
  if (sharedActive(cfg) && engine.timer) syncAndamento.ligarRelogio(engine, cfgDe);
  // a chave reabre pelo cache junto da leitura dos aparelhos (lib/engine/sync-chave.js)
  const [ap] = await Promise.all([lerAparelhos(engine), syncChave.abrirPeloCache(engine, cfg)]);
  if (geracao !== rt.geracao) return superado();
...
// O anúncio fora do carimbo periódico (abrir a chave): escreve e relê a frota, como o
// ciclo faz, para a lista local (tela, destinos da transferência) não ficar para trás.
async function anunciarPresenca(engine) {
  const r = await touchPresence(engine);
  if (!r.ok) return r;
...
function deveReconectar(rt) {
  ...
  return rt.status === STATUS.ERRO && !(rt.lastError && permanente(rt, rt.lastError.code));
}
...
function recentes(engine, dados) { return syncHistorico.recentesParaTela(engine, (engine.config && engine.config.sync) || {}, { dev: String((dados && dados.dev) || '') }); }
// Panorama e Meus PRs de outros aparelhos como a última leitura deixou: sem rede, a tela
// abre com o que o relógio já leu e depois acompanha pelo evento sync-lists.
function listasRemotas(engine) { return { ok: true, ...syncListas.projecaoDasListas(engine) }; }
...
function statusForUi(engine) {
  ...
  return {
    ...
    ...syncTelas.projecaoDasTelas(engine),
...
export default {
  ...
  touchPresence, anunciarPresenca, syncLogin, syncLogout, syncTest, syncUnlock, syncGerarChaveNova, syncTornarAdmin, syncPublicarPolitica, syncPublicarGrupo, vincularPerfil, marcarVisto, recentes, listasRemotas, abrirRevisao, medirEnvio, enviarHistorico, aparelho, chaveDeLimpeza, limpar, revogar, retirarConsentimento, lerAparelhos, statusForUi, seguraAutomacao, registrarEspera,
