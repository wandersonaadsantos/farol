...
async function ciclo(engine, cfg, opcoes = {}) {
  ...
  if (!pub.ok) {
    ...
    // a autoridade não depende de haver leitor: sozinho no conjunto, este aparelho vira
    // admin no banco e, sem o giro dos sinais, a tela seguia em "sem admin" para sempre
    if (pub.code === 'sem-frota') await sinais.cicloDosSinais(engine, cfg, { agora });
    return pub;
  }
