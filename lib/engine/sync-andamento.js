...
async function ciclo(engine, cfg, opcoes = {}) {
  ...
  if (!pub.ok) {
    ...
    // a autoridade não depende de haver leitor nem da chave do conjunto (os sinais são
    // assinados pela chave do admin, não cifrados): sozinho no conjunto ou com a chave
    // trancada, este aparelho vira admin no banco e, sem o giro, a tela seguia "sem admin"
    if (pub.code !== 'compartilhamento-desligado') await sinais.cicloDosSinais(engine, cfg, { agora });
    return pub;
  }
  ...
  // o que este aparelho deixou no banco e já não está aberto aqui (resolvido antes de um
  // reinício), senão os outros mostram "precisa de você" de PR já decidido para sempre
  if (abertas !== null) await pendencias.apagarOrfas(engine, cfg, abertas);
...
function prsDoCatalogo(engine) {
  ...
  // as revisões recentes, até onde a leitura dos outros alcança: sem elas, a lista de
  // revisões de todos os aparelhos mostraria quase só PRs sem nome
  const resolvidas = engine.decisions && Array.isArray(engine.decisions.resolved) ? engine.decisions.resolved : [];
  const recentes = resolvidas.slice(0, historico.LIMITE_LEITURA).map(prDaPendencia);
  ...
  return [...fila, ...distribuindo, ...pendentes, ...vivas, ...recentes].filter((pr) => {
