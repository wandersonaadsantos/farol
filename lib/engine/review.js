...
// get + delete: o sid guardado na recuperação do boot só serve pra ESTA
// primeira vez que o PR reaparece na fila, nunca reutilizável depois. Devolve
// junto o head que a sessão morta tinha lido, pra guarda de head do
// sidDeRetomada valer também no caminho do boot (o PR pode ter ganhado commit
// novo enquanto o app estava fora do ar).
function consumirRetomadaPendente(engine, key) {
  const vazio = { sid: '', head: '' };
  if (!engine.retomadaPendente || !engine.retomadaPendente.has(key)) return vazio;
  const pend = engine.retomadaPendente.get(key);
  engine.retomadaPendente.delete(key);
  if (!pend) return vazio;
  return { sid: String(pend.sid || ''), head: String(pend.head || '') };
}

// PR que vai pra entrada do retryAfterNet, carregando o sid da sessão que acabou de
// cair (Task 1 estampa err.sessionId). Sem sid novo (a queda foi antes de a sessão
...
// estampado pelo runHeadlessReview), pra guarda de head do sidDeRetomada poder
// descartar a retomada se o PR ganhar commit novo durante a espera, que pode durar
// horas. Sem headLido (queda antes de o head ser resolvido), fica o que já havia.
function prComRetomada(pr, err, guardado) {
...
/* Escolhe o PRÓXIMO PR a disparar entre os ELEGÍVEIS (conta abaixo do teto dela).
   ...
   números. */
function proximoHeadless(engine, limite) {
  let escolhido = -1;
...
function processHeadless(engine) {
  ...
  for (; ;) {
    ...
    const idx = proximoHeadless(engine, limite);
...
async function runOneHeadless(engine, pr, acct) {
  ...
  if (perfilEstourado) {
    ...
    estacionar(engine, pr.key, `perfil "${perfilEstourado.label}"`, 'orcamento');
  ...
  try {
    ...
    if (err.cancelled) {
      ...
      estacionar(engine, pr.key, 'cancelada por você', 'cancelado');
      ...
      if (tries < cap) {
        ...
        estacionar(engine, pr.key, msg, 'esgotado');
      ...
      estacionar(engine, pr.key, msg, 'falha');
      engine.log('ERROR', `revisao autonoma ${pr.key}: ${msg}`);
...
async function rodarSessao(engine, promptFinal, streamOpts, sid, jaAnunciada = false, blocoRetomada = '') {
  ...
  try {
    ...
    return await engine.runClaudeStream(promptFinal + blocoRetomada, { ...streamOpts, extraArgs: argsRetomada });
  } catch (err) {
...
function lerResultadoDaRevisao(engine, res) {
  try {
...
// `retomarSid` (queda transitória ou recuperação do boot) NÃO é opt-in, é a mesma
// revisão retomando; `resumeSid` (round incremental da re-revisão) segue atrás de
// config.reReviewResume. Formato sempre pela allowlist: sid fora dela degrada pra
// sessão nova em silêncio, nunca entra numa linha de shell.
// Guarda de head: a espera do retry pode ser LONGA (limite de plano tem cap 12 e
// hora de reset), então o PR pode ter ganhado commit novo entre a queda e a
// retomada. Retomar aí seria pedir pra não reler justamente o que mudou. Head
// desconhecido de qualquer um dos dois lados mantém a retomada (falta de dado nunca
// vira decisão nova), que é o comportamento seguro de sempre.
function sidDeRetomada(engine, pr, headShaAtual) {
  const valido = (s) => !!s && RESUME_SID_RE.test(String(s));
  const headMudou = !!(pr.knownHead && headShaAtual && pr.knownHead !== headShaAtual);
  if (valido(pr.retomarSid) && headMudou) return { sid: '', aposFalha: false, headMudou: true };
  if (valido(pr.retomarSid)) return { sid: String(pr.retomarSid), aposFalha: true, headMudou: false };
  const optIn = (engine.config || {}).reReviewResume && valido(pr.resumeSid);
  return { sid: optIn ? String(pr.resumeSid) : '', aposFalha: false, headMudou: false };
}

// bloco de prompt + linha de atividade da retomada por queda, num par só: quem
// retoma sem dizer por que retomou deixa a esteira parecendo revisão pela metade.
// Devolve '' quando a retomada não é por falha (round incremental ou sessão nova).
function anunciaRetomada(engine, id, retomada) {
  if (retomada.headMudou) {
    engine.pushActivity(id, 'info', 'O PR recebeu commit novo depois da queda: a sessão interrompida não é retomada, esta revisão lê o head atual do zero.');
    return '';
  }
...
// uma vez, no primeiro evento que traz session_id, e daí o inflight.json já tem
// o sid gravado, pra sobreviver a queda do app no meio.
...
// (antes o próprio bloqueio impedia o mecanismo que o resolveria). Duas
// proteções de orçamento, nunca de autonomia: debounce de head quieto
// (TEMPOS.HEAD_QUIETO_MS, contra rajada de pushes) e teto de rodadas
// automáticas por dia (MAX_RODADAS_AUTO_DIA; estourou, o aviso diz isso e o
// botão Re-revisar segue valendo). Conta com autoReview desligado: nada muda.
...
function classificaReRound(engine, pr, inflightKeys, agora = Date.now()) {
  ...
  if (!viaStale && !viaPendencia) return null;
  const headRound = viaStale ? info.head : pend.blockedHead;
  // debounce: só gasta sessão com o head QUIETO. No gatilho A o relógio é o
  // carimbo do refreshStaleStates; no B é a idade da pendência (a sessão acabou
  // de ver o head andar). Carimbo ausente = espera um ciclo (falta de dado
  // nunca dispara).
  const quieto = viaStale
    ? (() => { const q = (engine.headQuietoDesde || {})[pr.key]; return !!(q && q.head === headRound && agora - q.at >= TEMPOS.HEAD_QUIETO_MS); })()
    : (agora - Number(pend.createdAt || agora) >= TEMPOS.HEAD_QUIETO_MS);
  if (!quieto) return null;
  // G10: draft é trabalho sabidamente em andamento
  if (pr.isDraft) return null;
  const ancora = normalizeAncora((engine.reReviewLaunched || {})[pr.key]);
  // âncora por head: cada estado do PR relança NO MÁXIMO uma vez
  if (ancora.head === headRound) return null;
  ...
  if ((((engine.decisions || {}).pending) || []).some(d => d.key === pr.key && d.blockedKind !== 'stale_head')) return null;
  // as MESMAS travas do toReview do check(): quem mexer lá, mexe aqui
  const acct = engine.accountForPr(pr);
  if (engine.isMuted(acct)) return null;
  if (!engine.autoReviewFor(acct)) return null;
  if (!engine.tokenFor(acct)) return null;
  if (inflightKeys.has(pr.key)) return null;
  if (engine.autoReviewParked.has(pr.key)) return null;
  if (engine.retryAfterNet.has(pr.key)) return null;
...
function reReviewEsgotados(engine, inflightKeys, agora = Date.now()) {
  return candidatosReRound(engine).filter(pr => classificaReRound(engine, pr, inflightKeys, agora) === 'esgotado');
}
...
function estacionar(engine, key, motivo, tipo) {
  ...
  engine.parkedMotivos[key] = { at: new Date().toISOString(), motivo: String(motivo || ''), tipo: String(tipo || 'falha') };
...
function parkedParaUi(engine) {
  ...
  for (const k of engine.autoReviewParked || []) {
    ...
    out[k] = { at: String(m.at || ''), motivo: String(m.motivo || '').slice(0, PARKED_MOTIVO_UI_MAX), tipo };
  }
  return out;
}
...
async function launchReReviews(engine) {
  ...
  // teto do dia: avisa UMA vez por PR por dia (Set em memória; reinício repete o
  // aviso no máximo uma vez, aceitável) e nunca enfileira. O botão Re-revisar
  // continua valendo, e o texto diz exatamente isso.
  engine.avisoRodadasDia = engine.avisoRodadasDia || new Set();
  for (const pr of reReviewEsgotados(engine, inflight, agora)) {
    const chave = `${pr.key}:${diaLocal(agora)}`;
    if (engine.avisoRodadasDia.has(chave)) continue;
    engine.avisoRodadasDia.add(chave);
    engine.emit('toast', { kind: 'info', text: `${pr.key} atingiu o teto de ${MAX_RODADAS_AUTO_DIA} rodadas automáticas hoje; se quiser mais uma, use o botão Re-revisar.` });
  }
  ...
  // poda segura de headQuietoDesde e avisoRodadasDia: são só memória em
  // processo, e re-carimbar custa apenas um ciclo de espera do debounce (nunca
  // muda decisão). Mesmo critério de "aberto" do reReviewLaunched acima.
  for (const k of Object.keys(engine.headQuietoDesde || {})) {
    if (!abertos.has(k)) delete engine.headQuietoDesde[k];
  }
  for (const chave of [...engine.avisoRodadasDia]) {
    if (!chave.endsWith(`:${diaLocal(agora)}`)) engine.avisoRodadasDia.delete(chave);
  }
...
const reviewMod = {
  ...
  parallelLimit, globalParallelLimit, freeHeadlessSlot, reReviewTargets, reReviewEsgotados, classificaReRound,
  ...
  lastReviewSessionId, stageSummaryFrom, stageOfLine, fastModeBlock, rodarSessao, retomadaAposFalhaBlock,
