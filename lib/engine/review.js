...
function enqueueHeadless(engine, pr) {
  ...
  if (!pr.manual && !guardada && distribuicao.ativa(engine, cfgDaSync(engine))) {
    engine.headlessDistribuindo = engine.headlessDistribuindo || new Map();
    engine.headlessDistribuindo.set(pr.key, { pr, desde: Date.now() });
    engine.pushState();
    distribuicao.publicarCandidato(engine, cfgDaSync(engine), pr).then((r) => {
...
function processHeadless(engine) {
  ...
  for (; ;) {
    ...
    const vaga = comAdmissao ? admissao.reservar(engine, { tipo: 'review', ref: engine.headlessQueue[idx].key }) : null;
