...
async function runHeadlessReview(engine, pr) {
  ...
  try {
    ...
    // envelope na mesma conversa (lib/engine/reparo-envelope.js). Reparo que nao acontece
    // ou nao resolve cai no estacionamento de sempre, com a falha de contrato original.
    let result;
    try {
      result = lerResultadoDaRevisao(engine, res, id);
    } catch (err) {
      const novo = await reparoMod.pedirEnvelope(engine, err, res, id, streamOpts);
      if (!novo) throw err;
      result = lerResultadoDaRevisao(engine, novo, id);
      // o reparo pode nascer com id proprio: o resultado tem que apontar pra sessao que
      // de fato o entregou, senao o card e o Consumo mandam pra conversa errada
