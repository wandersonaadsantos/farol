  ...
  try {
    ...
    if (canAuto) {
      ...
      if (post.ok) {
        ...
        engine.emit('auto-approved', { pr, result });
        engine.emit('toast', { kind: 'ok', text: `✅ ${pr.key} aprovado automaticamente${points.length ? ` (${points.length} ponto(s) de atenção)` : ''}.` });
        return;
      }
    ...
    if (canReject) {
      ...
      if (post.ok) {
        ...
        engine.emit('toast', { kind: 'ok', text: `🔴 ${pr.key}: pedido de mudanças postado automaticamente (${(result.reasons || []).length || 'ver'} motivo(s)).` });
        return;
      }
    ...
    engine.emit('toast', { kind: 'info', text: `🟡 ${pr.key} precisa de você: ${(result.reasons || []).length || 'ver'} motivo(s).` });
  } finally {
