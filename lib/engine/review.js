...
async function rodarSessao(engine, promptFinal, streamOpts, sid) {
  ...
  try {
    return await engine.runClaudeStream(promptFinal, { ...streamOpts, extraArgs: ['--resume', sid] });
  } catch (err) {
...
/* ---------- label de revisão em andamento (pedido do Thiago, 18/08/2026) ----------
   Quando a revisão headless começa, o PR ganha a label "<conta>:in-progress" (ex.:
   thiagocarvalho-dev:in-progress) pro time ver no GitHub que o review está acontecendo,
   e a perde quando a sessão termina (sucesso, falha ou cancelamento, via finally).
   Condicionada à EXISTÊNCIA da label no repo: o `gh pr edit --add-label` falha quando a
   label não existe, e o Farol nunca cria label em repo alheio, então a falha da adição
   é o próprio "se existir" (silenciosa de propósito, não é erro). Best-effort dos dois
   lados: nada aqui toca a revisão em si. Só a remoção que falha DEPOIS de uma adição
   bem-sucedida vira WARN, porque label presa no PR é visível pro time. */
function inProgressLabelFor(user) {
  const u = String(user || '').trim();
  return u ? `${u}:in-progress` : '';
}
...
async function addInProgressLabel(engine, pr) {
  const acc = engine.accountForPr(pr);
  const label = inProgressLabelFor(acc);
  if (!label || !pr.url || !engine.tokenFor(acc)) return '';
  try {
    const r = await io.run('gh', ['pr', 'edit', pr.url, '--add-label', label], { env: engine.ghEnv(acc) });
    return r.ok ? label : '';
  } catch { return ''; }
}
...
const STAGE_ORDER = ['preparo', 'leitura', 'card', 'verificacao', 'raciocinio', 'redacao'];
const STAGE_LABEL = {
...
function stageOfLine(it) {
  ...
  if (/atlassian|jira/i.test(t)) return 'card';
...
function stageSummaryFrom(items, startedAt, endedAt) {
  ...
  ms.redacao = (ms.redacao || 0) + Math.max(0, endedAt - prev);
  return {
...
async function runHeadlessReview(engine, pr) {
  ...
  engine.writeInflight();
  engine.pushState();
  // sinaliza no PR que o review começou (label "<conta>:in-progress", só se ela
  ...
  try {
      ...
      if (post.ok) {
        ...
        engine.emit('toast', {
          kind: 'ok', text: publicPoints.length
            ? `⚠️ ${pr.key} aprovado com ${publicPoints.length} ressalva(s): ${publicPoints[0]}`
    ...
    if (canReject) {
      ...
      if (post.ok) {
        ...
        engine.emit('toast', { kind: 'ok', text: `🔴 ${pr.key} reprovado (mudanças pedidas): ${(publicItem.reasons || [])[0] || 'ver relatório'}` });
        return;
      }
    ...
    engine.emit('toast', { kind: 'info', text: `🟡 ${pr.key} precisa da sua atenção: ${(publicItem.reasons || [])[0] || 'ver relatório'}${extra}` });
  } finally {
...
const reviewMod = {
  ...
  lastReviewSessionId, stageSummaryFrom, stageOfLine, fastModeBlock,
