...
const { WORKSPACE, TEMPLATE_DIR, STATE_DIR } = require('../paths');
const { run, writeJsonAtomic } = require('../io');
const fanoutMod = require('./fanout');
const { classify, resetAtFrom } = require('../log-taxonomy');
...
async function runHeadlessReview(engine, pr) {
  ...
  // Falha aqui degrada pra headSha vazio (nunca filtra, nunca bloqueia por causa de uma
  // falha de rede), mesmo padrão do fan-out.
  try {
  ...
  try {
    ...
    // puro). Ver docs/superpowers/specs/2026-08-05-checkpoint-verificacao-design.md.
...
function launchReReviews(engine) {
  ...
  for (const pr of alvos) engine.enqueueHeadless({ ...pr, account: engine.accountForPr(pr), requested: true });
}

module.exports = {
  prFromUrl, launchReview, enqueueHeadless, headlessAcct, processHeadless, runOneHeadless, retryTargets,
  parallelLimit, freeHeadlessSlot, reReviewTargets, launchReReviews, saveReReviewLaunched,
