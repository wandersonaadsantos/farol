...
import { addReviewSignal, removeReviewSignal } from './review-signal.js';
import {
...
/* ---------- sinal de revisão em andamento (mudou de cara em 28/08/2026) ----------
   Até então o sinal era a label pública "<conta>:revisando" (pedido do Thiago,
   18/08/2026, com criação automática desde 19/08). Em 28/08/2026 o Wanderson viu
   a label no biud-frontend#845 e decidiu que NENHUM rastro de automação pode
   aparecer publicamente no GitHub: além de visível enquanto a sessão roda, os
   eventos de add/remove da label ficam PARA SEMPRE na timeline do PR. O sinal
   virou uma ref git customizada (refs/farol/revisando/..., invisível em toda a
   interface do GitHub), criada no início da revisão headless e removida no
   finally, com TTL pros casos em que a sessão morre sem remover. A mecânica
   inteira mora em lib/engine/review-signal.js (a história completa está no
   cabeçalho de lá); aqui só ficam os dois pontos de uso. As labels legadas de
   cópias antigas continuam sendo LIDAS no gate de pulo (transição), mas nunca
   mais escritas. */
...
async function runHeadlessReview(engine, pr) {
  ...
  engine.writeInflight();
  engine.pushState();
  // sinaliza que o review começou (ref git invisível na interface do GitHub,
  // ver review-signal.js; era a label pública até 28/08/2026). Declarada FORA
  // do try pra remoção no finally; atribuída DENTRO pra falha inesperada nunca
  // vazar a sessão registrada acima.
  let sinalEmAndamento = '';
  try {
    sinalEmAndamento = await addReviewSignal(engine, pr, headShaAtual);
    ...
    await removeReviewSignal(engine, pr, sinalEmAndamento);
