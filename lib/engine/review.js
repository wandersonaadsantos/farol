...
function enqueueHeadless(engine, pr) {
  ...
  if (!pr.manual && !pr.viaDistribuicao && !guardada && distribuicao.ativa(engine, cfgDaSync(engine))) {
...
function enfileirarDaDistribuicao(engine, pr, admissaoId) {
  ...
  return enqueueHeadless(engine, item);
}
...
function processHeadless(engine) {
  ...
  for (; ;) {
    ...
    const vaga = comAdmissao && !jaReservado ? admissao.reservar(engine, { tipo: 'review', ref: engine.headlessQueue[idx].key }) : null;
