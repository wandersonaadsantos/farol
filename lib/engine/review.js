...
function parkedParaUi(engine) {
  ...
  for (const k of engine.autoReviewParked || []) {
    const m = motivos[k];
    if (!m) continue;
    out[k] = { at: String(m.at || ''), motivo: String(m.motivo || '').slice(0, PARKED_MOTIVO_UI_MAX), tipo: String(m.tipo || 'falha') };
  }
  return out;
}
...
const reviewMod = {
  ...
  parallelLimit, freeHeadlessSlot, reReviewTargets, reReviewEsgotados, classificaReRound,
