...
// primeira vez que o PR reaparece na fila, nunca reutilizável depois.
function consumirRetomadaPendente(engine, key) {
  if (!engine.retomadaPendente || !engine.retomadaPendente.has(key)) return '';
  const sidPendente = engine.retomadaPendente.get(key);
...
function prComRetomada(pr, err, guardado) {
  ...
  return { ...pr, retomarSid: err.sessionId || anterior };
}
...
function enqueueHeadless(engine, pr) {
  ...
  const sidRecuperado = consumirRetomadaPendente(engine, pr.key);
  if (sidRecuperado) pr = { ...pr, retomarSid: sidRecuperado };
...
async function rodarSessao(engine, promptFinal, streamOpts, sid, jaAnunciada = false) {
...
// sid persiste assim que nasce (não só no fim da revisão): server.js grava
// no inflight.json a cada evento, pra sobreviver a queda do app no meio.
...
async function runHeadlessReview(engine, pr) {
  ...
  try {
    ...
    promptFinal += anunciaRetomada(engine, id, retomada);
    const streamOpts = {
      id,
      ...
      // sid persiste assim que nasce (não só no fim): server.js grava no
      // inflight.json a cada evento, pra sobreviver a queda do app no meio
    ...
    const res = await rodarSessao(engine, promptFinal, streamOpts, retomada.sid, retomada.aposFalha);
