...
async function runHeadlessReview(engine, pr) {
  ...
  try {
    ...
    // cobertura incompleta lidera tudo: é a diferença entre "está limpo" e "não olhei"
    const faltando = engine.coverageGap(result);
    if (faltando.length) {
      result.reasons = [`a revisão não cobriu ${faltando.length} arquivo(s) do diff (lista no relatório), confira antes de aprovar`,
        ...(result.reasons || [])];
    }
    // lacuna de cobertura lidera as reasons: é a informação que explica por que um
    // PR grande sem achado nenhum caiu na sua mesa em vez de auto-aprovar
