...
async function runHeadlessReview(engine, pr) {
  ...
  try {
    ...
    result.reasons = (result.reasons || []).map(r =>
      (r && typeof r === 'object' && typeof r.text === 'string') ? r : { text: String(r), kind: 'content' });
    ...
    if (pr.requested === false && (result.verdict === 'approve' || result.verdict === 'request_changes')) {
      result.reasons = [{ text: 'revisão iniciada por você (não era seu review pedido): nada é postado sem sua decisão', kind: 'gate' },
        ...(result.reasons || [])];
    }
    ...
    if (semCobertura.length) {
      ...
      result.reasons = [{ text: `a revisão não cobriu o diff inteiro (${semCobertura.length} pendência(s): ${amostra}${semCobertura.length > 3 ? ', ...' : ''}), então não posto sozinho`, kind: 'gate' },
        ...(result.reasons || [])];
    }
    ...
    if (gapCheckpoint.length) {
      result.reasons = [{ text: `verificação de afirmações com problema (${gapCheckpoint.join('; ')}), então não posto sozinho`, kind: 'gate' },
        ...(result.reasons || [])];
    }
    ...
    if (contested.length && autoDec.motivo === 'contestacao') {
      ...
      result.reasons = [{ text: `${contested.length} ${label} de outro review no PR, confira a redação antes de postar (detalhe no relatório)`, kind: 'gate' },
        ...(result.reasons || [])];
    }
    ...
    if (canAuto) {
      ...
      result.reasons = [...(result.reasons || []), { text: `falha ao postar o APPROVE: ${post.error}`, kind: 'infra' }];
      ...
      if (classify(post.error).kind === 'transitorio') {
        result.postRetry = { event: 'approve', attempts: 0 };
      }
    }
    ...
    if (canReject) {
      ...
      result.reasons = [...(result.reasons || []), { text: `falha ao postar o REQUEST_CHANGES: ${post.error}`, kind: 'infra' }];
      if (classify(post.error).kind === 'transitorio') {
        result.postRetry = { event: 'request_changes', attempts: 0 };
      }
    }
    ...
    if (autoDec.motivo === 'politica') {
      ...
      result.reasons = [{ text: why, kind: 'gate' }, ...(result.reasons || [])];
    }
