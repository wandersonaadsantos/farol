  ...
  try {
    ...
    if (err.coordenacao) {
      ...
      if (tries < cap) {
        ...
        estacionar(engine, pr.key, msg, 'esgotado', pr.headLido);
...
function estacionar(engine, key, motivo, tipo, head) {
  ...
  engine.parkedMotivos[key] = { at: new Date().toISOString(), motivo: String(motivo || ''), tipo: String(tipo || 'falha'), ...(head ? { head: String(head) } : {}) };
