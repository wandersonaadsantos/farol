...
async function rodarSessao(engine, promptFinal, streamOpts, sid) {
  ...
  // texto neutro de propósito: a retomada tem duas origens (round incremental
  // opt-in e queda transitória), e quem sabe qual é quem já disse antes.
  engine.pushActivity(streamOpts.id, 'info', 'Retomando a sessão anterior (continua a mesma conversa).');
  try {
...
function sidDeRetomada(engine, pr) {
  const valido = (s) => !!s && RESUME_SID_RE.test(String(s));
  if (valido(pr.retomarSid)) return { sid: String(pr.retomarSid), aposFalha: true };
  const optIn = (engine.config || {}).reReviewResume && valido(pr.resumeSid);
  return { sid: optIn ? String(pr.resumeSid) : '', aposFalha: false };
}
...
async function runHeadlessReview(engine, pr) {
  ...
  try {
    ...
    const retomada = sidDeRetomada(engine, pr);
    ...
    const res = await rodarSessao(engine, promptFinal, streamOpts, retomada.sid);
