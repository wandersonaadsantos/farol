...
function summarizeCheckpoint(entries, currentHeadSha) {
  ...
  for (const e of relevantes) {
    const claimNormalizada = String(e.claim || '').trim().replace(/[.!?;:]*$/, '').replace(/\s+/g, ' ').toLowerCase();
...
module.exports = { checkpointPath, appendCheckpointEntry, readCheckpoint, summarizeCheckpoint, resumeBlock };
