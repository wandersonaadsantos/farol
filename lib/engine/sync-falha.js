...
// 401 do banco é ambíguo por construção (regra ou token), e por isso ele é transitório por
// padrão. Com o token RECÉM-OBTIDO ele deixa de ser: renovar de novo ouviria a mesma
// recusa, e quem tem que agir é a pessoa (entrar de novo com a senha). É o corte de
// sessões do 7.C2 e qualquer regra que passe a recusar este aparelho.
function tokenRecemObtido(rt) {
  const fonte = rt && rt.tokenSource;
  if (!fonte || typeof fonte.obtidoHa !== 'function') return false;
  return fonte.obtidoHa() <= SYNC.TOKEN_RECEM_OBTIDO_MS;
}

function permanente(rt, code) {
  if (PERMANENTES.has(code)) return true;
  return code === SYNC_CODES.NAO_AUTORIZADO && tokenRecemObtido(rt);
}
...
function prefixoDaFalha(engine, code, rt) {
  if (code === SYNC_CODES.NAO_AUTORIZADO && tokenRecemObtido(rt)) return 'sincronização entre dispositivos recusada pelo banco; entre de novo com a senha';
...
function registrarFalha(engine, code, motivo) {
  ...
  if (novidade && typeof engine.log === 'function') {
    engine.log('WARN', `${prefixoDaFalha(engine, code, rt)}: ${frase} (${code})`);
  }
...
export default { STATUS, PERMANENTES, TRANSITORIAS, permanente, falhaSem, avisarTela, registrarFalha, superado };
export { STATUS, PERMANENTES, TRANSITORIAS, permanente, falhaSem, avisarTela, registrarFalha, superado };
