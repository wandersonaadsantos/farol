...
// Quem acabou de gravar o nó do admin já sabe o que o próximo giro leria; a decisão por
// assinatura não muda, porque ela sai do batimento e não daqui. O relógio do batimento
// zera: a geração é nova, e esperar o intervalo inteiro deixava a tela até dois intervalos
// dizendo "admin sem sinal de vida" logo depois do "agora é o admin".
function anotarAdmin(rt, { dev, generation }) {
  if (!rt) return;
  const s = sinaisDe(rt);
  s.admin = { dev: String(dev || ''), generation: Number(generation) || 0 };
  s.ultimoBatimentoEm = 0;
}
...
async function observarSinais(engine, { agora }) {
  ...
  // o instante em que o nó do admin foi LIDO com sucesso nesta conexão. Leitura que falha
  // volta antes daqui e não carimba nada: a tela precisa distinguir "ninguém administra"
  // de "ainda não li", e sem este carimbo as duas chegavam como o mesmo campo ausente
  s.adminLidoEm = agora;
...
export default { anotarAdmin, publicarSinal, publicarBatimento, observarSinais, modoAtual, voltarAoLocal, cicloDosSinais, esquecerSinais, NO_BATIMENTO, NO_PRONTIDAO };
export { anotarAdmin, publicarSinal, publicarBatimento, observarSinais, modoAtual, voltarAoLocal, cicloDosSinais, esquecerSinais, NO_BATIMENTO, NO_PRONTIDAO };
