...
// modelo: e o que impede a sessao de se autodeclarar completa. Extraida pra manter
// a profundidade da funcao principal dentro do contrato de qualidade.
function coverageReason(scope) {
  if (!scope || !Array.isArray(scope.total) || !Array.isArray(scope.reviewed)) {
    return { code: 'COVERAGE_UNKNOWN', detail: {} };
  }
  const missing = Array.isArray(scope.missing) ? scope.missing : [];
  if (missing.length || scope.reviewed.length < scope.total.length) {
    return { code: 'COVERAGE_INCOMPLETE', detail: { missing } };
  }
  return null;
}
...
function evaluateQualityEligibility(analysis, observed) {
  ...
  if (blockers.length) contra.push({ code: 'BLOCKER_PRESENT', detail: { count: blockers.length } });

  // quatro valores, nao tres: sobrecarregar `inconclusive` com "nao atende" e
  ...
  else if (a.cardMet !== true) faltando.push({ code: 'CARD_UNKNOWN', detail: {} });

  if (o.sessionOutcome !== 'complete') {
    faltando.push({ code: 'ANALYSIS_INCOMPLETE', detail: { outcome: o.sessionOutcome || null } });
  }

  const cobertura = coverageReason(o.scope);
  if (cobertura) faltando.push(cobertura);
...
const selfprMod = {
  reviewerCandidates, temCandidatos, setReviewers, evaluateQualityEligibility, qualityOf,
