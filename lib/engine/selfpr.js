...
const { SELF_FILE, WORKSPACE, TEMPLATE_DIR } = require('../paths');
const { isPermanentBranch } = require('../format');
...
async function reviewerCandidates(engine) {
  ...
  for (const owner of engine.allOwners()) {
    const env = engine.ghEnv(engine.accountForOwner(owner));
  ...
  engine.reviewerCands = { at: Date.now(), data: byOrg };
  return byOrg;
}
...
async function setReviewers(engine, url) {
  ...
  if (!acc || !(engine.tokens && engine.tokens[acc]) && !engine.token) {
...
function saveSelfAnalyses(engine) {
  try { fs.writeFileSync(SELF_FILE, JSON.stringify(engine.selfAnalyses, null, 2)); }
...
async function fetchMergeState(engine, url) {
  ...
  const r = await run('gh', ['pr', 'view', url, '--json', 'mergeable,mergeStateStatus,isDraft,state'], { env: engine.ghEnv(engine.accountForOwner(m && m[1])) });
  if (!r.ok) return null;
  try {
...
async function enrichMyPRBranches(engine) {
  ...
  for (const pr of (engine.myPRs || [])) {
    const r = await run('gh', ['pr', 'view', pr.url, '--json', 'headRefName,baseRefName,headRefOid'], { env: engine.ghEnv(engine.accountForPr(pr)) });
    if (!r.ok) continue;
    ...
    if (a && a.headSha && pr.headSha && a.headSha !== pr.headSha) {
      ...
      engine.log('WARN', `autoanálise de ${pr.key} descartada: PR mudou (commit novo)`);
      pruned = true;
    }
  }
...
async function fetchAutoMergeAllowed(engine, repo) {
  const r = await run('gh', ['api', `repos/${repo}`, '--jq', '.allow_auto_merge'], { env: engine.ghEnv(engine.accountForOwner(String(repo).split('/')[0])) });
  if (!r.ok) return null;
...
async function fetchRuleBlocked(engine, repo, base) {
  ...
  const r = await run('gh', ['api', `repos/${repo}/rules/branches/${base}`, '--jq', '[.[].type]'], { env: engine.ghEnv(engine.accountForOwner(String(repo).split('/')[0])) });
  if (!r.ok) return null;
  let blocked = null;
  try {
...
async function refreshStaleStates(engine) {
  ...
  const next = {};
  for (const pr of targets) {
    try { next[pr.key] = await engine.staleForReview(pr); }
    catch { next[pr.key] = false; }
  }
  engine.staleStates = next;
}
...
async function mergeSelfPR(engine, url, opts = {}) {
  ...
  if (!(engine.tokens && engine.tokens[acc]) && !engine.token) {
    engine.emit('toast', { kind: 'error', text: `Conta ${acc || '(nenhuma)'} não autenticada no gh. Rode: gh auth login` });
    return { ok: false, error: 'gh sem token' };
  }
...
async function runSelfAnalysis(engine, pr) {
  ...
  try {
    ...
    const shaR = await run('gh', ['pr', 'view', pr.url, '--json', 'headRefOid', '--jq', '.headRefOid'], { env: engine.ghEnv(engine.accountForPr(pr)) });
...
module.exports = {
  reviewerCandidates, setReviewers, saveSelfAnalyses, clearSelfAnalysis, fetchMergeState,
