...
function coverageReasons(scope, limitacoes) {
  ...
  // Sem OBSERVABILIDADE a lacuna é do instrumento, não da análise, e dizer a mesma
  // frase nos dois casos é culpar quem leu por uma limitação de quem mede. O caminho
  // do Codex não emite evento de leitura de arquivo (o stream dele tem
  // command_execution e agent_message, e nada que diga "abri este arquivo"), então ali
  // `reviewed` é vazio por construção. O DESFECHO não muda, o Merge segue indisponível,
  // porque sem prova de leitura não se libera merge; muda o que a linha afirma.
  // Campo ausente é lido como observável, que é o que todo registro em disco era.
  if (faltam.length) {
    out.push(scope.observavel === false
      ? { code: 'COVERAGE_UNOBSERVABLE', detail: {} }
      : { code: 'COVERAGE_INCOMPLETE', detail: { missing: faltam } });
  }
  return out;
}
...
function evaluateQualityEligibility(analysis, observed) {
  ...
  // Bloqueio de FORA do PR (card, infra, outra pessoa) segura o merge igual: o motivo
  // morar em outro lugar não deixa o PR mergeável. Ausente é registro anterior ao campo
  // (tudo ia em `blockers`), e por isso aqui ausência não conta como dado faltando.
  const externos = Array.isArray(a.externalBlockers) ? a.externalBlockers : [];
  if (externos.length) contra.push({ code: 'EXTERNAL_BLOCKER_PRESENT', detail: { count: externos.length } });
  ...
  // O PR ANDOU depois desta análise (carimbado pelo enrichMyPRBranches). É evidência
  // CONTRA, não evidência faltando: o engine não deixou de medir, ele mediu e viu o
  // head mudar. Existe porque o registro deixou de ser apagado nesse caso; enquanto
  // ele era apagado, o gate nunca via um registro vencido e não precisava desta linha.
  // NÃO é o EVIDENCE_STALE abaixo: aquele prova identidade DENTRO do registro (parecer
  // e evidência falam do mesmo sha), este prova que esse sha ainda é o do PR.
  if (o.stale === true) contra.push({ code: 'ANALYSIS_STALE', detail: { head: o.staleHeadSha || null } });
...
// OCULTAR não é APAGAR, e até 01/09/2026 era. O botão da tela dizia "Ocultar análise"
// com o title "é só sua, some da tela; dá pra reanalisar quando quiser", e por baixo
// fazia `delete` no registro: o relatório, as dicas e o `observed` que custaram uma
// sessão inteira morriam num clique que prometia ser reversível, e não existia o par
// "Mostrar". Reanalisar era a ÚNICA forma de ver de novo, e ela custa tokens de novo
// pra reproduzir o que já tinha sido pago.
//
// Agora a visibilidade é um campo do registro. O disco guarda a análise; a tela decide
// se mostra. `hidden` não participa de gate NENHUM (ver evaluateQualityEligibility):
// esconder um parecer não muda o que ele comprova, e ligar as duas coisas seria dar à
// preferência visual autoridade sobre merge.
function setSelfAnalysisVisibility(engine, key, hidden) {
  const a = engine.selfAnalyses[key];
  if (!a) return { ok: false, error: 'não achei essa autoanálise' };
  const alvo = hidden === true;
  if (!!a.hidden === alvo) return { ok: true, hidden: alvo };
  if (alvo) a.hidden = true; else delete a.hidden;
  engine.saveSelfAnalyses();
  engine.pushState();
  return { ok: true, hidden: alvo };
}
...
// (setSelfAnalysisVisibility), nunca com o PR, e vivia sendo confundido com isso.
//
...
// ENVELHECER deixou de ser APAGAR (01/09/2026). Até aqui o registro inteiro era deletado,
// e com ele o relatório, as dicas e o `observed` que uma sessão paga produziu. O head
// andar não torna o texto falso: torna o VEREDITO inválido pro código de agora. São coisas
// diferentes, e apagar as duas juntas era o que obrigava a reanalisar do zero (tokens de
// novo) só pra reler o que já existia.
//
// O carimbo mora em `observed`, não num campo solto, porque é EVIDÊNCIA DO ENGINE (ele
// mediu o head mudar), e é o `observed` que o gate consome. Sem ele o registro persistido
// continuaria `eligible` e o botão Merge apareceria sobre análise vencida: EVIDENCE_STALE
// não cobre este caso, porque compara parecer com evidência DENTRO do registro, e os dois
// estão carimbados no mesmo sha.
//
// Devolve se MUDOU alguma coisa (o chamador decide gravar). Carimba UMA vez: o `farol.log`
// é de falha, não de estado, e reescrever o WARN a cada ciclo de polling inundaria o
// Diagnóstico com a mesma linha (invariante 3).
function marcarDesatualizada(engine, pr) {
  const a = engine.selfAnalyses[pr.key];
  if (!a || !pr.headSha) return false;
  if (a.observed && a.observed.stale) return false;
  if (a.headSha && a.headSha === pr.headSha) return false;
  a.observed = { ...(a.observed || {}), stale: true, staleHeadSha: pr.headSha };
  // `parcial`, não `descartada`: o gasto não virou merge, mas virou um relatório que
  // continua na tela e continua sendo lido. Dizer "descartada" agora seria o erro inverso
  // do que o carimbo existe pra evitar.
  if (a.usageId) engine.marcarDesfecho(a.usageId, 'parcial');
  delete engine.mergeStates[pr.key];
  engine.log('WARN', a.headSha
    ? `autoanálise de ${pr.key} desatualizada: PR mudou (commit novo); o relatório fica, o veredito não vale mais`
    : `autoanálise de ${pr.key} desatualizada: análise sem SHA registrado (não dá pra provar que vale pro commit atual)`);
  return true;
}
...
async function enrichMyPRBranches(engine) {
  ...
  for (const pr of (engine.myPRs || [])) {
    ...
    if (marcarDesatualizada(engine, pr)) pruned = true;
  }
...
// allowlist fechada: modo desconhecido cai em 'normal', que é o mais restrito
const MODOS_DE_MERGE = new Set(['auto', 'admin']);

async function mergeSelfPR(engine, url, opts = {}) {
  const mode = MODOS_DE_MERGE.has(opts.mode) ? opts.mode : 'normal';
...
//  diz se o provedor da sessao emite evento de leitura de arquivo. Falso
// no Codex, cujo stream nao tem esse evento, e por isso  la e vazio por
// construcao, e nao por falta de leitura. Omitido = observavel, que e o que todo
// registro ja gravado era.
function montarObserved(headSha, sessionOutcome, escopo, lidos, verification, cardRes, observavel = true) {
  const total = (escopo && Array.isArray(escopo.total)) ? escopo.total : null;
  const reviewed = total ? total.filter(p => lidos.has(p)) : [];
  return {
    headSha: headSha || '',
    sessionOutcome,
    scope: total ? { total, reviewed, missing: total.filter(p => !lidos.has(p)), observavel } : null,
    verification,
...
function parseSelfResult(engine, raw) {
  ...
  data.externalBlockers = exigeLista(data.externalBlockers, 'externalBlockers', false);
  ...
  if (coerente && data.externalBlockers.length) {
    throw new Error('JSON da autoanálise fora do contrato: verdict approvable com externalBlockers listados');
  }
  return data;
}

// O aviso separa o que é do PR do que é de fora dele: "3 pontos antes de pedir review"
// somando os dois levaria quem lê a procurar no código o que mora no card.
function pontosAntesDoReview(result) {
  const noPr = result.blockers.length;
  const fora = (result.externalBlockers || []).length;
  const partes = [];
  if (noPr) partes.push(`${noPr} ponto(s) no PR`);
  if (fora) partes.push(`${fora} fora do PR`);
  return partes.length ? `${partes.join(', ')} antes de pedir review` : 'sem ponto listado';
}
...
async function runSelfAnalysis(engine, pr) {
  const { id, rotulo } = novoIdDeSessao(engine, 's');
  const scopeRoot = scopeMod.scopeRootFor(pr.key);
  engine.activeReviews.set(id, {
    id, rotulo, keys: [pr.key], label: `Autoanálise de ${pr.key}`, mode: 'self',
  ...
  try {
    ...
    // o Codex nao reporta leitura de arquivo, entao a cobertura dele nao e observavel:
    // sem isso o gate acusaria a ANALISE de incompleta por uma limitacao do INSTRUMENTO
    const auth = engine.resolveClaudeAuth(accPr) || {};
    const observed = montarObserved(shaAntes, sessionOutcome, escopo, lidos, verif, cardRes, auth.kind !== 'codex');
    ...
    const cabeca = parecerOk
      ...
      : `🔧 ${pr.key}: precisa de ajuste (${pontosAntesDoReview(result)}).`;
    engine.emit('toast', {
...
const selfprMod = {
  ...
  saveSelfAnalyses, setSelfAnalysisVisibility,
