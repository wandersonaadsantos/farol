...
import { MAX_RODADAS_AUTO_DIA, diaLocal, normalizeAncora, proximaAncora } from './review.js';
  ...
  for (const pr of (engine.myPRs || [])) {
    ...
    if (a && pr.headSha && (!a.headSha || a.headSha !== pr.headSha)) {
      ...
      engine.log('WARN', a.headSha
        ? `autoanálise de ${pr.key} descartada: PR mudou (commit novo); refaço sozinho no head novo`
        : `autoanálise de ${pr.key} descartada: análise sem SHA registrado (não dá pra provar que vale pro commit atual)`);
      // a análise foi pedida uma vez e o commit a invalidou: refaz no head novo
      // sem esperar clique (travado por debounce, âncora e teto, ver launchSelfReanalyses)
      marcaReanalise(engine, pr, pr.headSha);
      pruned = true;
    }
  }
...
/* ---------- relançamento da autoanálise (29/08/2026) ----------
   A autoanálise nasceu 100% manual: `launchSelfAnalysis` tinha um chamador só,
   a rota do botão. Quando entrava commit novo, o resultado era descartado (nos
   dois caminhos: `enrichMyPRBranches` no ciclo seguinte, e a re-checagem
   pós-sessão do `runSelfAnalysis`) e nada relançava, porque a regra escrita era
   "relançar é decisão do usuário". Na prática isso queimou 10 sessões em 3 dias
   sem entregar nada, e é o oposto da autonomia que o Wanderson pede.

   O que muda: uma análise que VOCÊ pediu e que um commit invalidou volta
   sozinha sobre o head novo. NÃO é "analisar todo PR meu automaticamente", que
   seria capacidade nova; é terminar o trabalho que já foi pedido.

   As travas são as MESMAS do round de review, de propósito (quem mexer lá, mexe
   aqui): debounce de head quieto, âncora por head, teto diário, conta com token,
   não silenciada, `autoReview` ligado, orçamento livre, e nada de duplicar o que
   já está na fila ou rodando. E a lição da v2.54.3: a âncora só é gravada por
   relançamento que ACONTECEU, nunca por alvo barrado, senão a trava de hoje vira
   cancelamento permanente. */

// Registra a INTENÇÃO de refazer a análise no head novo. Sem head não registra:
// intenção cega nunca vira sessão.
function marcaReanalise(engine, pr, head, agora = Date.now()) {
  const key = pr && pr.key;
  const h = String(head || '').trim();
  if (!key || !h) return;
  if (!engine.selfReanalisePendente || typeof engine.selfReanalisePendente !== 'object') engine.selfReanalisePendente = {};
  engine.selfReanalisePendente[key] = { head: h, at: agora };
  if (typeof engine.saveSelfReanalise === 'function') engine.saveSelfReanalise();
}

// Poda: PR que saiu dos "Meus PRs" (fechou, mergeou) leva junto intenção e âncora.
function podarReanalise(engine, abertos) {
  let mudou = false;
  for (const mapa of [engine.selfReanalisePendente, engine.selfReanaliseLancada]) {
    if (!mapa || typeof mapa !== 'object') continue;
    for (const key of Object.keys(mapa)) {
      if (!abertos.has(key)) { delete mapa[key]; mudou = true; }
    }
  }
  if (mudou && typeof engine.saveSelfReanalise === 'function') engine.saveSelfReanalise();
}

// Já está na fila ou rodando? Mesma pergunta do launchSelfAnalysis.
function reanaliseEmAndamento(engine, key) {
  return [...engine.activeReviews.values()].some(s => s.mode === 'self' && (s.keys || []).includes(key))
    || engine.headlessQueue.some(p => p.kind === 'self' && p.key === key);
}

/* Quem relança AGORA. Sem IO: usa o `headSha` que o enrichMyPRBranches acabou de
   buscar fresco neste mesmo ciclo, então a checagem de head não custa chamada
   nenhuma. Devolve os PRs prontos; quem carimba a âncora é o chamador, depois de
   enfileirar de verdade. */
function selfReanalysisTargets(engine, agora = Date.now()) {
  const pendentes = engine.selfReanalisePendente || {};
  const alvos = [];
  for (const pr of (engine.myPRs || [])) {
    const intencao = pendentes[pr.key];
    if (!intencao || !intencao.head) continue;
    // head andou DE NOVO desde a intenção: recomeça o relógio no head atual e
    // espera, senão uma rajada de pushes vira uma sessão por push
    const headAtual = String(pr.headSha || '').trim();
    if (headAtual && headAtual !== intencao.head) {
      pendentes[pr.key] = { head: headAtual, at: agora };
      continue;
    }
    if ((agora - Number(intencao.at || agora)) < TEMPOS.HEAD_QUIETO_MS) continue;
    if (pr.isDraft) continue;
    const acct = engine.accountForPr(pr);
    if (!engine.tokenFor(acct)) continue;
    if (engine.isMuted(acct)) continue;
    if (!engine.autoReviewFor(acct)) continue;
    if (engine.budgetBlockedFor(acct)) continue;
    if (engine.autoReviewParked.has(pr.key)) continue;
    if (reanaliseEmAndamento(engine, pr.key)) continue;
    const ancora = normalizeAncora((engine.selfReanaliseLancada || {})[pr.key]);
    if (ancora.head === intencao.head) continue;
    if (ancora.dia === diaLocal(agora) && ancora.rodadas >= MAX_RODADAS_AUTO_DIA) continue;
    alvos.push({ pr, head: intencao.head });
  }
  return alvos;
}

async function launchSelfReanalyses(engine, agora = Date.now()) {
  const alvos = selfReanalysisTargets(engine, agora);
  if (!alvos.length) return;
  if (!engine.selfReanaliseLancada || typeof engine.selfReanaliseLancada !== 'object') engine.selfReanaliseLancada = {};
  for (const alvo of alvos) {
    engine.selfReanaliseLancada[alvo.pr.key] = proximaAncora(engine.selfReanaliseLancada[alvo.pr.key], alvo.head, agora);
    delete engine.selfReanalisePendente[alvo.pr.key];
    engine.headlessQueue.push({ ...alvo.pr, account: engine.accountForPr(alvo.pr), kind: 'self' });
  }
  if (typeof engine.saveSelfReanalise === 'function') engine.saveSelfReanalise();
  engine.emit('toast', {
    kind: 'info',
    text: alvos.length === 1
      ? `↻ ${alvos[0].pr.key} recebeu commit novo depois da sua análise: analisando de novo.`
      : `↻ ${alvos.length} PRs seus receberam commit novo depois das análises: analisando de novo.`
  });
...
async function runSelfAnalysis(engine, pr) {
  ...
  try {
    ...
    if (shaAntes && shaDepois && shaAntes !== shaDepois) {
      engine.log('WARN', `autoanálise de ${pr.key} descartada: commit novo durante a análise (${shaAntes.slice(0, 7)} -> ${shaDepois.slice(0, 7)}); refaço sozinho no head novo`);
      engine.emit('toast', { kind: 'info', text: `${pr.key}: entrou commit novo durante a autoanálise; refaço sozinho quando o head parar de andar.` });
      marcaReanalise(engine, pr, shaDepois);
      return;
    }
    ...
    // entregou: a intenção de refazer morre aqui (senão o ciclo relançaria de novo)
    if (engine.selfReanalisePendente) {
      delete engine.selfReanalisePendente[pr.key];
      if (typeof engine.saveSelfReanalise === 'function') engine.saveSelfReanalise();
    }
...
const selfprMod = {
  ...
  marcaReanalise, podarReanalise, selfReanalysisTargets, launchSelfReanalyses,
};
