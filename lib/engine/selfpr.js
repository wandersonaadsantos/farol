...
const { SELF_FILE, HIDDEN_FILE, WORKSPACE, TEMPLATE_DIR } = require('../paths');
...
// this.myPRs. `listaOk` diz se a busca de PRs meus funcionou NESTE ciclo (o mesmo
// sinal que ja decide se this.myPRs foi substituido ou preservado).
//
// Duas regras, e a segunda e a sutil:
//  1) chave ainda na lista com updatedAt DIFERENTE do guardado = o PR recebeu
//     atividade nova, entao ele volta a aparecer sozinho. Entrada guardada com
...
//  2) chave que sumiu da lista = o PR fechou/mergeou e a entrada virou lixo, mas
//     SO limpamos quando `listaOk`. Com a busca falha, myPRs fica congelado do ciclo
//     anterior (ou vazio, logo apos o boot) e "sumiu da lista" nao prova nada: limpar
//     ali faria uma queda de rede desocultar TODOS os PRs de uma vez, justamente no
//     ciclo em que o usuario nao tem como saber o que aconteceu.
function reconcileHiddenPRs(engine, listaOk) {
  ...
  for (const [key, entry] of Object.entries(engine.hiddenPRs)) {
    ...
    if (listaOk) { delete engine.hiddenPRs[key]; mudou = true; }
  }
...
async function fetchMergeState(engine, url) {
  ...
  const r = await run('gh', ['pr', 'view', url, '--json', 'mergeable,mergeStateStatus,isDraft,state,baseRefName'], { env: engine.ghEnv(acc) });
  if (!r.ok) return null;
  try {
