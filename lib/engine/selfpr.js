...
function montarObserved(headSha, sessionOutcome, escopo, lidos, verification) {
...
function verificacaoObservada(entries, headSha) {
  const doHead = (entries || []).filter(e => e && (!e.headSha || !headSha || e.headSha === headSha));
...
async function runSelfAnalysis(engine, pr) {
  ...
  try {
    ...
    const observed = montarObserved(shaAntes, sessionOutcome, escopo, lidos, verif);
