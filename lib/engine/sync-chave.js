...
import reautenticacao from './sync-reautenticacao.js';
...
// anterior.
// A presença anuncia `keyReady`, e é ela que os outros leem para decidir se vale publicar
// (lib/sync/frota.js). Sem reescrever aqui, o aparelho que acabou de abrir a chave seguiria
// anunciando "fechada" até o próximo carimbo, que é de 5 em 5 minutos: nesse meio tempo
// ninguém publica para ele, e a transferência o recusa por "sem a chave do conjunto aberta".
// Best-effort de propósito: falhar o anúncio não desfaz a chave que já abriu aqui.
async function anunciarChavePronta(engine) {
  if (typeof engine.syncPresenca !== 'function') return;
  try { await engine.syncPresenca(); } catch { /* o próximo carimbo tenta de novo */ }
}

async function guardarMaterial(engine, rt, cfg, r) {
  rt.material = r.material;
  // a geração corrente vive no chaveiro, e é ela que diz com qual K_enc cifrar o que sobe
  rt.cur = r.chaveiro.cur;
  rt.chaveVista = true;
  rt.chaveMotivo = '';
  ...
  avisar(engine);
  await anunciarChavePronta(engine);
  return { ok: true };
}
...
// Boot e reconexão: reabre a chave pelo cache local, sem senha e sem scrypt, só quando o
// cache é deste destino e o kcv confere com o chaveiro do banco. Sem isto o cache nunca
// era lido no boot, e cada reinício (o do auto-update inclusive) trancava o compartilhamento
// cifrado até alguém digitar a senha de novo. Cache que não confere não abre nada e não é
// apagado: ele é o que salva a recuperação depois de uma troca de senha.
async function abrirPeloCache(engine, cfg) {
  const rt = engine.sync;
  if (!sharedActive(cfg) || !rt || rt.material || !rt.client || !rt.uid) return { ok: false, code: 'nada-a-abrir' };
  const local = cacheChave.lerCache();
  if (!cacheChave.cacheServe(local, { uid: rt.uid, destino: outboxTarget(rt.uid, cfg.databaseUrl) })) return { ok: false, code: 'sem-cache' };
  const lido = await chaveiro.lerChaveiro(rt.client, rt.uid);
  if (!lido.ok || !lido.chaveiro) return { ok: false, code: 'chaveiro-indisponivel' };
  if (!cacheChave.cacheConfere(local, lido.chaveiro)) return { ok: false, code: 'cache-diverge' };
  return guardarMaterial(engine, rt, cfg, { material: { v: 1, id: local.id, enc: { ...local.enc } }, chaveiro: lido.chaveiro });
}
...
async function syncUnlock(engine, cfg, fetchImpl, { password } = {}) {
  ...
  reautenticacao.adotarEntrada(rt, entrada);
  return abrirChaveDoConjunto(engine, rt, cfg, senha);
}

export default { estadoDaChave, motivoDaChave, falhaDaChave, guardarMaterial, abrirChaveDoConjunto, syncUnlock, syncGerarChaveNova, esquecerChave, abrirPeloCache };
export { estadoDaChave, motivoDaChave, falhaDaChave, guardarMaterial, abrirChaveDoConjunto, syncUnlock, syncGerarChaveNova, esquecerChave, abrirPeloCache };
