...
// resposta: o que decide é o JSON. Roda com o mesmo ambiente limpo da sessão de login.
async function statusDoClaude(dir) {
  const r = await io.runShell('claude auth status --json', { env: loginConsoleEnv(dir) });
  return lerStatus(r);
}
