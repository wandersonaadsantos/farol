...
const TEXT_RULES = [
  ...
  // A versão do Farol viaja só na tabela de Consumo (lib/engine/usage.js), nunca
  // no texto de review. O alvo é PROVENIÊNCIA (quem/o que fez a revisão), não
  // qualquer menção: "Farol vX.Y.Z" sozinho é assunto técnico legítimo em PR de
  // release do próprio repo (ex.: "o Farol v2.42.0 corrige o bug do detector",
  // "bumpa o Farol pra v2.42.0"), e a doutrina do projeto já protege isso (Farol
  // NÃO é proibido sozinho). Por isso a regra exige, no mesmo padrão verbo+
  // preposição das regras irmãs acima, um verbo de proveniência (gerado, revisado,
  // postado, analisado, aprovado, produzido, feito, criado, escrito, redigido,
  // elaborado, preparado) colado numa preposição (pelo/pela/por/com/via) logo antes
  // do par Farol+versão, nas duas ordens do par ("Farol vX.Y.Z" e "vX.Y.Z do/da
  // Farol"): só aí a versão denuncia proveniência tão bem quanto "gerado pelo
  // Farol" sozinho, e escapava do STRONG_REVIEW_ACTOR porque muitas dessas frases
  // não têm objeto de review nem auto-referência (ver contextualLanguageIssues).
  ['tool_version', /\b(?:gerad|revisad|postad|analisad|aprovad|produzid|feit|criad|escrit|redigid|elaborad|preparad)\w*\s+(?:pelo|pela|por|com|via)\s+(?:o\s+|a\s+)?farol\s+v?\d{1,4}\.\d{1,4}\.\d{1,4}\b/],
  ['tool_version', /\b(?:gerad|revisad|postad|analisad|aprovad|produzid|feit|criad|escrit|redigid|elaborad|preparad)\w*\s+(?:pelo|pela|por|com|via)\s+(?:o\s+|a\s+)?v?\d{1,4}\.\d{1,4}\.\d{1,4}\s+(?:do\s+|da\s+)?farol\b/],
];
...
// Falar de SI em terceira pessoa (23/09/2026). Caso medido em biudtech/biud-frontend#1150:
// rodando na conta @Gabrielk5, o Farol postou "@Gabrielk5 ja deixou isso registrado na
// review de aprovacao". Quem escreveu e quem foi citado eram a mesma conta. Esta na mesma
// familia do vocabulario de proveniencia que este arquivo ja recusa: nao e um deslize de
// estilo, e a prova, no texto publicado, de que quem escreveu nao e a pessoa que assina.
//
// Le o texto JA MASCARADO, entao a propria conta dentro de crase, bloco de codigo ou URL
// nao conta: ali ela e literal tecnico (o nome de um job, o dono de um repositorio), e nao
// alguem sendo citado. Sem login conhecido nao existe regra: conta desconhecida nunca pode
// virar bloqueio de texto legitimo.
const ACAO_DE_QUEM_REVISA = '(?:ja\\s+)?(?:deixou|apontou|comentou|escreveu|disse|registrou|revisou|aprovou|reprovou|pediu|sugeriu|levantou|mencionou|observou|notou|indicou)';

function selfMentionIssues(text, login, field) {
  const quem = String(login || '').trim();
  if (!quem) return [];
  const nome = fold(quem).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  if (!nome) return [];
  // com arroba e sempre mencao a uma pessoa, e a pessoa aqui e o proprio autor do texto
  if (new RegExp('@' + nome + '\\b').test(text)) return [{ field, code: 'self_third_person' }];
  // sem arroba so conta como mencao quando o nome age como quem revisa
  if (new RegExp('\\b' + nome + '\\b\\s+' + ACAO_DE_QUEM_REVISA + '\\b').test(text)) {
    return [{ field, code: 'self_third_person' }];
  }
  return [];
}

function publicTextLanguageIssues(value, field = 'body', login = '') {
  ...
  issues.push(...selfMentionIssues(text, login, field));
...
// `login` e a conta que VAI POSTAR este texto. Opcional de proposito: quem nao souber
// dizer quem assina continua com o gate de sempre, sem a regra de identidade.
function publicReviewLanguageIssues(payload, login = '') {
  const issues = publicTextLanguageIssues(payload && payload.body, 'body', login);
  const comments = Array.isArray(payload && payload.comments) ? payload.comments : [];
  comments.forEach((comment, index) => {
    issues.push(...publicTextLanguageIssues(comment && comment.body, `comments[${index}].body`, login));
  });
  return issues;
}
...
// A ação explícita manda; sem ela, o veredito decide. O par oposto vem por último
// porque é o menos provável de ser o texto certo, nunca porque seja inválido.
function ordemDosPayloads(item) {
  if (item.action) {
    const oposto = item.action === 'approve' ? 'request_changes' : 'approve';
    return [item.action, 'comment', oposto];
  }
  if (item.verdict === 'request_changes') return ['request_changes', 'comment', 'approve'];
...
function reviewMarkdownForUi(item) {
  ...
  const order = ordemDosPayloads(item);
...
// Categorias de motivo que a tela agrupa (ui/pure.js). 'content' é o padrão de
// propósito: entrada antiga do decisions.json (string solta, gravada antes da
// v2.48.0) e qualquer coisa sem etiqueta reconhecida cai nela, que é a leitura
// conservadora ("a revisão apontou isso"), nunca inventando um gate ou uma falha
// de infra que não houve.
const REASON_KINDS = new Set(['gate', 'content', 'infra']);

// Um motivo chega como { text, kind } (v2.48.0+) ou como string solta (histórico
// gravado antes). Normaliza os dois num par só; etiqueta desconhecida cai no
// padrão pelo mesmo motivo que a string solta cai.
function reasonPair(value) {
  if (!value || typeof value !== 'object') return { raw: value, kind: 'content' };
  return { raw: value.text, kind: REASON_KINDS.has(value.kind) ? value.kind : 'content' };
}

function uniqueHumanTexts(values) {
  const out = [], seen = new Set();
  for (const value of Array.isArray(values) ? values : []) {
    const { raw, kind } = reasonPair(value);
    const text = diagnosticTextForUi(raw);
    const key = fold(text).trim();
    if (text && !seen.has(key)) { seen.add(key); out.push({ text, kind }); }
  }
  return out;
}
...
// blockedReason (G13) entra direto: é texto NOSSO, fixo, gravado só por decide()
// quando postReview bloqueia por internal_language, nunca deriva do relatório.
function decisionForUi(item) {
  if (!item) return null;
  const projected = {};
  for (const key of [
    'id', 'createdAt', 'resolvedAt', 'key', 'card', 'cardMet', 'verdict',
    'status', 'action', 'author', 'account', 'blockedReason', 'blockedKind',
    // head lido e head que andou: o card de commit novo mostra "de X para Y" (v2.59.3)
    'headSha', 'blockedHead',
  ...
  if (item.pr && typeof item.pr === 'object') {
    ...
    // A conta dona da decisão mora em `pr.account` (quem grava é a revisão, que já a
    // resolveu). O campo do topo é o que a UI lê pra filtrar por conta, e ele só existe
    // em quem o gravou lá: sem esta linha a projeção PODAVA a conta, a tela caía no mapa
    // org->conta e uma org fora do config resolvia vazio, sumindo o card de todo filtro
    // (Edicoes-CNBB#13, 10/09/2026). Org mapeada mascarava a poda, por isso demorou.
    if (projected.account === undefined && item.pr.account !== undefined) projected.account = item.pr.account;
  }
  if (item.stages && typeof item.stages === 'object') {
    const sts = Array.isArray(item.stages.stages) ? item.stages.stages : [];
    projected.stages = {
      totalMs: Number(item.stages.totalMs) || 0,
      stages: sts.map(s => ({ id: String(s.id || ''), label: String(s.label || ''), ms: Number(s.ms) || 0 })),
    };
  }
  ...
  // postRetry entra PROJETADO, nunca cru: `lastError` é stderr do gh (texto de
  // fora, pode carregar qualquer coisa) e não tem por que chegar na tela. A tela
  // só precisa saber se o app ainda vai tentar postar sozinho ou se desistiu.
  if (item.postRetry && typeof item.postRetry === 'object') {
    projected.postRetry = {
      attempts: Number(item.postRetry.attempts) || 0,
      exhausted: item.postRetry.exhausted === true,
    };
  }
...
const publicReview = {
  ...
  publicTextLanguageIssues, selfMentionIssues, publicReviewLanguageIssues, normalizeReviewPayload,
