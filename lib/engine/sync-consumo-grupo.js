...
// De quem o grupo lê. São TRÊS perguntas diferentes, e até 20/09/2026 elas eram uma só:
//
//   1. de quem eu EXIJO dado fresco?      aparelho da conta, não aposentado;
//   2. que gasto entra na janela?         o que já foi publicado, aposentado ou não;
//   3. que reserva ainda vale?            a que tem retrato fresco (TTL de `reservasDe`).
//
// Misturar as três dava um travamento permanente: um aparelho aposentado e desligado
// continuava sendo exigido, nunca publicava retrato novo, devolvia `sem-dados` e o grupo
// ficava para sempre não verificável, segurando toda conta controlada.
//
// ATENÇÃO ao que NÃO mudou: o filtro continua sem olhar `lastSeenAt`. Usar
// `frota.aparelhoV2` aqui (que exige presença em 24 h) tiraria da exigência o aparelho
// apenas OFFLINE, e aí bastaria alguém desligar o notebook por um dia para o teto do grupo
// se soltar sozinho. Sumir não é aposentar: uma é um fato do mundo, a outra é um ato
// explícito do admin (lib/engine/sync-aparelho.js).
//
// O próprio aparelho fica de fora dos dois: o gasto dele vem do disco, em `localDoGrupo`.
function aparelhoDaConta(d) {
  return objeto(d) && d.contract === 2 && d.keyReady === true;
}

function aposentado(d) {
  return objeto(d) && Number(d.retiredAt) > 0;
}

function participantes(rt) {
  const devices = objeto(rt.devices) ? rt.devices : {};
  return Object.keys(devices).filter((dev) => dev !== rt.deviceId && aparelhoDaConta(devices[dev]));
}
...
async function lerRemotos(rt, inicio) {
  ...
  for (const dev of participantes(rt)) {
    ...
    // `aposentado` viaja junto porque quem avalia precisa saber se a falta de retrato é
    // uma pendência (aparelho ativo que não publicou) ou o estado esperado
    remotos.push({ dev, rollups, aposentado: aposentado(devices[dev]), status: arvore ? statusRemoto(rt, dev, arvore) : null });
  }
  return remotos;
}
...
function resumoParaTela(engine) {
  ...
  return aceitos.map((x) => x && x.grupo).filter(objeto).map((g) => {
    ...
    return { ...grupo.resumoDoGrupo(g, { vinculos }), requisitos, ...visaoDoRetrato(engine, g, retratos[g.id]) };
  });
}

const SEM_RETRATO = { verificavel: null, custoUsd: null, parcialmenteEstimado: false, motivos: [], calculadoEm: 0, tetoAtingido: null, projecaoUsd: null };

// Retrato velho vale como não verificável no gate (statusDoGrupo); a tela diz o mesmo, com
// o mesmo motivo, em vez de mostrar como fechada uma soma que ninguém usa mais.
function motivosDoRetrato(retrato, agora) {
  if (agora - Number(retrato.calculadoEm) > SYNC.GRUPO_RETRATO_MAX_MS) return [{ dev: '', motivo: 'retrato-velho' }];
  return (Array.isArray(retrato.motivos) ? retrato.motivos : []).map((m) => ({ dev: String(m.dev || ''), motivo: String(m.motivo || '') }));
}

// Sem retrato (grupo inativo ou ainda não calculado), a tela não inventa número. Com ele,
// teto atingido e projeção saem do MESMO veredito do gate, nunca de uma conta da tela.
function visaoDoRetrato(engine, g, retrato, agora = Date.now()) {
  if (!retrato) return { ...SEM_RETRATO };
  const motivos = motivosDoRetrato(retrato, agora);
  const verificavel = retrato.verificavel === true && motivos.length === 0;
  const base = { verificavel, custoUsd: retrato.custo, parcialmenteEstimado: retrato.parcialmenteEstimado, motivos, calculadoEm: Number(retrato.calculadoEm) || 0 };
  if (!verificavel) return { ...base, tetoAtingido: null, projecaoUsd: null };
  const v = veredito(engine, g, retrato, '');
  return { ...base, tetoAtingido: v.bloqueado, projecaoUsd: v.projecaoUsd };
}
