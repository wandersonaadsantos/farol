...
const { WORKSPACE, HOME, IS_WIN, IS_LINUX } = require('../paths');
const { ensureDir } = require('../io');
const { DEFAULT_PORT, TEMPOS } = require('../constants');
...
function spawnConsolePosix(engine, slash, label, keys, account, launcher) {
  ...
  const child = spawn(launcher.cmd, [...launcher.args, script], { stdio: 'ignore' });
  child.on('error', (err) => {
