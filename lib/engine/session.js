...
import { WORKSPACE, HOME, IS_WIN, IS_LINUX } from '../paths.js';
import { ensureDir } from '../io.js';
import { DEFAULT_PORT, TEMPOS } from '../constants.js';
import env from '../env.js';
import { modelLabel, semJsonText } from '../format.js';
import { sanitizeClaudeModel, sanitizeClaudeEffort, effortForModel, claudeAuthShellLines, claudeAuthPosixPrefix, applyClaudeAuthEnv } from '../parse.js';
import { logSpawn } from '../spawnlog.js';
// mesma tabela que o review.js usa pra decidir retry/estacionamento: uma fonte só pro
// que é infra, senão o envelope e a sessão morta discordariam sobre a mesma falha.
import { classify, DESCONHECIDO } from '../log-taxonomy.js';
import { checkpointPath, appendCheckpointEntry } from './verification-checkpoint.js';
import { prPathFromRead } from './pr-scope.js';
...
import loginCelular from './login-celular.js';
...
function buildSessionScript(engine, slash, account, reviewCap = '') {
  const stub = env.reviewCmdStub(); // usado so em testes: substitui o claude
  ...
  return [
    ...
    `cd /d "${WORKSPACE}"`,
    `set "FAROL_PORT=${String(engine.config.port || DEFAULT_PORT)}"`,
...
function buildSessionScriptMac(engine, slash, id, user, reviewCap = '') {
  ...
  return [
    ...
    `    --data '{"id":"${id}"}' 'http://127.0.0.1:${engine.config.port || DEFAULT_PORT}/api/session-exit' >/dev/null 2>&1`,
    '  rm -f -- "$0"',
    '}',
    'trap notify EXIT',
    'export GH_PAGER=cat PAGER=cat',
    `export FAROL_PORT='${String(engine.config.port || DEFAULT_PORT)}'`,
    `export FAROL_REVIEW_CAP='${String(reviewCap || '')}'`,
    authLines,
    `GH_TOKEN="$(gh auth token${userArg} 2>/dev/null)"`,
    // com conta pedida, token vazio ABORTA como o ghEnv do Windows (raiz A1):
    // seguir calado cairia na conta ativa do keyring e a sessão agiria no gh
    // com identidade errada. Sem conta atribuível, segue no login do keyring.
    ...(acc ? [
      'if [ -z "$GH_TOKEN" ]; then',
      `  echo " [Farol] ERRO: conta '${acc}' nao autenticada no gh deste Mac. Rode: gh auth login"`,
      '  exit 1',
      'fi'
    ] : []),
...
// Terminais Linux em ordem de preferência: x-terminal-emulator é o alternatives
// do Debian/Ubuntu (respeita a escolha do usuário), os demais são fallback comum.
...
// PURA (exists injetável pra teste): primeiro candidato cujo executável existe,
// { cmd, args }, ou null; quem chama avisa ALTO, nunca falha em silêncio.
function pickLinuxTerminal(candidates, exists) {
  for (const [cmd, args] of (candidates || [])) {
    if (exists(cmd)) return { cmd, args };
  }
  return null;
}
// existe no PATH? varredura direta dos diretórios, sem spawn e sem shell
function commandOnPath(cmd) {
  return String(process.env.PATH || '').split(':').filter(Boolean).some(d => {
    try { fs.accessSync(path.join(d, cmd), fs.constants.X_OK); return true; } catch { return false; }
  });
}

// Núcleo compartilhado mac/linux da sessão de terminal: escreve o script bash
// (o MESMO buildSessionScriptMac, que é POSIX genérico) e lança pelo comando do
// SO. launcher = { cmd, args, nome } (open -a Terminal no mac, o terminal
// escolhido no linux). Os dois lançadores retornam na hora, então vale o mesmo
// contrato do M5: exit != 0 = a janela nunca abriu, limpa e devolve as keys.
function spawnConsolePosix(engine, slash, label, keys, account, launcher) {
  ...
  logSpawn(`spawnConsole(${launcher.nome})`, [launcher.cmd, ...launcher.args, script]);
  const child = cp.spawn(launcher.cmd, [...launcher.args, script], { stdio: 'ignore' });
  child.on('error', (err) => {
    try { fs.unlinkSync(script); } catch { /* best-effort: unlink de tmp que pode nem ter sido criado */ }
    revokeSessionReviewCaps(engine, id);
    engine.activeReviews.delete(id);
    engine.log('ERROR', `falha ao abrir sessao "${label}" (${launcher.nome}): ${err.message}`);
    engine.emit('toast', { kind: 'error', text: `Não consegui abrir a sessão: ${err.message}` });
    engine.pushState();
  });
  // o lançador retorna na hora: exit 0 = terminal lançado (o fim REAL da sessão
  // chega depois, pelo trap EXIT do script via /api/session-exit). Exit != 0 =
  // a janela nunca abriu (permissão de automação negada, MDM): sem isto o pill
  // ficava preso e as keys vistas pra sempre, PR sumido da fila (M5). O delete
  // devolve false se o error já limpou (pode disparar error E exit pro mesmo filho).
  child.on('exit', (code) => {
    if (!code) return;
    try { fs.unlinkSync(script); } catch { /* best-effort: unlink de tmp em caso de falha */ }
    revokeSessionReviewCaps(engine, id);
    if (!engine.activeReviews.delete(id)) return;
    for (const k of keys) engine.unsee(k);
    engine.log('ERROR', `falha ao abrir sessao "${label}" (${launcher.nome}): saiu com codigo ${code}`);
...
function spawnConsoleMac(engine, slash, label, keys = [], account) {
  return spawnConsolePosix(engine, slash, label, keys, account, { cmd: 'open', args: ['-a', 'Terminal'], nome: 'Terminal.app' });
}

function spawnConsoleLinux(engine, slash, label, keys = [], account) {
  const term = pickLinuxTerminal(LINUX_TERMINALS, commandOnPath);
  if (!term) {
    engine.log('ERROR', `sessao "${label}": nenhum emulador de terminal encontrado (x-terminal-emulator/gnome-terminal/konsole/xterm)`);
    engine.emit('toast', { kind: 'error', text: 'Nenhum emulador de terminal encontrado. Instale um (ex.: sudo apt install gnome-terminal) pra abrir sessões interativas.' });
    return;
  }
  return spawnConsolePosix(engine, slash, label, keys, account, { cmd: term.cmd, args: term.args, nome: term.cmd });
}
...
function handleSessionExit(engine, { keys = [], label, id, code, login } = {}) {
  revokeSessionReviewCaps(engine, id);
  engine.activeReviews.delete(id);
  ...
  // fim de sessão de login (macOS e Linux chegam por aqui): o selo do perfil se atualiza
  if (login) engine.doctor().catch(err => { void err; });
  engine.pushState();
  if (keys.length) engine.checkNow();
}
...
function sessionExit(engine, id) {
  ...
  if (!s) return { ok: true };
  handleSessionExit(engine, { keys: s.keys || [], label: s.label, id: s.id, code: 0, login: s.login === true });
  return { ok: true };
}

function spawnConsole(engine, slash, label, keys = [], account) {
  if (IS_LINUX) return engine.spawnConsoleLinux(slash, label, keys, account);
...
function buildLoginScriptMac(engine, dir, id, antes = []) {
  const stub = env.reviewCmdStub();
  const skip = engine.config.skipPermissions ? ' --dangerously-skip-permissions' : '';
  const claudeLine = stub || `claude${skip}`;
  return [
    ...
    'trap notify EXIT',
    'export GH_PAGER=cat PAGER=cat',
    // mesma promessa do loginConsoleEnv do Windows: console de login SEM GH_TOKEN.
    // O Terminal.app sourceia o profile do usuário antes deste script, então um
    // export GH_TOKEN perdido lá entraria herdado; o unset devolve o gh ao keyring.
    'unset GH_TOKEN',
    ...
    ...antes, ...claudeAuthShellLines({ kind: 'dir', dir }, false),
    claudeLine,
    'echo',
    'echo " [Farol] Sessao encerrada. Pode fechar esta janela."'
  ].join('\n') + '\n';
...
function loginConsoleEnv(dir) {
  const env = baseLoginConsoleEnv();
  applyClaudeAuthEnv(env, { kind: 'dir', dir: dir || '' });
  return env;
}

function codexLoginConsoleEnv() {
  const out = baseLoginConsoleEnv();
  applyClaudeAuthEnv(out, { kind: 'codex' });
  return out;
}

function buildCodexLoginScript(engine) {
  void engine;
  return [
    '@echo off',
    'chcp 65001>nul',
    'title Farol - login do Codex',
    `cd /d "${WORKSPACE}"`,
    'where codex>nul 2>nul',
    'if errorlevel 1 (',
    '  echo [Farol] Codex CLI nao encontrado no PATH deste processo.',
    '  echo [Farol] Instale ou reabra o Farol depois de instalar o Codex.',
    ') else (',
    '  codex login',
    '  echo.',
    '  codex login status',
...
function buildCodexLoginScriptMac(engine, id) {
  return [
    '#!/bin/bash',
    '# Farol: sessao SO de login do Codex. Este arquivo se apaga ao terminar.',
    `cd '${WORKSPACE}' || exit 1`,
    'notify() {',
    `  curl -fsS -m 5 -X POST -H 'x-farol: 1' -H 'Content-Type: application/json' \\`,
    `    --data '{"id":"${id}"}' 'http://127.0.0.1:${engine.config.port || DEFAULT_PORT}/api/session-exit' >/dev/null 2>&1`,
    ...
    'trap notify EXIT',
    'export GH_PAGER=cat PAGER=cat',
    'unset GH_TOKEN OPENAI_API_KEY CODEX_API_KEY',
...
function spawnLoginConsole(engine, dir) {
  if (IS_LINUX) return engine.spawnLoginConsoleLinux(dir);
  ...
  const id = `t${++engine.sessionSeq}`;
  engine.activeReviews.set(id, { id, keys: [], label, mode: 'terminal', startedAt: Date.now() });
  const cleanup = () => { try { fs.unlinkSync(script); } catch (err) { void err; } };
  child.on('exit', (code) => {
    cleanup();
    engine.activeReviews.delete(id);
    ...
    // o selo do perfil (A2) muda assim que o login termina, como no Codex
    engine.doctor().catch(err => { void err; });
    engine.pushState();
  });
  child.on('error', (err) => {
    cleanup();
...
function spawnCodexLoginConsole(engine) {
  if (IS_LINUX) return engine.spawnCodexLoginConsoleLinux();
  if (!IS_WIN) return engine.spawnCodexLoginConsoleMac();
  const sessionsDir = path.join(HOME, 'sessions');
  ensureDir(sessionsDir);
  const script = path.join(sessionsDir, `login-codex-${Date.now()}.cmd`);
  fs.writeFileSync(script, engine.buildCodexLoginScript());
  const label = 'Login do Codex';
  const ps = `$p = Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','"${script}"' ` +
    `-WorkingDirectory '${WORKSPACE}' -PassThru -Wait; exit $p.ExitCode`;
  logSpawn('spawnCodexLoginConsole(terminal-visivel)', ['cmd.exe', script]);
  const child = cp.spawn('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', ps], {
    cwd: WORKSPACE,
    env: codexLoginConsoleEnv(),
    stdio: 'ignore',
    windowsHide: true
  });
  const id = `t${++engine.sessionSeq}`;
...
function spawnLoginConsoleMac(engine, dir) {
  return spawnLoginConsolePosix(engine, dir, { cmd: 'open', args: ['-a', 'Terminal'], nome: 'Terminal.app' });
}

function spawnLoginConsoleLinux(engine, dir) {
  const term = pickLinuxTerminal(LINUX_TERMINALS, commandOnPath);
  // sem terminal: no celular devolve o comando para o Termux, no desktop avisa (login-celular.js)
  if (!term) return loginCelular.semTerminal(engine, dir, buildLoginScriptMac);
  return spawnLoginConsolePosix(engine, dir, { cmd: term.cmd, args: term.args, nome: term.cmd });
}

function spawnCodexLoginConsoleMac(engine) {
  return spawnCodexLoginConsolePosix(engine, { cmd: 'open', args: ['-a', 'Terminal'], nome: 'Terminal.app' });
}

function spawnCodexLoginConsoleLinux(engine) {
  const term = pickLinuxTerminal(LINUX_TERMINALS, commandOnPath);
  if (!term) {
    engine.log('ERROR', 'login do Codex: nenhum emulador de terminal encontrado (x-terminal-emulator/gnome-terminal/konsole/xterm)');
  ...
  return spawnCodexLoginConsolePosix(engine, { cmd: term.cmd, args: term.args, nome: term.cmd });
}

function spawnCodexLoginConsolePosix(engine, launcher) {
  const sessionsDir = path.join(HOME, 'sessions');
  ensureDir(sessionsDir);
  const id = `t${++engine.sessionSeq}`;
  const script = path.join(sessionsDir, `login-codex-${Date.now()}.command`);
  fs.writeFileSync(script, engine.buildCodexLoginScriptMac(id), { mode: 0o700 });
  const label = 'Login do Codex';
  engine.activeReviews.set(id, { id, keys: [], label, mode: 'terminal', startedAt: Date.now() });
  logSpawn(`spawnCodexLoginConsole(${launcher.nome})`, [launcher.cmd, ...launcher.args, script]);
  const child = cp.spawn(launcher.cmd, [...launcher.args, script], { stdio: 'ignore' });
  child.on('error', (err) => {
    try { fs.unlinkSync(script); } catch (e) { void e; }
    engine.activeReviews.delete(id);
...
function spawnLoginConsolePosix(engine, dir, launcher) {
  const sessionsDir = path.join(HOME, 'sessions');
  ensureDir(sessionsDir);
  const id = `t${++engine.sessionSeq}`;
  const script = path.join(sessionsDir, `login-${Date.now()}.command`);
  // 0o700 como a sessão de fila: o script pode carregar ANTHROPIC_API_KEY em
  // claro (perfil de chave) e 0o755 dava leitura a qualquer usuário da máquina
  fs.writeFileSync(script, engine.buildLoginScriptMac(dir, id), { mode: 0o700 });
  const label = 'Login do Claude';
  engine.activeReviews.set(id, { id, keys: [], label, mode: 'terminal', startedAt: Date.now(), login: true });
  logSpawn(`spawnLoginConsole(${launcher.nome})`, [launcher.cmd, ...launcher.args, script]);
  const child = cp.spawn(launcher.cmd, [...launcher.args, script], { stdio: 'ignore' });
  child.on('error', (err) => {
    try { fs.unlinkSync(script); } catch { }
    engine.activeReviews.delete(id);
...
function pushActivity(engine, id, kind, text, agent, stage) {
  const list = engine.activity.get(id);
  if (!list) return;
  const item = { t: Date.now(), k: kind, text: String(text || '').slice(0, 400) };
  // `a` = rótulo do subagente dono da linha (fan-out de leitura ou verificação);
  // ausente = linha da sessão principal, o caso de sempre
  if (agent) item.a = String(agent).slice(0, 40);
  // `s` = etapa da revisão (stageOfLine, review.js), estampada pelo chamador: é o
  // que move a esteira de etapas do card na UI e o resumo final por etapa
  if (stage) item.s = String(stage).slice(0, 20);
  list.push(item);
  if (list.length > 120) list.splice(0, list.length - 120);
  engine.emit('activity', { id, item });
}

/* ---------- rastreio de subagentes da sessão (visível na UI) ----------
   O stream do CLI marca evento vindo de DENTRO de um subagente com
   parent_tool_use_id; o registro do agente nasce quando a sessão PRINCIPAL
   dispara o tool_use de Task, e morre quando o tool_result desse Task volta.
   O rótulo viaja nas linhas do feed (item.a) e a contagem vai no snapshot
   (projectSessions), pra UI mostrar quantos agentes trabalham e o que cada
   um roda. Sessão sem registro em activeReviews (chat, ferramentas) não
   rastreia nada, de propósito. */

// A ferramenta de disparo mudou de nome entre versões do CLI: era Task, o 2.1.x
// chama Agent (visto num feed real em 17/08/2026, que deixou o badge mudo). As
// duas contam; nome novo desconhecido degrada pro fallback "agente" nas linhas,
// nunca pra linha sem etiqueta.
const AGENT_TOOL_NAMES = new Set(['Task', 'Agent']);

function registrarAgenteDeTask(engine, opts, block) {
  const review = engine.activeReviews && engine.activeReviews.get(opts.id);
  if (!review || !AGENT_TOOL_NAMES.has(block.name) || !block.id) return;
  review.agents = review.agents || {};
  if (review.agents[block.id]) return;
  const n = Object.keys(review.agents).length + 1;
  const tipo = String((block.input && block.input.subagent_type) || 'agente');
  review.agents[block.id] = {
    n,
    label: `${tipo} ${n}`,
    desc: String((block.input && block.input.description) || '').slice(0, 120),
    done: false,
    startedAt: Date.now(),
  };
  engine.pushState();
...
function rotuloDoAgente(engine, opts, parentId) {
  if (!parentId) return '';
  const review = engine.activeReviews && engine.activeReviews.get(opts.id);
  const a = review && review.agents && review.agents[parentId];
  // evento de subagente que não nasceu de um Task visto (ex.: corte do feed):
  // ainda é de agente, só sem número; nunca deixa a linha passar como principal
  return a ? a.label : 'agente';
}

// tool_result de um Task registrado, vindo da sessão principal = o subagente
// terminou (com sucesso ou erro, tanto faz pra contagem de "trabalhando agora")
function concluirAgentesDoEvento(engine, opts, ev) {
  if (ev.parent_tool_use_id) return; // resultado interno DE um subagente, não o fim dele
  const review = engine.activeReviews && engine.activeReviews.get(opts.id);
  if (!review || !review.agents || !ev.message || !Array.isArray(ev.message.content)) return;
  let mudou = false;
  for (const block of ev.message.content) {
    const a = block && block.type === 'tool_result' && review.agents[block.tool_use_id];
    if (a && !a.done) { a.done = true; mudou = true; }
  }
  if (mudou) engine.pushState();
}
...
function projectSessions(records) {
  return (records || []).map((s) => {
    ...
    void fileBlobs; void filesRead; void scopeRoot;
...
function toolSummary(engine, name, input) {
  ...
  if (AGENT_TOOL_NAMES.has(name)) return input.description || String(input.prompt || '').slice(0, 160);
...
function buildModelFlags(config, opts = {}) {
  if (opts.stub) return '';
  const cfg = config || {};
  const modelConfigurado = sanitizeClaudeModel(cfg.reviewModel) || '';
  const temOverrideEsforco = opts.effort != null;
  // opts.model / opts.effort: override por revisão (roteador auto). Ausente = config.
  let model = sanitizeClaudeModel(opts.model != null ? opts.model : modelConfigurado) || '';
  // auto é do Farol, não do CLI: sem override do roteador, não manda flag
  // (sessões que não passam pelo router, como pushback, herdam o padrão do plano).
  if (model === 'auto') model = '';
  let effort = sanitizeClaudeEffort(opts.effort != null ? opts.effort : cfg.reviewEffort) || '';
  // Auto controla modelo e esforço como um par. Fora da revisão roteada não há
  // métricas nem override explícito; nesse caso ambos ficam no padrão do CLI,
  // conforme a interface promete, em vez de vazar o reviewEffort fixo.
  if (modelConfigurado === 'auto' && !temOverrideEsforco) effort = '';
  // modo rápido (opts.fast, só o caminho de REVISÃO passa): o prompt corta as
  // etapas, mas o relógio de verdade é o raciocínio entre comandos, e isso só a
  // flag --effort derruba (medido em 17/08/2026, #776: com fast e effort default
  // a "leitura" ainda levou 6m28s, quase tudo pensamento pré-comando). Regra
  // determinística: fast usa 'medium', salvo 'low' explícito do usuário (que é
  // ainda mais rápido e fica). Valor fixo de allowlist, nunca texto livre.
  if (opts.fast) effort = (effort === 'low') ? 'low' : 'medium';
  effort = effortForModel(model, effort);
  return (model ? ` --model ${model}` : '') + (effort ? ` --effort ${effort}` : '');
}

// registra o checkpoint de verificação sinalizado por um Bash da sessão headless
// (ver "Checkpoint de verificação" no docs/REVIEW-GATES.md). Extraído do handleEvent pra
// achatar o fluxo; cada if aninhado do original virou guard clause aqui.
function registrarCheckpointDeBash(engine, opts, block) {
  ...
  try {
    ...
    // A propriedade que habilita a captura NAO e o modo: e a sessao ter sido lancada
    // pelo engine COM o protocolo de checkpoint no prompt e um PR dono. `checkpoint`
    // carrega a LOJA que ela alimenta ('review' | 'self'); ausente = nao participa
    // (terminal, chat, ferramenta). Voltar pra `mode !== 'auto'` reabriria o furo do
    // P0b: a autoanalise nunca produzia verificacao capturavel, e era justamente ela
    // que autorizava merge.
    if (!prKey || !review.checkpoint) return;
    const prUrl = review.pr.url;
    appendCheckpointEntry(checkpointPath(prKey, review.checkpoint), prKey, prUrl || '', {
      ...
      // blob do arquivo citado no momento da verificação (mapa preenchido por
      // runHeadlessReview via fetchPrFiles): permite a entrada continuar relevante
      // num head futuro enquanto o arquivo não mudar (ver relevantEntries).
      blobSha: (review && review.fileBlobs && parsed.file && review.fileBlobs[String(parsed.file)]) || '',
...
// NUMERADOR da cobertura: registra que a sessao ABRIU um arquivo do escopo do PR.
// Vale tambem pra subagente (o fan-out le nos subagentes; exigir a sessao principal
// zeraria a cobertura de PR grande), e o Set deduplica leitura repetida sozinho.
// So conta caminho DENTRO da raiz materializada, entao Read de arquivo qualquer do
// workspace nunca vira cobertura. Ver lib/engine/pr-scope.js.
function observarLeitura(engine, opts, block) {
  if (block.name !== 'Read') return;
  const review = engine.activeReviews && engine.activeReviews.get(opts.id);
  if (!review || !review.scopeRoot) return;
  const prPath = prPathFromRead(review.scopeRoot, block.input && block.input.file_path);
  if (!prPath) return;
  if (!review.filesRead) review.filesRead = new Set();
  review.filesRead.add(prPath);
}

// processa um bloco de content de evento "assistant" do stream headless: texto
// vira linha do feed, tool_use vira resumo de ferramenta + captura passiva do
// checkpoint quando é Bash. Extraído do handleEvent pra achatar o fluxo (era
// profundidade 6 dentro do for; contrato: máximo 3).
function processarBlocoDeConteudo(engine, opts, onEvent, block, agente) {
  if (block.type === 'text') {
    if (String(block.text || '').trim()) onEvent({ kind: 'text', text: block.text, agent: agente || undefined });
    return;
  }
  if (block.type !== 'tool_use') return;
  if (!agente) registrarAgenteDeTask(engine, opts, block);
  const sum = engine.toolSummary(block.name, block.input);
  onEvent({ kind: 'tool', text: sum ? `${block.name} · ${sum}` : block.name, agent: agente || undefined });
  observarLeitura(engine, opts, block);
  // captura do checkpoint: SÓ blocos da sessão PRINCIPAL. O protocolo manda a
  // principal REEMITIR o veredito dos subagentes (prompts/pr-review-auto.md);
  // capturar os dois lados duplicaria entradas no append-only.
  if (!agente) registrarCheckpointDeBash(engine, opts, block);
}

// Pura, exportada pra teste. Acrescenta ao fim da mensagem de erro quantas
// reconexões (eventos system/api_retry) a sessão sofreu antes de morrer: é o
// texto que a taxonomia (lib/log-taxonomy.js, classe 'rede') passa a casar, no
// lugar de cair em 'desconhecido' -> permanente.
function comReconexoes(msg, n) {
  if (!n) return msg;
...
function runClaudeStream(engine, prompt, opts = {}) {
  ...
  return admitirOperacao(engine, opts).then((admissao) => {
    ...
    return avisarAdmissao(opts, handle).then(() => runProvedor(engine, prompt, opts)).then(
      ...
      // o .catch no abort é o ponto: a sessão JÁ falhou, e uma falha ao devolver o lease
      // (rede caindo junto) não pode substituir o erro real, que é o que o retry e a
      // taxonomia leem. O lease que não sair agora expira sozinho pelo TTL.
      (err) => { const fim = handle ? Promise.resolve(handle.abort()).catch(() => undefined) : Promise.resolve(); return fim.then(() => { if (handle && handle.lost) err.coordenacao = 'perdido'; throw err; }); }
    );
  });
}
...
// Sessão de LEITURA (A3): o diagnóstico com IA lê o app e o log, e não muda nada. O
// --dangerously-skip-permissions do headless é fixo, então a restrição tem que vir da
// lista de ferramentas; sem MCP junto, porque servidor de MCP é escrita por outro caminho.
// `--tools` só tira ferramenta do contexto do modelo: hooks de settings (usuário, projeto do
// workspace, plugins) rodam mesmo assim, e nem ele nem o skip-permissions os afetam
// (verificação B, 16/09/2026, `claude --help` da 2.1.268 e documentação de hooks). Por isso
// `--safe-mode` (sem hooks, plugins, skills, MCP, agentes customizados, CLAUDE.md e memória;
// autenticação e ferramentas embutidas seguem) e `--disable-slash-commands` (sem skills).
// Hook de política gerenciada continua valendo: é da organização, não desta sessão.
const FLAGS_SOMENTE_LEITURA = Object.freeze(['--tools', 'Read,Grep,Glob', '--strict-mcp-config', '--safe-mode', '--disable-slash-commands']);
function argsDeSomenteLeitura(opts) {
  return (opts && opts.somenteLeitura) ? [...FLAGS_SOMENTE_LEITURA] : [];
}
...
function runProvedor(engine, prompt, opts = {}) {
  ...
  if (auth.kind === 'codex') {
    return runCodexStream(engine, prompt, { ...opts, authProfileId: auth.id || '' });
  }
  const base = (stub || 'claude -p --output-format stream-json --verbose --dangerously-skip-permissions')
    + buildModelFlags(engine.config, {
      stub: !!stub,
      fast: !!opts.fast,
      model: opts.model,
      effort: opts.effort,
    });
  const extra = [...argsDeSomenteLeitura(opts), ...(opts.extraArgs || [])].join(' ');
  ...
  return new Promise((resolve, reject) => {
    ...
    // resolvido uma vez só: qual PERFIL atende esta conta, de chave de API ou de
    // assinatura. Até a v2.48.3 o id de perfil de assinatura era descartado aqui, e
    // isso sozinho tornava impossível qualquer teto pra quem roda por assinatura: o
    // gasto nunca era atribuído, então byProfileDay ficava vazio e profileSpend
    // devolvia zero pra sempre. Agora os dois tipos são atribuídos; o legado (sem
    // perfil nenhum configurado) segue com id vazio, porque não há a quem atribuir.
    ...
    const child = IS_WIN
      ? cp.spawn('cmd.exe', ['/d', '/s', '/c', `"${cmdline}"`], {
        cwd: WORKSPACE,
        env,
      ...
      : cp.spawn('/bin/sh', ['-lc', claudeAuthPosixPrefix(auth) + cmdline], {
        cwd: WORKSPACE,
        env,
        stdio: ['pipe', 'pipe', 'pipe'],
        detached: true
    ...
    // conta os eventos system/api_retry do stream: quando a sessão morre depois de
    // reconexões, a mensagem de erro final ganha o sufixo (comReconexoes), e o
    // painel de Diagnóstico casa esse texto com a classe 'rede' da taxonomia em vez
    // de 'desconhecido' -> permanente.
    let reconexoes = 0;
    // Tokens acumulados MENSAGEM A MENSAGEM. Existe porque o registro de consumo
    // dependia só do evento final, e sessão morta no meio (kill do cancelamento,
    // timeout) não gerava registro nenhum: o gasto sumia da tela e do teto de
    // orçamento. Cada evento `assistant` do stream carrega o `usage` daquela chamada
    // (conferido no transcript real de uma sessão do Farol), e a soma delas é o que a
    // sessão gastou até morrer. Só é usado quando o evento final NÃO chega: quando ele
    // chega, ele já traz o total da sessão, e somar os dois contaria duas vezes.
    const parcial = { input_tokens: 0, output_tokens: 0, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 };
    ...
    const finish = (err, value) => {
      ...
      if (err) {
        // um lugar só pros 3 caminhos de erro (result is_error, exit code != 0,
        // timeout): sufixo de reconexão pra taxonomia casar em 'rede', e sessionId
        // pra Task 3 saber que sessão retomar.
        if (reconexoes > 0) err.message = comReconexoes(err.message, reconexoes);
    ...
    const handleEvent = (ev) => {
      ...
      if (ev.session_id && !sessionId) {
        ...
        if (opts.onSession) opts.onSession(sessionId);
      }
      if (ev.type === 'system' && ev.subtype === 'init') {
        ...
        if (acumularParcial(parcial, ev.message.usage, ev.message.id)) houveParcial = anotarParcial(engine, attemptId, parcial, usedModel, ev.message);
        // evento de dentro de subagente vem com parent_tool_use_id: a linha do
        // feed carrega o rótulo dele (visível na UI), e o checkpoint não captura
        const agente = rotuloDoAgente(engine, opts, ev.parent_tool_use_id);
        for (const block of ev.message.content) {
          processarBlocoDeConteudo(engine, opts, onEvent, block, agente);
        }
      } else if (ev.type === 'user') {
        concluirAgentesDoEvento(engine, opts, ev);
    ...
    child.on('error', (e) => { fecharTentativa(engine, attemptId); finish(e); });
    child.on('close', (code) => {
...
/* Fecha a conta de consumo da sessão. Fora do handler de close de propósito: ali
   dentro ele passava do teto de profundidade do gate, e o que ele decide não é fluxo
   de stream, é contabilidade.

   Duas fontes, nunca as duas juntas: o evento FINAL traz o total da sessão e o custo
   medido; o acumulado por mensagem só vale quando esse evento não chegou, e somar os
   dois contaria o mesmo gasto duas vezes. Sem evento final e sem token acumulado não
   há o que registrar, e aí a sessão morreu sem gastar nada. */
...
// Soma o usage de UMA mensagem no acumulador da sessão. Devolve se somou alguma coisa,
// que é o que distingue "sessão morreu sem gastar nada" de "gastou e morreu". PURA
// (muta o acumulador que recebe, e é só ele). Campo ausente vale zero: mensagem sem
// usage não pode zerar o que já foi contado, nem inventar token.
const CAMPOS_USO = ['input_tokens', 'output_tokens', 'cache_read_input_tokens', 'cache_creation_input_tokens'];

// Soma o uso de UMA mensagem no acumulador da sessão. MEDIDO em três sessões reais do CLI
// (A1, item 1, 16/09/2026): cada bloco de conteúdo chega como evento separado, com o MESMO
// message.id e o MESMO uso, e somar tudo dobrava entrada e cache. Cada id conta uma vez,
// com o último uso visto. A saída desses eventos é um PISO (o total só chega no evento
// final), por isso a linha parcial segue estimada. Mensagem sem id continua somando: sem id
// não há como deduplicar. O mapa por id não é enumerável e nunca viaja no diário.
function acumularParcial(acc, usage, messageId) {
  ...
  if (typeof messageId === 'string' && messageId) descontarAnterior(acc, messageId, valores);
  for (const campo of CAMPOS_USO) acc[campo] += valores[campo];
  return true;
}

function descontarAnterior(acc, messageId, valores) {
...
// Um único bloco explicitamente JSON delimita o resultado, mesmo se a prosa usa
// chaves (ex.: {{N}}). Não escolhe entre blocos nem repara JSON inválido: ambos
// continuam falhas de contrato, sem buscar uma decisão alternativa no texto.
function parseHeadlessJson(text) {
  try { return jsonDaSessao(text); } catch { /* texto com prosa ou cerca Markdown */ }
  const blocos = [...text.matchAll(/^[ \t]*(`{3,}|~{3,})json[ \t]*\r?$/gim)];
  if (blocos.length > 1) throw new Error('JSON da sessão ambíguo: múltiplos blocos json');
  let trecho;
  if (blocos.length === 1) {
    const bloco = blocos[0];
    const resto = text.slice(bloco.index + bloco[0].length);
    const fim = new RegExp('^[ \\t]*' + bloco[1][0] + '{' + bloco[1].length + ',}[ \\t]*\\r?$', 'm').exec(resto);
    if (!fim) throw new Error('JSON da sessão com bloco json não encerrado');
    trecho = resto.slice(0, fim.index);
  } else {
    trecho = jsonSemCerca(text);
  }
  return jsonDaSessao(trecho);
}

function jsonDaSessao(text) {
  try { return JSON.parse(text); }
  catch { throw new SyntaxError('JSON da sessão inválido'); }
}

// Compatibilidade com respostas antigas que cercam o objeto apenas com prosa.
function jsonSemCerca(text) {
  const a = text.indexOf('{'), b = text.lastIndexOf('}');
  if (a < 0) throw Object.assign(new Error(semJsonText(text)), { code: 'FAROL_RESULT_MISSING' });
  if (b <= a) throw new SyntaxError('JSON da sessão inválido');
  return text.slice(a, b + 1);
}

function parseHeadlessResult(engine, raw) {
  const data = parseHeadlessJson(engine.parseEnvelope(raw));
  if (!data || !data.decision || !data.payloads || !data.reportMarkdown) throw new Error('JSON da sessão fora do contrato');
...
const sessionMod = {
  buildSessionScript, buildSessionScriptMac, spawnConsoleMac, spawnConsoleLinux, sessionExit, spawnConsole,
  handleSessionExit, pickLinuxTerminal, LINUX_TERMINALS,
  buildLoginScript, buildLoginScriptMac, buildCodexLoginScript, buildCodexLoginScriptMac,
  loginConsoleEnv, codexLoginConsoleEnv, spawnLoginConsole, spawnLoginConsoleMac, spawnLoginConsoleLinux,
  ...
  runClaudeStream, runProvedor, argsDeSomenteLeitura, OPERACOES, parseEnvelope, parseHeadlessResult, buildModelFlags,
