...
function processarBlocoDeConteudo(engine, opts, onEvent, block) {
  if (block.type === 'text') {
    if (String(block.text || '').trim()) onEvent({ kind: 'text', text: block.text });
    return;
  }
  if (block.type !== 'tool_use') return;
  const sum = engine.toolSummary(block.name, block.input);
  onEvent({ kind: 'tool', text: sum ? `${block.name} · ${sum}` : block.name });
  registrarCheckpointDeBash(engine, opts, block);
}
    ...
    const handleEvent = (ev) => {
      ...
      if (ev.type === 'system' && ev.subtype === 'init') {
        ...
        for (const block of ev.message.content) {
          processarBlocoDeConteudo(engine, opts, onEvent, block);
        }
