  ...
  return new Promise((resolve, reject) => {
    ...
    const handleEvent = (ev) => {
      ...
      if (ev.type === 'system' && ev.subtype === 'init') {
        ...
        if (acumularParcial(parcial, ev.message.usage)) houveParcial = true;
    ...
    child.on('close', (code) => {
      ...
      registrarConsumo(engine, opts, { resultEvent, cancelada: run.cancelled, parcial, houveParcial, usedModel, authProfileId });
      if (run.cancelled) {
...
function registrarConsumo(engine, opts, ctx) {
  const { resultEvent, cancelada, parcial, houveParcial, usedModel, authProfileId } = ctx;
  let ev = null;
...
function acumularParcial(acc, usage) {
  ...
  for (const campo of ['input_tokens', 'output_tokens', 'cache_read_input_tokens', 'cache_creation_input_tokens']) {
    const n = Number(usage[campo]) || 0;
    if (n > 0) { acc[campo] += n; somou = true; }
  }
  return somou;
}
