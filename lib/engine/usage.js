...
function usageSummary(engine) {
  ...
  return {
    ...
    last7: sumDaysSince(store.days, from7),
...
// zera o registro (botão "limpar" no painel)
function clearUsage(engine) {
  engine.usage = defaultUsage();
  saveUsage(engine);
  engine.pushState();
  return { ok: true };
}
...
module.exports = {
  ...
  recordUsage, saveUsage, usageSummary, clearUsage,
};
