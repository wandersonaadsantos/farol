...
// `sem-base`: o token continua registrado (ele é medido) e o custo fica declarado como
// desconhecido, em vez de virar um número que ninguém pode auditar. PURA.
function estimarCusto(sessions, kind, model, outputTokens) {
  const taxa = taxaObservada(sessions, kind, model);
  if (taxa === null || !(Number(outputTokens) > 0)) return { costUsd: 0, source: 'sem-base' };
...
// resultado x quanto foi perdido, e quanto do total é medido x estimado. PURA.
// Registro antigo não tem `costSource`, e conta como MEDIDO, que é o que ele era.
function auditoriaDeConsumo(sessions) {
  const zero = () => ({ sessions: 0, costUsd: 0 });
  const r = { total: zero(), util: zero(), perdido: zero(), medido: zero(), estimado: zero() };
  for (const s of sessions || []) {
    ...
    soma(s.costSource === 'estimado' ? r.estimado : r.medido);
  }
  return r;
}
...
function recordUsage(engine, id, account, resultEvent, model, profileId, ref) {
  ...
  engine.usageSessions.sessions.push({
    ...
    status: parcial ? 'parcial'
      : (resultEvent && resultEvent.farol_cancelled) ? 'cancelada'
...
const usageMod = {
  ...
  accountSpendInProfile, quotaStatusFor, pesoDaConta,
  dailyCapFor, dailyCapSource, sugestaoTetoDiario, mediana,
  taxaObservada, estimarCusto, auditoriaDeConsumo, marcarDesfecho, DESFECHOS,
};
