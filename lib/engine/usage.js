...
function profileBudgetStatus(profile, store, tipico = 0) {
  ...
  return veredito('diario', spend.today, profile.budgetDaily)
...
function budgetsFrom(store, profiles, tipico = 0) {
  return (profiles || []).map(p => {
    const st = profileBudgetStatus(p, store, tipico);
    return {
      id: p.id, label: p.label, kind: p.kind === 'apikey' ? 'apikey' : 'assinatura',
...
function usageSummary(engine) {
  ...
  return {
    ...
    budgets: budgetsFrom(store, engine.config && engine.config.claudeProfiles, custoTipicoDeReview(sessions)),
