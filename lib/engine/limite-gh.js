...
// A hora do reset pela boca do proprio GitHub. O endpoint rate_limit e documentado como
// nao contando contra o limite, entao perguntar nao piora a condicao; e a busca ja
// falhou, entao nao ha o que atrasar. Falha ou resposta sem numero devolve 0, e a
// espera padrao assume.
async function resetDoGh(engine, user) {
  const r = await io.run('gh', ['api', 'rate_limit', '--jq', '.resources.graphql.reset'],
    { env: engine.ghEnv(user) });
  if (!r.ok) return 0;
  const seg = Number(String(r.stdout || '').trim());
  return Number.isFinite(seg) && seg > 0 ? seg * 1000 : 0;
}
...
async function entrarEmEspera(engine, user, agora = Date.now()) {
  const ate = esperaAte(await resetDoGh(engine, user), agora);
...
async function tratarFalhaDeBusca(engine, user, stderr, agora = Date.now()) {
  ...
  await entrarEmEspera(engine, user, agora);
  return true;
}

export default { ehLimite, esperaAte, limiteAte, emEspera, resetDoGh, entrarEmEspera, tratarFalhaDeBusca };
export { ehLimite, esperaAte, limiteAte, emEspera, resetDoGh, entrarEmEspera, tratarFalhaDeBusca };
