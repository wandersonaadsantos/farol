...
async function sincronizarHistorico(engine, cfg, { agora = Date.now() } = {}) {
  ...
  for (const d of candidatas(engine, desde)) {
    ...
    const ui = typeof engine.decisionForUi === 'function' ? engine.decisionForUi(d) : d;
    const corpoOk = await publicarCorpo(rt, reviewId, versao, historico.corpoDe(ui));
    const indiceOk = corpoOk && await publicarIndice(rt, reviewId, d, historico.indiceDe(d, { kId }));
    if (!indiceOk) continue;
...
export default { sincronizarHistorico, lerRecentes, abrirRevisao, LIMITE_POR_CICLO };
export { sincronizarHistorico, lerRecentes, abrirRevisao, LIMITE_POR_CICLO };
