...
// `naoAbriram` conta as linhas que não decifraram: elas ficam fora da lista (nunca pela
// metade), e a tela diz quantas, para a lista não parecer completa. Falha da leitura é `null`.
async function lerRecentesContadas(engine, { dev = '', desdeT = 0 } = {}) {
  const rt = engine.sync;
  ...
  if (!objeto(r.data)) return { revisoes: [], naoAbriram: 0 };
  const abertas = Object.entries(r.data).map(([id, no]) => abrirIndice(rt, id, no));
  const lista = abertas.filter(Boolean);
  return { revisoes: historico.ordenar(lista), naoAbriram: abertas.length - lista.length };
}

// A leitura que a tela usa: a contagem e, em cada revisão, o PR resolvido pelo catálogo
// cifrado a partir da tag do índice (`pr` nulo quando o catálogo não abre).
async function recentesParaTela(engine, cfg, opcoes = {}) {
  const r = await lerRecentesContadas(engine, opcoes);
  if (!r) return null;
  const revisoes = [];
  for (const item of r.revisoes) {
    revisoes.push({ ...item, pr: item.prTag ? await publicacao.prDaTag(engine, cfg, item.prTag) : null });
  }
  return { ...r, revisoes };
}

async function lerRecentes(engine, opcoes = {}) {
  const r = await lerRecentesContadas(engine, opcoes);
  return r ? r.revisoes : null;
}
...
export default { LIMITE_LEITURA, sincronizarHistorico, lerRecentes, lerRecentesContadas, recentesParaTela, abrirRevisao, publicarUma, bytesDe, marcoDe, LIMITE_POR_CICLO };
export { LIMITE_LEITURA, sincronizarHistorico, lerRecentes, lerRecentesContadas, recentesParaTela, abrirRevisao, publicarUma, bytesDe, marcoDe, LIMITE_POR_CICLO };
