...
async function destinosDaTransferencia(engine, cfg, { dono, acctTag: tagDaConta, agora } = {}) {
  const rt = engine.sync;
  if (!sharedActive(cfg)) return recusa('compartilhamento-desligado', 'o compartilhamento cifrado está desligado');
  if (!rt || !rt.client || !rt.uid || !rt.material) return recusa(SYNC_CODES.SEM_CREDENCIAL, 'este aparelho não está conectado');
  const origem = String(dono || '').trim();
  if (!origem) return recusa('forma', 'falta dizer qual aparelho roda a análise');
  ...
  const base = { dono: origem, acctTag: TAG_RE.test(String(tagDaConta || '')) ? String(tagDaConta) : '', agora: instante };
  ...
  const motivoOrigem = transferencia.motivoDaOrigem(resumoDaCapacidade(rt, origem, status[origem]), { agora: instante });
  return { ok: true, destinos, origem: { deviceId: origem, motivo: motivoOrigem } };
}
