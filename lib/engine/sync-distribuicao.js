...
import fechamento, { NO_FILA, NO_ATRIBUICAO } from './sync-fechamento.js';
import { prPorTag, headAtual } from './pr-por-tag.js';
...
// Os quatro últimos são os códigos de AUTORIDADE de `atribuicaoValida`. Sem eles na lista,
// `daLista` apagava o detalhe e o publicador via `inapto` pelado: recusa sem motivo legível
// é o que faz o agendador repetir a mesma atribuição sem ninguém entender por quê.
const DETALHES = ['presenca-vencida', 'root', 'provedor-nao-pronto', 'pausado', 'memoria-desconhecida', 'memoria-insuficiente', 'sem-vaga', 'tipo-desconhecido', 'grupo-nao-verificavel', 'nao-publiquei', 'nao-aceita-admin', 'vencida', 'geracao', 'assinatura'];
...
function anotarEspera(rt, itemId, motivo, agora, detalhe = {}) {
  ...
  rt.motivosDaEspera.set(itemId, {
    ...
    // `papel` diz de quem e o `dev`: 'escolhido', 'recusou' ou vazio (nao se sabe daqui)
    dev: String(detalhe.dev || ''), papel: String(detalhe.papel || ''),
    aparelhos: Array.isArray(detalhe.aparelhos) ? detalhe.aparelhos : [],
  });
}
...
async function cicloDoAgendador(engine, cfg, { agora = Date.now() } = {}) {
  ...
  if (r.item) {
    ...
    avaliados.push(desfechoDaAtribuicao(r.item.itemId, feito, r.dev));
    if (feito) {
  ...
  for (const a of avaliados) anotarEspera(rt, a.itemId, motivoDoAvaliado(a), agora, { dev: a.dev, papel: a.papel, aparelhos: a.aparelhos });
  ...
  await fechamento.faxinaDaFila(rt, { fila: fila.valor, atribuicoes: atribuicoes.valor, agora });
...
// O admin SABE quem ele escolheu: o item recém-atribuído leva o aparelho junto, com o
// papel dito. Sem isto, o card do publicador dizia "o distribuidor já escolheu um aparelho"
// no giro em que o próprio distribuidor acabara de nomear um.
function desfechoDaAtribuicao(itemId, feito, dev) {
  if (feito) return { itemId, desfecho: 'atribuido', motivo: '', dev: String(dev || ''), papel: 'escolhido' };
...
// CONSENTIMENTO ENTRA NA ELEGIBILIDADE (20/09/2026). O executor sempre conferiu
// `aceitarAdmin` ao receber (`atribuicaoValida`), e o agendador não olhava: com o padrão
// `false`, o admin atribuía trabalho a aparelhos que recusam por princípio, recebia a
// recusa e repetia o laço a cada ATRIBUICAO_TTL_MS, para sempre. O admin que também
// executa caía nisso consigo mesmo.
//
// O predicado é o MESMO da transferência (`lib/sync/transferencia.js`), que já exigia
// consentimento: duas leituras diferentes da mesma capacidade davam "tem vaga" num caminho
// e "sem consentimento" no outro, no mesmo instante.
//
// Campo AUSENTE não é campo falso, e aqui isso é declarado: aparelho em versão que ainda
// não publica `aceitarAdmin` continua elegível, como na transferência. Tratar ausência
// como recusa deixaria a frota antiga sem trabalho nenhum; ela segue conferindo o
// consentimento localmente ao receber, que é onde a decisão sempre morou. Resumo AUSENTE
// ou VELHO já não elege ninguém: quem barra é `ocup.temResumo`.
function semConsentimento(resumo) {
  return !!resumo && resumo.aceitarAdmin === false;
}
...
function estadoDoAparelho(rt, dev, no, { lista, agora }) {
  ...
  const recusaPorPrincipio = semConsentimento(resumo);
  const teto = resumo ? Math.max(1, Number(resumo.teto) || 1) : 1;
  return {
    apto: ocup.temResumo && !pausado && !recusaPorPrincipio,
    pausado, semConsentimento: recusaPorPrincipio, teto, ocupadas: ocup.total, prioridade: 0,
  };
}
...
function resumoDoAparelho(rt, dev, no) {
  ...
  return {
    ...
    // CRU de propósito: `true`, `false` e ausente são três coisas, e só `false` é recusa
    aceitarAdmin: c.aceitarAdmin,
  };
}
...
async function aceitarAtribuicoes(engine, cfg, arvore, { agora = Date.now() } = {}) {
  ...
  for (const [itemId, atribuicao] of Object.entries(mapaDe(arvore))) {
    ...
    aceitas.push({ itemId, pr: { ...meu.pr, itemIdDistribuido: itemId }, admissaoId: vaga.id });
  }
  for (const r of recusas) {
    anotarEspera(rt, r.itemId, r.code, agora, { dev: rt.deviceId, papel: 'recusou', aparelhos: [{ deviceId: rt.deviceId, motivo: r.problema || r.code }] });
...
// A TRANSFERÊNCIA para um aparelho que não publicou o candidato (7.C7b). Só publicador
// executa, e o agendador ignora a preferência por quem não publicou: medido na bancada com
// engines reais (17/09/2026), o comando voltava `aplicado` e o PR nunca chegava ao destino.
// O destino que conhece o PR e vê o MESMO head vira publicador, com a mesma preferência, e
// daí em diante o caminho é o de sempre. Head diferente é outro material, e não é adotado.
async function adotarUm(engine, cfg, kId, item, agora) {
  const pr = prPorTag(engine, kId, item.prTag);
  if (!pr) return '';
  const head = await headAtual(engine, { ...pr, headSha: pr.headSha || pr.knownHead || '' });
  if (!head || candidato.matTag(kId, head) !== item.matTag) return '';
  const r = await publicarCandidato(engine, cfg, { ...pr, headSha: head }, { agora, preferencia: { dev: item.prefDev, ate: item.prefAte } });
  return r.ok ? r.itemId : '';
}

async function adotarPreferidos(engine, cfg, { agora = Date.now() } = {}) {
  if (!ativa(engine, cfg)) return { adotados: [] };
  const rt = engine.sync;
  const lidoEm = Date.now();
  const fila = await publicar.lerNo(rt.client, `/users/${rt.uid}/${NO_FILA}`);
  const adotados = [];
  if (!fila) return { adotados, fila: null, lidoEm };
  const kId = kIdDe(rt);
  for (const item of fundirFila(engine, fila.valor, { agora }).itens) {
    if (preferenciaValida({ dev: item.prefDev, ate: item.prefAte }, { agora }) !== rt.deviceId) continue;
    // quem manda é o registro VIVO no banco: lembrar de uma adoção que já venceu deixava o
    // destino fora dos publicadores, e o item voltava para a origem
    if ((item.publicadores || []).includes(rt.deviceId)) continue;
    const adotado = await adotarUm(engine, cfg, kId, item, agora);
    if (adotado) adotados.push(adotado);
  }
  // fila vazia é `null` no banco, e é leitura válida: é exatamente quando o último item saiu
  return { adotados, fila: mapaDe(fila.valor), lidoEm };
}
...
async function cicloDaDistribuicao(engine, cfg, { agora = Date.now() } = {}) {
  ...
  const adocao = await adotarPreferidos(engine, cfg, { agora });
  if (adocao.fila) fechamento.esquecerConcluidos(engine, adocao.fila, { lidoEm: adocao.lidoEm });
  const atribuicoes = await publicar.lerNo(rt.client, `/users/${rt.uid}/${NO_ATRIBUICAO}`);
  ...
  if (atribuicoes) {
    for (const [itemId, e] of await espera.lerEsperas(engine, { atribuicoes: atribuicoes.valor, agora })) {
      anotarEspera(rt, itemId, e.motivo, e.at || agora, { dev: e.dev, papel: e.papel, aparelhos: e.aparelhos });
    }
  }
...
export default { ativa, distribuindo, semConsentimento, cicloDaDistribuicao, publicarProntidao, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, adotarPreferidos, responder, motivoDaEspera, esperaDoPr, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA, NO_PRONTIDAO };
export { ativa, distribuindo, semConsentimento, cicloDaDistribuicao, publicarProntidao, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, adotarPreferidos, responder, motivoDaEspera, esperaDoPr, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA, NO_PRONTIDAO };
