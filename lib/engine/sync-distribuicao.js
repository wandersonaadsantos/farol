...
async function aceitarAtribuicoes(engine, cfg, arvore, { agora = Date.now() } = {}) {
  ...
  for (const a of aceitas) await responder(engine, cfg, { itemId: a.itemId, estado: 'aceita', admissaoId: a.admissaoId, agora });
  return { aceitas, recusas };
}

export default { ativa, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, responder, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA };
export { ativa, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, responder, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA };
