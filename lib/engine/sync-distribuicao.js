...
function resumoDoAparelho(rt, dev, no) {
  ...
  const aberto = envelope.decifrar({
    enc: no.enc, material: rt.material, uid: rt.uid, caminho: `live/deviceStatus/${dev}`,
    campo: 'capacidade', esquema: 'cap1',
  });
  if (!aberto.ok || !objeto(aberto.valor) || !objeto(aberto.valor.c)) return null;
  const c = aberto.valor.c;
  const adm = objeto(c.admissao) ? c.admissao : { porEstado: {} };
  return {
    frescoAte: (Number(no.u) || 0) + SYNC.FROTA_JANELA_MS,
...
async function aceitarAtribuicoes(engine, cfg, arvore, { agora = Date.now() } = {}) {
  ...
  for (const [itemId, atribuicao] of Object.entries(mapaDe(arvore))) {
    ...
    const vaga = admissao.reservar(engine, { tipo: 'review', ref: meu.pr.key, agora });
...
// A prontidão é o mesmo sinal do batimento, noutro caminho: sequência monotônica assinada
// pelo admin. Só sai depois de um ciclo saudável, e por isso "renovar" significa alguma
// coisa (CT-PRONT).
async function publicarProntidao(engine, cfg, { agora }) {
  const rt = engine.sync;
  const admin = await publicar.lerNo(rt.client, `/users/${rt.uid}/live/control/admin`);
  const generation = admin && admin.valor ? Number(admin.valor.generation) || 0 : 0;
  const minha = adminChave.lerChaveDeAdmin();
  if (!adminChave.chaveServe(minha, { uid: rt.uid, destino: outboxTarget(rt.uid, cfg.databaseUrl), generation })) return false;
  const anterior = await publicar.lerNo(rt.client, `/users/${rt.uid}/${NO_PRONTIDAO}`);
  const sequencia = (anterior && anterior.valor ? Number(anterior.valor.sequencia) || 0 : 0) + 1;
  const valor = { dev: rt.deviceId, generation, sequencia, beatAt: agora };
  const sig = assinatura.assinar(minha.jwk, { uid: rt.uid, caminho: NO_PRONTIDAO, generation, valor });
  if (!sig) return false;
  const w = await rt.client.put(`/users/${rt.uid}/${NO_PRONTIDAO}`, { ...valor, sig }, {});
  return !!(w && w.ok);
}

export default { ativa, cicloDaDistribuicao, publicarProntidao, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, responder, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA, NO_PRONTIDAO };
export { ativa, cicloDaDistribuicao, publicarProntidao, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, responder, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA, NO_PRONTIDAO };
