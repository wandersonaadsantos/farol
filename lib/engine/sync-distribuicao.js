...
async function cicloDoAgendador(engine, cfg, { agora = Date.now() } = {}) {
  ...
  await candidatos.anotarFilaDoConjunto(engine, cfg, { itens, atribuicoes: atribuicoes.valor, agora });
...
async function aceitarAtribuicoes(engine, cfg, arvore, { agora = Date.now() } = {}) {
  ...
  for (const r of recusas) {
    ...
    await responder(engine, cfg, { itemId: r.itemId, estado: 'recusada', code: r.code, detalhe: r.problema || '', esperaAte: agora + SYNC.ATRIBUICAO_TTL_MS, agora });
  }
