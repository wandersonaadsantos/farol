...
async function cicloDoAgendador(engine, cfg, { agora = Date.now() } = {}) {
  ...
  for (const a of avaliados) anotarEspera(rt, a.itemId, motivoDoAvaliado(a), agora);
...
async function aceitarAtribuicoes(engine, cfg, arvore, { agora = Date.now() } = {}) {
  ...
  for (const r of recusas) {
    anotarEspera(rt, r.itemId, r.code, agora);
    await responder(engine, cfg, { itemId: r.itemId, estado: 'recusada', code: r.code, esperaAte: agora + SYNC.ATRIBUICAO_TTL_MS, agora });
  }
...
export default { ativa, distribuindo, cicloDaDistribuicao, publicarProntidao, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, responder, motivoDaEspera, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA, NO_PRONTIDAO };
export { ativa, distribuindo, cicloDaDistribuicao, publicarProntidao, publicarCandidato, fundirFila, cicloDoAgendador, aceitarAtribuicoes, responder, motivoDaEspera, NO_FILA, NO_ATRIBUICAO, NO_RESPOSTA, NO_PRONTIDAO };
