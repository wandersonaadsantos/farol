...
function esperandoParaTela(rt, x) {
  ...
  return { key, desde: Number(x.desde) || 0, motivo: key ? distribuicao.motivoDaEspera(rt, key) : '' };
}
...
function destinosDaTransferencia(engine, dados) {
  const d = dados || {};
  return transferenciaEng.destinosDaTransferencia(engine, cfgDe(engine), { dono: d.dono, acctTag: d.acctTag });
}
...
async function estadoDaLimpeza(engine) {
  ...
  const rt = engine.sync;
  if (!rt || !rt.client || !rt.uid) return recusa(SYNC_CODES.SEM_CREDENCIAL, 'este aparelho não está conectado');
  const [no, admin] = await Promise.all([
    publicar.lerNo(rt.client, `/users/${rt.uid}/live/control/cleanup`),
    publicar.lerNo(rt.client, `/users/${rt.uid}/live/control/admin`),
  ]);
  if (!no || !admin) return recusa(SYNC_CODES.INDISPONIVEL, 'não deu para ler a chave de limpeza agora');
  if (!objeto(no.valor)) return { ok: true, estado: 'desligada' };
  const ligada = limpeza.chaveDeLimpezaLigada(engine, { no: no.valor, admin: admin.valor, autoridade: rt.autoridade });
  return { ok: true, estado: ligada ? 'ligada' : 'desligada-ou-nao-verificavel' };
}
...
export default { projecaoDasTelas, adminParaTela, distribuicaoParaTela, admissaoParaTela, emitidosParaTela, emitirComando, desfechoDoComando, avisoDaTomada, destinosDaTransferencia, estadoDaLimpeza };
export { projecaoDasTelas, adminParaTela, distribuicaoParaTela, admissaoParaTela, emitidosParaTela, emitirComando, desfechoDoComando, avisoDaTomada, destinosDaTransferencia, estadoDaLimpeza };
