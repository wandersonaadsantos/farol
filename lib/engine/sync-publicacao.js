...
// o campo e o esquema moram com quem abre a capacidade: cifrar e decifrar não divergem
const CAMPO_STATUS = capacidade.CAMPO;
const ESQUEMA_STATUS = capacidade.ESQUEMA;
// RAM livre em FAIXA, não em número: o número exato muda a cada segundo, e até
// arredondado ele cruza a fronteira do bloco sozinho, o que faria a capacidade "mudar" e
// ser republicada sem nada ter mudado de verdade. A faixa é relativa ao piso de memória
// da admissão, que é o que interessa a quem lê ("cabe mais trabalho aqui?").
const FAIXAS_DE_RAM = ['baixa', 'media', 'alta'];
...
function faixaDeRam() {
  const livre = admissao.memoriaLivreMb();
  if (livre === null) return 'desconhecida';
  if (livre < SYNC.PISO_MEMORIA_MB) return FAIXAS_DE_RAM[0];
  return livre < SYNC.PISO_MEMORIA_MB * 3 ? FAIXAS_DE_RAM[1] : FAIXAS_DE_RAM[2];
}
...
function capacidadeDe(engine, cfg) {
  ...
  // o admin agenda pela política EFETIVA (a publicada por ele, quando aceita), não pelo
  // número local: publicar o local faria o agendador ignorar a própria pausa
  const efetiva = admissao.politicaDoAparelho(engine);
  ...
  return {
    ...
    paralelismo: Math.max(1, Number(efetiva.tetoParalelismo) || 1),
    pausado: efetiva.pausado === true,
    ramLivre: faixaDeRam(),
    aceitarAdmin: cfg.aceitarAdmin === true,
    keyReady: !!rt.material,
    // resumo da admissão local (CT-ADM): só CONTAGEM por estado e por tipo. As
    // referências de PR ficam de fora mesmo cifradas: quem lê a capacidade quer saber
    // se cabe mais trabalho, não o que está rodando.
    admissao: contagemDaAdmissao(engine),
    // teto do grupo (C4b): sequência e dia da última sessão com grupo, e reservas vivas por
    // grupo. É o que deixa os outros aparelhos enxergarem lacuna e reserva alheia.
    consumo: consumoGrupo.consumoParaCapacidade(engine),
  };
}

function contagemDaAdmissao(engine) {
  const r = admissao.resumo(engine);
  return { total: r.total, porEstado: r.porEstado, porTipo: r.porTipo };
}
...
// O PR de uma tag, na forma que as telas leem (`pr: { key, account, title, author }`), ou
// `null`. É APRESENTAÇÃO e endereço de leitura (o aviso da tomada lê o lease pela chave e
// pela conta), nunca decisão: quem executa confere tudo de novo com o que sabe.
//
// Falha fechada, e com uma conferência a mais: a linha aberta tem que ser DESTA tag. O
// envelope já amarra o caminho, e a conferência mantém a garantia se um dia o caminho
// mudar. A conta sai do mapeamento local (owner para conta), porque ela não viaja.
async function prDaTag(engine, cfg, tag) {
  const chave = String(tag || '');
  const rt = engine.sync;
  if (!TAG_RE.test(chave) || !rt || !rt.material) return null;
  let linha = null;
  try { linha = await lerDoCatalogo(engine, cfg, chave); } catch { return null; }
  if (!linha || !linha.key || prTag(kek.bufferDe(rt.material.id), linha.key) !== chave) return null;
  const conta = typeof engine.accountForPr === 'function' ? engine.accountForPr({ key: linha.key, repo: linha.repo }) : '';
  return { key: linha.key, account: String(conta || ''), title: linha.title, author: linha.author };
}

function resumoDe(valor) { return resumoDoClaro(valor); }

export default { resumoDe, capacidadeDe, publicarCapacidade, podePublicar, jaPublicado, marcarPublicado, publicarNoCatalogo, lerDoCatalogo, prDaTag };
export { resumoDe, capacidadeDe, publicarCapacidade, podePublicar, jaPublicado, marcarPublicado, publicarNoCatalogo, lerDoCatalogo, prDaTag };
