...
// A sobra que a memória não alcança (25/09/2026). O apagar de sincronizarPendencias só
// conhece o que ESTA sessão publicou, e o mapa nasce vazio a cada boot: pendência
// resolvida enquanto o Firebase estava fora, seguida de um reinício (o auto-update), ficava
// no banco para sempre, e os outros aparelhos mostravam "precisa de você" de PR já postado.
// Medido: três nós do admin, o mais velho de 18/09, com zero pendências locais. Aqui a
// conferência é contra o BANCO: nó deste aparelho cujo id não sai de nenhuma pendência
// aberta agora é apagado. O id é determinístico (itemIdDe), então não depende de memória.
async function apagarOrfas(engine, cfg, arvore) {
  const pode = publicacao.podePublicar(engine, cfg);
  if (!pode.ok || !objeto(arvore)) return [];
  const rt = engine.sync;
  const kId = kek.bufferDe(rt.material.id);
  const abertas = engine.decisions && Array.isArray(engine.decisions.pending) ? engine.decisions.pending : [];
  const vivos = new Set(abertas.filter((item) => item && item.id).map((item) => pendencia.itemIdDe(kId, rt.deviceId, item.id)));
  const apagadas = [];
  for (const [itemId, no] of Object.entries(arvore)) {
    if (!objeto(no) || no.dev !== rt.deviceId || vivos.has(itemId)) continue;
    const r = await rt.client.del(`/users/${rt.uid}/${NO}/${itemId}`);
    if (r && r.ok) apagadas.push(itemId);
  }
  return apagadas;
}
...
// A mesma aplicação, com o PR de cada pendência nomeado pelo catálogo (`prDaTag`, a mesma
// boca das operações do andamento; null quando o catálogo não está disponível). É o
// caminho do relógio.
async function aplicarPendenciasIdentificadas(engine, cfg, arvore, vistos) {
  const lista = [];
  for (const p of lerPendencias(engine, arvore, vistos)) lista.push({ ...p, pr: await publicacao.prDaTag(engine, cfg, p.prTag) });
...
export default { sincronizarPendencias, apagarOrfas, lerPendencias, aplicarPendencias, aplicarPendenciasIdentificadas, marcarVisto, limparVistos };
export { sincronizarPendencias, apagarOrfas, lerPendencias, aplicarPendencias, aplicarPendenciasIdentificadas, marcarVisto, limparVistos };
