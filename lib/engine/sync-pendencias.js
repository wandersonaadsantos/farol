...
function lerPendencias(engine, arvore, vistos) {
  ...
  for (const [itemId, no] of Object.entries(arvore)) {
    ...
    saida.push({ itemId, dev: no.dev, aparelho: aparelho || '', at: Number(no.at) || 0, visto: objeto(vistos) && objeto(vistos[itemId]), ...aberto.valor.p });
  }
...
function aplicarPendencias(engine, arvore, vistos) {
  const rt = engine.sync;
  const lista = lerPendencias(engine, arvore, vistos);
...
export default { sincronizarPendencias, lerPendencias, aplicarPendencias, marcarVisto, limparVistos };
export { sincronizarPendencias, lerPendencias, aplicarPendencias, marcarVisto, limparVistos };
