...
// O head ATIVO do PR já tem review decisivo de OUTRA pessoa? Decisivo é APPROVED
// ou CHANGES_REQUESTED; DISMISSED e COMMENTED não contam, e as contas de
// NAO_SAO_PESSOAS (ferramenta) também não (comparação sem caixa). Head novo zera
// o histórico por construção: reviewsDeOutros filtra por commit_id === head.
...
async function bloqueadoPorHistorico(engine, pr) {
  ...
  const decisivos = reviews.filter(r => {
    if (r.state !== 'APPROVED' && r.state !== 'CHANGES_REQUESTED') return false;
    const login = String(r.quem || '').trim();
    return !!login && !NAO_SAO_PESSOAS.has(login.toLowerCase());
  });
  const grafias = new Map(); // chave minúscula -> primeira grafia vista
  for (const r of decisivos) {
    const chave = String(r.quem).trim().toLowerCase();
    if (!grafias.has(chave)) grafias.set(chave, String(r.quem).trim());
  }
  return {
    bloqueado: decisivos.length > 0,
    head,
    quem: [...grafias.values()].sort((a, b) => a.localeCompare(b)),
    // os estados viajam junto pro toast acertar a concordância (aprovou x
    // pediu mudanças); quem consome a decisão usa só `bloqueado`
    decisivos: decisivos.map(r => ({ quem: String(r.quem).trim(), state: r.state })),
  };
}
