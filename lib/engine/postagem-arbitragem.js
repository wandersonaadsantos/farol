...
// Firebase configurado mas fora do ar (sem conexão, login vencido, sem credencial): a
// arbitragem não tem onde gravar intenção nem posse, e antes da v2.62.1 isso barrava TUDO,
// inclusive o clique e a revisão que você mandou rodar sem coordenação (que rodava, gastava a
// sessão e morria aqui). O ato EXPLÍCITO segue pelo caminho sem Firebase, com a conferência no
// GitHub (`conferirNoGithub`); o que é automático (reenvio, co-assinatura, revisão com lease)
// continua esperando a conexão, porque é ali que dois aparelhos poderiam postar juntos.
const VIAS_EXPLICITAS = new Set(['clique', 'sessao']);

function conectado(engine) {
  const rt = engine && engine.sync;
  return !!rt && rt.status === 'conectado' && !!rt.client;
}

// Coordenação ligada na configuração e sem conexão agora.
function coordenacaoForaDoAr(engine) {
  return postagemCoordenadaLigada(engine) && !conectado(engine);
}

function contornoComFirebaseFora(engine, opcoes) {
  if (conectado(engine)) return false;
  const o = opcoes || {};
  const via = String(o.via || '');
  if (VIAS_EXPLICITAS.has(via)) return true;
  // o handle noop, com a coordenação ligada, só nasce do clique em "revisar sem coordenação"
  return via === 'revisao' && !!(o.handle && o.handle.noop === true);
}

// Sem a arbitragem, quem impede o segundo review é o próprio GitHub: veredito igual naquele
// commit já está lá, então não posta; sem conseguir conferir, também não (nunca às cegas).
async function conferirNoGithub(engine, pr, payload) {
  const evento = String((payload && payload.event) || '').toUpperCase();
  const head = shaValido(payload && payload.commit_id) || (await headVivo(engine, pr));
  let estados = null;
  try { estados = head ? await engine.myReviewStates(pr, head) : null; } catch { estados = null; }
  if (!Array.isArray(estados)) return naoEnviada('estado-desconhecido');
  if (estados.includes(ESTADO_NO_GITHUB[evento])) return { ok: true, deduped: true, estado: 'confirmada', motivo: 'ja-no-github', attempted: false, reviewId: '' };
  return null;
}
...
export default { arbitrarPostagem, reconciliarPostagensIncertas, postagemCoordenada, postagemCoordenadaLigada, lancarSePossePerdida, falhaPassageira, postarComRetentativas, retentativa, contornoComFirebaseFora, conferirNoGithub, coordenacaoForaDoAr, MOTIVOS };
export { arbitrarPostagem, reconciliarPostagensIncertas, postagemCoordenada, postagemCoordenadaLigada, lancarSePossePerdida, falhaPassageira, postarComRetentativas, retentativa, contornoComFirebaseFora, conferirNoGithub, coordenacaoForaDoAr, MOTIVOS };
