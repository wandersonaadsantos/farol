// Roteador do modo Auto (PURA). Quando config.reviewModel === 'auto', a revisão headless
// não manda 'auto' pro CLI: escolhe o modelo (e esforço/fast) a partir das métricas do PR,
// que o Farol já mediu pra fan-out.
//
// Desde 26/09/2026 a qualidade vem antes do custo, por decisão do dono: toda faixa usa o
// Opus no esforço herdado (o mesmo do Opus fixo) e o contexto sensível SOBE o raciocínio.
// O porquê, com a auditoria que mediu o Haiku e o Sonnet, está em lib/modelos.js, junto
// de AUTO_POR_FAIXA. O nome das faixas continua, porque o rótulo da atividade diz qual
// valeu e porque o fan-out usa os mesmos limiares.
//
...
// sensível ou PR muito grande pedem mais (AUTO_POR_CONTEXTO: desde 26/09/2026, o Opus com
// raciocínio xhigh, acima do Opus fixo); o primeiro que vale,
...
const CAMINHOS_SENSIVEIS_PADRAO = Object.freeze([
  ...
  // kustomize: o infra-k8s da BIUD vive em plataforma/zonas/<app>/overlays/<env>/, e nenhum
  // padrão acima casava (26/09/2026: configmap e deployment de HMG caíram no modelo leve).
  'overlays/', 'kustomization.yaml', '**/*configmap*',
...
const DESCRICAO_DA_FAIXA = {
  semMetrica: 'métrica indisponível, {m}',
...
function descricaoDoModelo(f, nome) {
  ...
  if (f.effort) return `${nome} com esforço ${f.effort}`;
  return nome;
}
...
function rotuloOrigem(origem, gatilho = null) {
  ...
  if (origem === ORIGEM_DO_CONTEXTO) {
    ...
    return `auto: ${motivo}, ${descricaoDoModelo(AUTO_POR_CONTEXTO, nomeDaSelecao(AUTO_POR_CONTEXTO.selecao))}`;
  }
