...
function exportarConversa(chat, { agora = Date.now(), versao = '' } = {}) {
  ...
  // jsonTexto: o arquivo .json pronto, serializado aqui (io.js é o santuário de JSON cru), para
  // a tela só entregar o arquivo sem montar texto de novo
  return { ok: true, nome: nomeDoArquivo(json.pr, agora), markdown: markdownDe(json), json, jsonTexto: safeStringify(json) };
}
