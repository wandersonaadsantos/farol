...
// Ver docs/QUALITY.md e docs/RELEASE.md (seção Release: fonte de verdade = release do GitHub).
...
import { assertElectronRuntime, packageElectronRequirement, checkElectronPackage } from '../electron-runtime.js';
...
// update: a cópia instalada só pode ser preservada quando satisfaz o manifesto novo.
...
// NADA_ERROR e o outro nao-erro do applyUpdateInner: ele re-checa o release antes de
// baixar, e quando essa re-checagem nao ve mais atualizacao (o guard externo do
// maybeAutoUpdate leu um engine.update de um ciclo anterior, ou a release saiu do ar)
// o retorno significa "nao havia o que fazer", nao "a atualizacao falhou". Tem nome
// aqui, e nao literal no ponto de uso, porque o maybeAutoUpdate precisa reconhece-lo
// pra nao logar ERROR: o farol.log e so falha, e 'nenhuma atualizacao disponivel'
// apareceu la em 23/08/2026 como se fosse uma.
const NADA_ERROR = 'nenhuma atualização disponível';
function sessionsBusy(engine) {
  ...
  return motivoOcupado(engine) !== null;
}
...
// O que está segurando o update, em palavras, ou null quando nada segura. É a MESMA
// pergunta do sessionsBusy (que só a transforma em sim ou não), para a tela nunca dizer
// "esperando" por um motivo diferente do que de fato segurou. Revisão também ocupa uma
// entrada em `running`; ela é contada como revisão, e o resto de `running` é chat,
// autoanálise ou ferramenta.
function motivoOcupado(engine) {
  ...
  let revisoes = 0;
  for (const n of engine.headlessBusyAccounts.values()) revisoes += n;
  const outras = Math.max(0, engine.running.size - revisoes);
  const partes = [];
  if (revisoes) partes.push(plural(revisoes, 'revisão em andamento', 'revisões em andamento'));
  if (engine.headlessQueue.length) partes.push(plural(engine.headlessQueue.length, 'PR na fila de revisão', 'PRs na fila de revisão'));
  if (outras) partes.push(`${plural(outras, 'sessão de IA em andamento', 'sessões de IA em andamento')} (chat, autoanálise ou ferramenta)`);
  if (terminalVivo) partes.push('sessão de terminal aberta');
  return partes.length ? partes.join(', ') : null;
}

// O adiamento vira estado visível no status do update (25/09/2026): a v2.62.14 esperou
// sete minutos por um motivo que não ficou em lugar nenhum. O farol.log fica de fora, porque
// esperar uma revisão terminar é o comportamento certo, não uma falha. O `desde` mora no
// engine porque o checkUpdate recria o objeto engine.update a cada ciclo.
function marcarAdiado(engine, motivo) {
  if (!engine.updateAdiadoDesde) engine.updateAdiadoDesde = Date.now();
  if (engine.update) engine.update.adiado = { motivo, desde: engine.updateAdiadoDesde };
  if (typeof engine.pushState === 'function') engine.pushState();
}

function limparAdiado(engine) {
  engine.updateAdiadoDesde = 0;
  if (engine.update) delete engine.update.adiado;
}
...
async function maybeAutoUpdate(engine, deps = {}) {
  ...
  if (engine.config.autoUpdate === false && !engine.updateQueued) { limparAdiado(engine); return { ok: false, skipped: 'desligado' }; }
  ...
  const ocupado = motivoOcupado(engine);
  if (ocupado) { marcarAdiado(engine, ocupado); return { ok: false, skipped: 'ocupado' }; }
  const backoffMs = TEMPOS.AUTO_UPDATE_BACKOFF_MS;
  if (engine.autoUpdateFailedAt && Date.now() - engine.autoUpdateFailedAt < backoffMs) {
    marcarAdiado(engine, `a última tentativa falhou; tenta de novo às ${horaCurta(engine.autoUpdateFailedAt + backoffMs)}`);
  ...
  try {
    ...
    // NADA_ERROR entra ao lado do BUSY_ERROR pelo mesmo motivo: os dois sao corrida
    // PRE-tentativa (nada foi baixado, nada foi mexido), nao falha de tentativa real.
    // Nem loga, nem arma o backoff de 30 min, que castigaria a proxima release de
    // verdade por causa de um ciclo em que simplesmente nao havia o que atualizar.
    if (!r.ok && r.error !== BUSY_ERROR && r.error !== NADA_ERROR) {
...
// Pacote leve não troca o runtime. Recusar antes de escrever scripts/marcador ou
// iniciar o installer evita aplicar código que a instalação atual não suporta.
function updateRuntimeError(source, version) {
  try {
    const requirement = packageElectronRequirement(path.join(source, 'package.json'));
    let native = IS_WIN ? 'electron.exe' : 'electron';
    if (IS_MAC) native = path.join('Electron.app', 'Contents', 'MacOS', 'Electron');
    const bundled = path.join(source, 'node_modules', 'electron', 'dist', native);
    if (fs.existsSync(bundled)) checkElectronPackage(source, true);
    else assertElectronRuntime(version, requirement);
    return null;
...
async function applyUpdateInner(engine, deps) {
  ...
  await check(engine);
  if (!engine.update.available) return { ok: false, error: NADA_ERROR };
  ...
  if (sessionsBusy(engine)) return { ok: false, error: BUSY_ERROR };
  const installerName = IS_WIN ? 'install.ps1' : posixInstallerName(IS_MAC);
  const installer = path.join(engine.update.source, 'installer', installerName);
  if (!fs.existsSync(installer)) return { ok: false, error: `installer não encontrado em ${engine.update.source}/installer` };
  const runtimeError = updateRuntimeError(engine.update.source, deps.electronVersion ?? process.versions.electron);
  if (runtimeError) {
    engine.emit('toast', { kind: 'error', text: runtimeError });
...
const updateMod = {
  ...
  sessionsBusy, motivoOcupado, pruneOldDownloads, BUSY_ERROR, maybeAutoUpdate,
};
