...
async function maybeAutoUpdate(engine, deps = {}) {
  ...
  if (engine.config.autoUpdate === false) return { ok: false, skipped: 'desligado' };
  ...
  try {
    ...
    const r = await (deps.applyUpdate || applyUpdateInner)(engine, deps);
...
function applyUpdateMac(engine) {
  ...
  fs.writeFileSync(scriptFile, builder(installer, path.join(STATE_DIR, 'update.log')), { mode: 0o700 });
