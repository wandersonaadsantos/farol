...
async function applyUpdate(engine) {
  await checkUpdate(engine);
  ...
  if (engine.update.channel === 'remote') {
    try { engine.update.source = await downloadRemoteUpdate(engine); }
    catch (e) {
...
module.exports = {
  ...
  downloadRemoteUpdate, applyUpdate, applyUpdateMac,
};
