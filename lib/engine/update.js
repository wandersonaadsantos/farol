...
function buildUpdateScriptMac(installer, logFile) {
  return [
    '#!/bin/bash',
...
function applyUpdateMac(engine) {
  ...
  fs.writeFileSync(scriptFile, buildUpdateScriptMac(installer, path.join(STATE_DIR, 'update.log')), { mode: 0o700 });
  logSpawn('applyUpdateMac', ['/bin/bash', scriptFile]);
...
module.exports = {
  ...
  downloadRemoteUpdate, applyUpdate, applyUpdateMac, buildUpdateLaunchCommand, buildUpdateScriptMac,
