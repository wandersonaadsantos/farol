...
import { DELIVERIES_LIMIT } from '../paths.js';
import { TEMPOS } from '../constants.js';
import { buscarFatiado, isoSegundos } from './entregas-fatias.js';
import contasGh from './contas-gh.js';
import limiteGh from './limite-gh.js';

// fim da janela das entregas: agora mais uma hora. A busca precisa de fim concreto
// pra poder fatiar, e um relogio local atrasado em relacao ao GitHub cortaria os
// merges mais recentes; merge no futuro nao existe, entao a folga nao traz ruido.
const FOLGA_RELOGIO_MS = TEMPOS.HORA_MS;

async function searchPRs(engine, extraArgs, user, agora = Date.now()) {
  // conta no limite de requisicoes do GitHub: nem tenta. Insistir falha E consome cota,
  // entao a tentativa prolonga o proprio bloqueio (lib/engine/limite-gh.js). Devolve null,
  // que e o mesmo contrato de falha de busca: o check() preserva o que ja sabia.
  if (limiteGh.emEspera(engine, user, agora)) return null;
  ...
  if (!engine.tokenFor(user)) {
    contasGh.avisarSemToken(engine, user, `gh search pulado (${user || 'primaria'} sem token no gh)`);
    return null;
  }
  // `labels` viaja desde a v2.48.4 pro gate de "outra pessoa já está revisando"
  // (lib/engine/skip-review.js). Desde 28/08/2026 ela carrega SÓ o sinal LEGADO
  // de cópias antigas do Farol (`<conta>:revisando`): a versão nova escreve o
  // sinal como ref git invisível (lib/engine/review-signal.js) e lê os dois na
  // transição. Vem da MESMA chamada, sem custo de rede extra; só o nome de cada
  // label entra no objeto, porque cor/id/descrição não decidem nada e o objeto
  // do PR vai cru pro snapshot da UI a cada push.
  const args = ['search', 'prs', ...extraArgs, '--state', 'open', '--limit', '100',
    '--json', 'url,title,isDraft,author,number,repository,updatedAt,labels'];
  const r = await io.run('gh', args, { env: engine.ghEnv(user) });
  if (!r.ok) {
    if (await limiteGh.tratarFalhaDeBusca(engine, user, r.stderr, agora)) return null;
...
// só os NOMES das labels, que é o que o gate de pulo lê. Entrada torta (campo
// ausente, item sem name) vira lista vazia em vez de derrubar a busca inteira.
function labelNames(labels) {
  return Array.isArray(labels) ? labels.map(l => String((l && l.name) || '')).filter(Boolean) : [];
}
...
async function myAuthoredPRs(engine, user, agora = Date.now()) {
  const acc = user || engine.primaryUser();
  const me = (acc || '').toLowerCase();
  if (!me) return null;
  if (limiteGh.emEspera(engine, user, agora)) return null;
  if (!engine.tokenFor(user)) {
    contasGh.avisarSemToken(engine, user, `gh search prs --author @me pulado (${acc} sem token no gh)`);
    return null;
  }
  const r = await io.run('gh', ['search', 'prs', '--author', '@me', '--state', 'open', '--limit', '50',
...
function deliveriesSince(engine, days) {
  ...
  return isoSegundos(d.getTime());
}

// Uma consulta de UMA fatia [ini..fim] de uma org, no formato que buscarFatiado
// espera. ok false = falha do gh ou JSON invalido (vira partial, nunca lista vazia).
// --sort updated (desc) segue valendo dentro da fatia minima cheia: o default "best
// match" (relevancia) cortava um subconjunto arbitrario e furava o grafico em dias
// aleatorios.
function buscadorDeEntregas(engine, t) {
  return async (ini, fim, limite) => {
    // a busca de entregas fatia a janela: numa conta no limite seriam varias chamadas
    // falhas por org, cada uma consumindo cota. Fatia nao lida vira partial, que a tela
    // ja sabe mostrar.
    if (limiteGh.emEspera(engine, t.user)) return { ok: false };
    const r = await io.run('gh', ['search', 'prs', `merged:${ini}..${fim}`, '--owner', t.owner,
      '--sort', 'updated', '--order', 'desc',
      '--limit', String(limite), '--json', 'url,title,author,number,repository,closedAt'], { env: engine.ghEnv(t.user) });
    if (!r.ok) {
      if (await limiteGh.tratarFalhaDeBusca(engine, t.user, r.stderr)) return { ok: false };
      engine.log('WARN', `gh search entregas (${t.user}/${t.owner}): ${r.stderr.trim().slice(0, 200)}`);
      return { ok: false };
    }
    try { return { ok: true, list: JSON.parse(r.stdout || '[]') }; } catch { return { ok: false }; }
  };
}
...
async function fetchDeliveries(engine, days, owner) {
  ...
  for (const t of targets) {
    ...
    // janela fatiada por data de merge (entregas-fatias.js): uma consulta devolve no
    // maximo 1000, entao so fatia quando a fatia volta no teto
    const res = await buscarFatiado({
      inicioMs: Date.parse(since), fimMs: Date.now() + FOLGA_RELOGIO_MS,
      buscar: buscadorDeEntregas(engine, t), tetoTotal: DELIVERIES_LIMIT
    });
...
async function prState(engine, pr) {
  ...
  const r = await io.run('gh', ['pr', 'view', url, '--json', 'state'], { env: engine.ghEnv(acc) });
  if (!r.ok) return null;
...
async function headSha(engine, pr) {
  ...
  const url = pr.url || `https://github.com/${pr.repo}/pull/${pr.number}`;
  const r = await io.run('gh', ['pr', 'view', url, '--json', 'headRefOid', '--jq', '.headRefOid'], { env: engine.ghEnv(acc) });
  if (!r.ok) return '';
  return String(r.stdout || '').trim();
}

// Fontes da chave do card: título, branch e corpo. Conta dona e token resolvidos
// como em prState e headSha: buscar com outra identidade devolveria resultado que
// PARECE válido. Falha degrada pra vazio, porque card não verificável já é estado
// previsto do fluxo, e derrubar a revisão inteira por causa disso seria pior.
//
// io.parseJson, não JSON.parse: os cinco JSON.parse deste arquivo são dívida
// travada na baseline do gate, e um sexto reprova por regressão.
async function prCardSources(engine, pr) {
  const vazio = { title: '', headRefName: '', body: '' };
  const acc = engine.accountForPr(pr);
  if (!engine.tokenFor(acc)) return vazio;
  const url = pr.url || `https://github.com/${pr.repo}/pull/${pr.number}`;
  const r = await io.run('gh', ['pr', 'view', url, '--json', 'title,headRefName,body'], { env: engine.ghEnv(acc) });
  if (!r.ok) {
    engine.log('WARN', `metadados do PR ${pr.repo}#${pr.number} indisponíveis, card fica não verificável: ${String(r.stderr || '').trim().slice(0, 300)}`);
    return vazio;
  }
  const dados = io.parseJson(r.stdout, null) || {};
  return { title: String(dados.title || ''), headRefName: String(dados.headRefName || ''), body: String(dados.body || '') };
}
...
async function refreshContributors(engine) {
  ...
  const r = await io.run('gh', ['api', `repos/${repo}/contributors?per_page=100`], { env });
  if (!r.ok) {
  ...
  let ownerName = '';
  const ro = await io.run('gh', ['api', `users/${owner}`, '--jq', '.name // ""'], { env });
  if (ro.ok) ownerName = String(ro.stdout || '').trim();
  engine.credits = { at: Date.now(), repo, owner: { login: owner, name: ownerName }, contributors };
  engine.pushState();
}

const ghQueries = { searchPRs, myAuthoredPRs, deliveriesSince, fetchDeliveries, prState, headSha, prCardSources, refreshContributors };
export default ghQueries;
export { searchPRs, myAuthoredPRs, deliveriesSince, fetchDeliveries, prState, headSha, prCardSources, refreshContributors };
