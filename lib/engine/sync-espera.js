...
function daAtribuicao(no, agora) {
  ...
  return { motivo: 'atribuicao-viva', dev: String(a.dev), aparelhos: [], at: Number(a.ttl) - SYNC.ATRIBUICAO_TTL_MS };
}
...
function doVeredito(rt, itemId, no, agora) {
  ...
  return { motivo: aberto.motivo, dev: '', aparelhos: aberto.aparelhos, at: Number(a.espera.ttl) - TTL_MS };
}
...
function daResposta(no, agora) {
  ...
  return {
    motivo: String(r.code), dev: String(r.dev || ''), at: Number(r.at) || 0,
