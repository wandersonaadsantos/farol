...
async function prMetrics(engine, pr) {
  const r = await io.run('gh', ['pr', 'view', pr.url, '--json', 'additions,deletions,changedFiles,files'],
...
// Módulo patchado em teste (`fanout.prMetrics = async () => null`, ver
// test/dedup-round.test.js, test/review-commit-id.test.js, test/review-reasons.test.js):
// mesma regra do lib/io.js, default MUTÁVEL e consumo interno/externo só via default.
const fanout = {
  prMetrics, shouldFanOut, planLotes, fanOutBlock, lotesAlvo,
  FANOUT_MIN_LINES, FANOUT_MIN_FILES, MAX_LOTES
};
export default fanout;
export { shouldFanOut, planLotes, fanOutBlock, lotesAlvo, FANOUT_MIN_LINES, FANOUT_MIN_FILES, MAX_LOTES };
