...
async function lerEscopo(engine, { tipo, conta, desdeU = 0 }) {
  const rt = engine.sync;
  if (!TETOS[tipo] || !rt || !rt.client || !rt.material) return { linhas: [], saidas: [] };
  const kId = kek.bufferDe(rt.material.id);
  const scope = acctTag(kId, String(conta));
  ...
  for (const [id, no] of Object.entries(cruas || {})) {
    ...
    const tag = id.slice(scope.length + 1);
    if (no.del === true) { saidas.push(tag); continue; }
    ...
    if (!aberto.ok || !objeto(aberto.valor) || !objeto(aberto.valor.l)) continue;
    linhas.push({ prTag: tag, u: Number(no.u) || 0, somenteLeitura: tipo === 'myPrs', ...aberto.valor.l });
  }
  return { linhas: linhas.sort((a, b) => b.u - a.u), saidas };
}
