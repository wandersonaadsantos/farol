...
import { SYNC, ATIVACAO_TETO_GRUPO_C4B } from '../constants.js';
...
// A liberação da medição vem da constante; o engine só a sobrescreve em memória, e isso
// existe para teste: nem rota nem config alcançam essa propriedade.
function ativacaoLiberada(engine) {
  return ATIVACAO_TETO_GRUPO_C4B === true || (!!engine && engine.ativacaoTetoGrupo === true);
}

const FRASES_DOS_REQUISITOS = {
  compartilhamento: 'o compartilhamento cifrado está desligado',
  'sem-grupo': 'o grupo não tem identidade',
  'sem-teto': 'o grupo não tem teto',
  'medicao-pendente': 'falta a medição do atraso do consumo entre dois aparelhos reais',
};

async function syncPublicarGrupo(engine, cfg, { grupo: bruto } = {}) {
  const limpo = grupo.sanearGrupo(bruto);
  if (limpo.ativo === true) {
    const faltam = grupo.requisitosDaAtivacao({ grupo: limpo, compartilhamento: sharedActive(cfg), medicaoFeita: ativacaoLiberada(engine) });
    if (faltam.length) return recusa('ativacao-bloqueada', `não dá para ativar o teto: ${faltam.map((f) => FRASES_DOS_REQUISITOS[f]).join('; ')}`);
  }
...
export default { syncPublicarGrupo, aceitarGrupo, grupoAceito, syncVincularPerfil, ativacaoLiberada };
export { syncPublicarGrupo, aceitarGrupo, grupoAceito, syncVincularPerfil, ativacaoLiberada };
