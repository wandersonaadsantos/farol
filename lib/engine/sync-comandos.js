...
//
// O HEAD CONFERIDO É O ATUAL. O PR da fila vem da busca do GitHub, que não traz o head;
// sem ele o executor pergunta agora (`engine.headSha`), e falha na pergunta é head
// desconhecido, que recusa. O `headSha` que o objeto já tiver continua valendo primeiro.
...
async function tomar(engine, args, agora) {
  ...
  if (!vaga.ok) return { estado: 'recusado', code: vaga.motivo === 'sem-vaga' ? 'sem_vaga' : 'inapto' };
  const item = { ...comando.prDoComando(pr), tomarLease: true, admissaoId: vaga.id };
