...
async function emitir(engine, cfg, { alvo, tipo, args, ttlMs = TTL_PADRAO_MS, agora = Date.now() } = {}) {
  ...
  // `vence` é o prazo gravado no nó: a tela mostra até quando o comando espera o recibo
  return { ok: true, cmdId, estado: 'enviado', vence: no.ttl };
}

// O desfecho REAL de um comando emitido: só o recibo do alvo conta. Esta função é a LEITURA
// crua do nó; quem decide se aquele recibo pertence ao alvo é `comando.conferirRecibo`, e
// quem junta as duas pontas é `desfechoDoComando` (sync-telas.js), que sabe o alvo.
...
// Para QUEM este comando foi? O `alvo` viaja em claro no nó, é assinado junto com o resto
// (`comando.valorAssinado`) e sobrevive ao reinício do emissor — ao contrário de
// `rt.comandosEmitidos`, que é memória de sessão. Vazio quando o nó já saiu (TTL) ou quando
// a leitura falha: "não sei o alvo" nunca pode virar "o alvo confirmou".
async function alvoDoComando(engine, cmdId) {
  const rt = engine.sync;
  if (!rt || !rt.client || !rt.uid) return '';
  const r = await publicar.lerNo(rt.client, `/users/${rt.uid}/${NO}/${cmdId}`);
  return r && objeto(r.valor) ? String(r.valor.alvo || '') : '';
}

// --- lado do executor ------------------------------------------------------------------

// O endereço do comando, lido do nó em claro. Sem deviceId local não existe "meu": um id
// vazio casaria com um `alvo` vazio e o aparelho responderia por qualquer um.
function eMeu(rt, no) {
  const meu = String((rt && rt.deviceId) || '');
  return !!meu && objeto(no) && String(no.alvo || '') === meu;
}
...
// presumido igual ao que o admin viu. O PR daqui vem da busca do GitHub, que NÃO traz o
// head, então quem responde qual é o head de agora é o `headAtual` (o mesmo da tomada).
async function repetir(engine, kId, args) {
  const pr = prPorTag(engine, kId, args.prTag);
  if (!pr) return { estado: 'recusado', code: 'nao_conheco' };
  const head = await headAtual(engine, pr);
...
//
// O head do CANDIDATO é o de quando ele foi publicado, e comparar a tag com ele mesmo não
// é conferência nenhuma: o commit novo cria um item novo e o antigo continua no mapa. Por
// isso o head é perguntado agora, com o `headSha` do objeto deixado de fora de propósito.
async function iniciar(engine, args, agora) {
  const rt = engine.sync;
  ...
  const kId = kIdDe(rt);
  const head = await headAtual(engine, { ...meu.pr, headSha: '' });
  if (!head || matTag(kId, head) !== args.matTag) return { estado: 'recusado', code: 'head_mudou' };
  ...
  engine.enfileirarDaDistribuicao({ ...comando.prDoComando(meu.pr), itemIdDistribuido: `${args.prTag}_${args.matTag}` }, vaga.id);
  return { estado: 'aplicado' };
}
...
// A transferência é a única que mexe em sessão VIVA daqui: ela encerra a de origem depois
// de mandar a memória, e só então devolve o item ao conjunto.
async function transferir(engine, cfg, args, agora) {
  const r = await transferencia.transferir(engine, cfg, { ...args, agora });
  if (r.ok) return { estado: 'aplicado' };
  return { estado: 'recusado', code: r.code || 'nao_transferiu' };
}

// 7.C8: tomar é atribuição forçada COM tomada de lease. Ela sempre passa pela admissão
// local, e o aviso de risco já foi mostrado a quem pediu (o comando exige `confirmado`).
// O head conferido é o ATUAL (`headAtual`), nunca o que o comando presume.
async function tomar(engine, args, agora) {
  const rt = engine.sync;
  const kId = kIdDe(rt);
  const pr = prPorTag(engine, kId, args.prTag);
  if (!pr) return { estado: 'recusado', code: 'nao_conheco' };
  const head = await headAtual(engine, pr);
  if (!head || matTag(kId, head) !== args.matTag) return { estado: 'recusado', code: 'head_mudou' };
  const vaga = admissao.reservar(engine, { tipo: 'review', ref: pr.key, agora, grupo: engine.grupoDaConta ? engine.grupoDaConta(engine.accountForPr(pr)) : '' });
  if (!vaga.ok) return { estado: 'recusado', code: vaga.motivo === 'sem-vaga' ? 'sem_vaga' : 'inapto' };
  const item = { ...comando.prDoComando(pr), tomarLease: true, admissaoId: vaga.id, itemIdDistribuido: `${args.prTag}_${args.matTag}` };
  engine.enfileirarDaDistribuicao(item, vaga.id);
  return { estado: 'aplicado' };
}

async function executar(engine, cfg, cmdId, claro, agora) {
  const rt = engine.sync;
  const kId = kIdDe(rt);
  ...
  if (tipo === 'transferir') return transferir(engine, cfg, args, agora);
  if (tipo === 'tomar') return tomar(engine, args, agora);
...
async function aplicarUm(engine, cfg, { cmdId, no, admin, agora }) {
  ...
  const desfecho = await executar(engine, cfg, cmdId, claro, agora);
...
async function cicloDosComandos(engine, cfg, { agora = Date.now() } = {}) {
  ...
  for (const [cmdId, no] of Object.entries(arvore)) {
    ...
    // ROTEAMENTO ANTES DE QUALQUER EFEITO: a árvore inteira chega aqui (a regra do banco não
    // separa por alvo), e comando de outro endereço não pode produzir recibo, registro local
    // nem linha de resultado. Isto SÓ descarta; quem autoriza continua sendo `podeAplicar`,
    // com assinatura, geração, frescor e TTL, e ele repete a conferência de alvo.
    if (!eMeu(rt, no)) continue;
...
// A outra saída do pedido: a pessoa DESTE aparelho diz não. Sem isto o pedido ficava sem
// desfecho, e quem designou via o comando esperando para sempre um recibo que não vinha.
// O registro local vem antes do recibo, pelo mesmo motivo do executar: a recusa não pode
// virar duas.
async function recusarDesignacao(engine, { agora = Date.now() } = {}) {
  const rt = engine.sync;
  const pedido = rt && objeto(rt.designacaoAdmin) ? rt.designacaoAdmin : null;
  if (!pedido) return recusa('sem-pedido', 'não há pedido de designação esperando neste aparelho');
  if (!rt.client || !rt.uid) return recusa(SYNC_CODES.SEM_CREDENCIAL, 'este aparelho não está conectado');
  rt.designacaoAdmin = null;
  gravarFeito(pedido.cmdId, { estado: 'recusado', code: 'recusado_no_aparelho', tipo: 'designar-admin', at: agora });
  const reciboEnviado = await responder(rt, pedido.cmdId, { estado: 'recusado', code: 'recusado_no_aparelho', agora });
  if (typeof engine.pushState === 'function') engine.pushState();
  return { ok: true, reciboEnviado };
}

function caminhoDosFeitos() { return ARQUIVO; }

export default { emitir, desfechoDe, alvoDoComando, cicloDosComandos, confirmarDesignacao, recusarDesignacao, caminhoDosFeitos, lerFeitos, NO, NO_RECIBO, TTL_PADRAO_MS };
export { emitir, desfechoDe, alvoDoComando, cicloDosComandos, confirmarDesignacao, recusarDesignacao, caminhoDosFeitos, lerFeitos, NO, NO_RECIBO, TTL_PADRAO_MS };
