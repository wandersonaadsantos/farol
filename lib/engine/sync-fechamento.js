...
// A sessão que a TRANSFERÊNCIA encerra não é desfecho daqui: o item acabou de ser
// publicado de novo, com a preferência pelo destino, e fechá-lo no fim da sessão apagaria
// justamente esse registro. A marca é posta antes do cancelamento e consumida no fim da
// sessão (medido na bancada com engines reais, 17/09/2026).
function transferidas(engine) {
  if (!(engine.sessoesTransferidas instanceof Set)) engine.sessoesTransferidas = new Set();
  return engine.sessoesTransferidas;
}

function marcarTransferida(engine, key) {
  transferidas(engine).add(String(key || ''));
}

function foiTransferida(engine, key) {
  return transferidas(engine).has(String(key || ''));
}

function soltarTransferida(engine, key) {
  return transferidas(engine).delete(String(key || ''));
}

export default { fecharItem, esquecerConcluidos, faxinaDaFila, marcarTransferida, foiTransferida, soltarTransferida, NO_FILA, NO_ATRIBUICAO };
export { fecharItem, esquecerConcluidos, faxinaDaFila, marcarTransferida, foiTransferida, soltarTransferida, NO_FILA, NO_ATRIBUICAO };
