...
// teste sem dormir.
//
// A base é o HOME e não o STATE_DIR: state/ fica dentro do workspace em que a
// sessão de revisão roda com permissão liberada, e o protocolo já manda o modelo
// escrever lá. Com o cache ali, uma sessão da empresa B lê o corpo dos cards da
// empresa A, que é a leitura cruzada que este recurso existe pra impedir.
...
function cardCachePath(siteId, key) {
  return path.join(HOME, JIRA.CACHE_DIR, sanitizar(siteId), `${sanitizar(key)}.json`);
}
