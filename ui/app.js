...
const RELEASE_NOTES = [
  ['2.45.1', ['Migração pra ESM puro: todo arquivo .js é agora módulo nativo (zero CommonJS), a validação de sintaxe passa a usar node --check, e engines declarado >=22.12. O código perdeu o truque de carga dupla da UI e ficou mais limpo.', 'Gate de qualidade automático com ratchet: npm run lint compara o código atual contra a baseline gravada, monitora 10 regras (porta, env direto, JSON cru, catch vazio, tempo mágico, profundidade, ternário aninhado, tamanho, stringify e referência de card), e reprova regressão em qualquer uma. Extração e refino deste ciclo: check() de 297 pra 74 linhas (pura), handler de sessão achatado, e reduções em portaLiteral (6→0), processEnvDireto (19→11), jsonParseCru (32→26), emptyCatch (20→17), tempoMagico (12→10) e profundidadeExcedida (87→82).']],
