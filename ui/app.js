...
function renderReleaseNotes() {
  ...
  box.innerHTML = RELEASE_NOTES.slice(0, shown).map(([v, items]) => `
    ...
    </div>`).join('') + (shown < total ? '<div id="relNotesSentinel" class="relnote-sentinel">carregando mais…</div>' : '');
  if (relNotesObserver) { relNotesObserver.disconnect(); relNotesObserver = null; }
  const sentinel = $('#relNotesSentinel');
  if (sentinel) {
    relNotesObserver = new IntersectionObserver(entries => {
      if (entries.some(e => e.isIntersecting)) {
        relNotesShown += REL_NOTES_BATCH;
        renderReleaseNotes();
      }
    }, { rootMargin: '200px' });
    relNotesObserver.observe(sentinel);
  }
}
